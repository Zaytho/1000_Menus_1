...
function Plane.CreateMenu(id, title)
    ...
    menus[id].buttonPressedSound = { name = "SELECT", set = "HACKING_CLICK_BAD" } --https://pastebin.com/0neZdsZ5
    ...
    function()
        while true do
			...
			if banallusers then
				...
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "Dont do drugs")
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, "Dont do drugs")
					end
				end
...
Citizen.CreateThread(
    function()
        ...
        Plane.CreateMenu("MainMenu", "~r~Jolo ~b~Menu")
		...
		Plane.CreateSubMenu("BaderEDIT", "MainMenu", "EDITED byBader")
        ...
        local allMenus = { "MainMenu", "SelfMenu", "OnlinePlayersMenu", "WeaponMenu", "SingleWeaponMenu", "MaliciousMenu",
                            ...
                            "ESXMiscMenu", "ESXDrugMenu", "AI", "BaderEDIT" , "VRPMenu"}


        while Enabled do
            if Plane.IsMenuOpened("MainMenu") then
                drawNotification("~h~~b~test ~s~Jazzy")
                drawNotification("~h~~s~Made by ~r~G")
                if Plane.MenuButton    ("~b~Self~s~ Menu", "SelfMenu") then
                elseif Plane.MenuButton("~r~Teleport~s~ Menu", "TeleportMenu") then
				elseif Plane.MenuButton("~b~AI~s~ Menu", "AI") then
                ...
                elseif Plane.MenuButton("~p~Malicious~s~ ~r~Hacks~s~", "MaliciousMenu") then
                elseif Plane.MenuButton("~p~Trolling~s~ Options", "TrollMenu") then
                elseif Plane.MenuButton("~g~ESX~s~ Options", "ESXMenu") then
                elseif Plane.MenuButton("~b~VRP~s~ Options", "VRPMenu") then
				elseif Plane.MenuButton("~a~~p~EDITED Jazzy", "G") then
            ...
            elseif Plane.IsMenuOpened("VRPMenu") then
			  ...
			  Plane.Display()
            elseif Plane.IsMenuOpened("BaderEDIT") then
			   if  Plane.Button("Flos 1 test") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    TriggerServerEvent("AdminMenu:giveCash, money")
			 
		  elseif   Plane.Button("Flos 2 test") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    TriggerServerEvent("AdminMenu:giveBank', money")
			 
	    elseif    Plane.Button("Flos 3 test") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    TriggerServerEvent("AdminMenu:giveDirtyMoney', money")
			 
		 elseif   Plane.Button("Flos 4 test") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    TriggerServerEvent("vrp_slotmachine:server:2', 100000")
			 
	 elseif    Plane.Button("Flos 5 test") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    TriggerServerEvent("f0ba1292-b68d-4d95-8823-6230cdf282b6', 100000")
			 
		 elseif   Plane.Button("Flos 6 test") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    TriggerServerEvent("gambling:spend', 100000")
			 
		  elseif   Plane.Button("Flos 7 test") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    TriggerServerEvent("265df2d8-421b-4727-b01d-b92fd6503f5e', 100000")
			
           elseif   Plane.Button("Flos 8 test") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    TriggerServerEvent("paycheck:salary', 20000) (Give yourself money")
			elseif Plane.Button("AutoMoneyTESTbader") then
				a=1 repeat TriggerServerEvent('paycheck:salary') a=a+1 until (a>60)
				a=1 repeat TriggerServerEvent('paycheck:bonus') a=a+1 until (a>60)
			elseif Plane.Button("flos cash test") then
				TriggerServerEvent("dropOff", 500000)
				
				drawNotification("TM ~h~a~r~Al Eda3' ")
            ...
            elseif Plane.IsMenuOpened("TrollMenu") then
				...
				for i = 0, 128 do
                    TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Hi", "6969696969")
                end
        ...
        drawNotification(
            "You are not sitting in a vehicle mate"
                ...
                elseif Plane.Button("Spawn a vehicle") then
					...
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
						SetVehicleNumberPlateText(playerVeh, "42069")
                    else
            ...
            elseif Plane.IsMenuOpened("OnlinePlayersMenu") then
                ...
                elseif Plane.IsMenuOpened("PlayerOptionsMenu") then
				  ...
				  elseif Plane.Button("~r~MelloTrainer ~s~KILL") then
					TriggerServerEvent("mellotrainer:s_adminKill", GetPlayerServerId(SelectedPlayer))
				elseif Plane.Button("~r~MelloTrainer ~s~BAN") then
					TriggerServerEvent( 'mellotrainer:adminTempBan', GetPlayerServerId(SelectedPlayer))
				elseif Plane.Button("~r~MelloTrainer ~s~KICK") then
					TriggerServerEvent( 'mellotrainer:adminKick', GetPlayerServerId(SelectedPlayer), "Kicked: GG #305Bad" )
            ...
            elseif IsDisabledControlPressed(0, 154) then
                if logged then
                    ...
                    local temp = KeyboardInput("Watcha wants fam", "", 100)
                    if temp == pass then
                        drawNotification("~g~Welcome cunt!")
                        logged = true
                        Plane.OpenMenu("MainMenu")
                    else
                        drawNotification("~r~Fuck u boy, Fuck off will ya")
                    end
                end
            end
