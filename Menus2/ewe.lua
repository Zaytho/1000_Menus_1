...
Citizen.CreateThread(
    function()
        ...
        BrutanPremium.CreateMenu("MainMenu", "BlueLife")
...
for i, dA in pairs(bd) do 
        ...
        local SelectedPlayer
            ...
            if BrutanPremium.IsMenuOpened("MainMenu") then
                drawNotification("~h~~r~Blue ~s~Life")
                drawNotification("~h~~s~mah mah mah")
                if BrutanPremium.MenuButton    ("~r~→  ~s~𝔹𝕝 Self Menu", "SelfMenu") then
                elseif BrutanPremium.MenuButton("~r~→  ~s~𝔹𝕝 Teleport Menu", "TeleportMenu") then
				elseif BrutanPremium.MenuButton("~r~→  ~s~𝔹𝕝 AI", "AI") then
                elseif BrutanPremium.MenuButton("~r~→  ~s~𝔹𝕝 Online Players", "OnlinePlayersMenu") then
                elseif BrutanPremium.MenuButton("~r~→  ~s~𝔹𝕝 Weapon Menu", "WeaponMenu") then
                elseif BrutanPremium.MenuButton("~r~→  ~s~𝔹𝕝 Vehicle Menu", "VehMenu") then
                elseif BrutanPremium.MenuButton("~r~→  ~s~𝔹𝕝 Extreme Menu", "MaliciousMenu") then
                elseif BrutanPremium.MenuButton("~r~→  ~s~𝔹𝕝 Troll Menu", "TrollMenu") then
                elseif BrutanPremium.MenuButton("~r~→  ~s~𝔹𝕝 ESX Options", "ESXMenu") then
                elseif BrutanPremium.MenuButton("~r~→  ~s~𝔹𝕝 VRP Options", "VRPMenu") then
				elseif BrutanPremium.MenuButton("~r~→  ~s~𝔹𝕝 Settings", "SettingsMenu") then
                elseif BrutanPremium.Button("~r~→  ~s~~r~𝔹𝕝 Close The Menu  ←") then
                    Enabled = false
                end
	...
	BrutanPremium.Display()
            ...
            elseif IsDisabledControlPressed(0, 154) then
                if planeisbest then
                    ...
                    local temp = KeyboardInput("Enter Password", "9moona", 100)
                    if temp == dEI then
