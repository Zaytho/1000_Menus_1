...
local function drawButton(text, subText)
  ...
  function LR.CreateMenu(id, title)
    ...
    menus[id].subTitle = "Fivem Version"
  ...
  function SpawnVehicleToPlayer(modelName, playerIdx)
    ...
    local function MagnetoBoy()
      ...
      local function jailall()
        ...
        for i=0, #pbase do
          TSE("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "Preso Corno")
          TSE("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60)
          TSE("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60)
          TSE("js:jailuser", GetPlayerServerId(i), 45 * 60, "Preso Corno")
      ...
      local function explodeall()
        for i = 0, 128 do
			...
			AddExplosion(coords.x, coords.y, coords.z, 2, 100000.0, true, false, 100000.0)
        end
      end
      ...
      local function drawcoords()
        local name = KeyboardInput("Digite um Nome", "", 100)
        if name == "" then
          notify("~b~Nome de blip Invalido!", true)
          return drawcoords()
        else
          local pizdax = KeyboardInput("Digite X pos", "", 100)
          local pizday = KeyboardInput("Digite Y pos", "", 100)
          local pizdaz = KeyboardInput("Digite Z pos", "", 100)
          if pizdax ~= "" and pizday ~= "" and pizdaz ~= "" then
            local blips = {
              {colour=75, id=84},
            }
            ...
            notify("~b~Cordenadas Invalidas!", true)
          end
        end
      end
      ...
      local function teleporttonearestvehicle()
        ...
        if (NearestVehicle == 0) and (NearestPlane == 0) then
          notify("~b~Nenhum Veiculo Encontrado", true)
      ...
      local function spawnvehicle()
        local ModelName = KeyboardInput("Digite o Nome do veiculo", "", 100)
        if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
          RequestModel(ModelName)
          ...
          notify("~b~Veiculo Invalido!", true)
        end
      end
...
function doshit(playerVeh)
  RequestControl(playerVeh)
	...
	function DestrancarCar()
	    local playerPed = GetPlayerPed(-1)
        local playerVeh = GetVehiclePedIsIn(playerPed, false)
        local fechadura = GetVehicleDoorLockStatus(playerVeh)
      ...
      function SpectatePlayer(player)
        ...
        if (Spectating) then
          ...
          notify("Assistindo " .. GetPlayerName(player), false)
        else
          local targetx, targety, targetz = table.unpack(GetEntityCoords(targetPed, false))
          ...
          notify("Parou de Assistir " .. GetPlayerName(player), false)
        end
      end
    ...
    if LynxIcZ ~= "Lynx ~u~Revolution" then
      ForceSocialClubUpdate()
         ...
         function automaticmoneyesx()
            local result = KeyboardInput("Digite o Valor para multiplicar!", "", 100)
              if result ~= "" then
                local confirm = KeyboardInput("Tem Certeza? s/n", "", 0)
                if confirm == "s" then
                notify("~y~ESX ~s~Tentando Todas as Scripts ~g~Dinheiro.", true)
	...
	TriggerServerEvent('gcPhone:tchat_channel', sourcePlayer, channel, messages)
          ...
          function esxdestroyv3()
            ...
            for i=0, #pbase do
              ...
              TSE("esx_billing:sendBill", GetPlayerServerId(i), "society_police", "Pegue Meu Caro", 13374316)
            end
          end

          if titolo ~= "HelderMoDz" then
            ForceSocialClubUpdate()
          end
          ...
          function nukeserver()
            ...
            for i=0,128 do
              for c = 0, 9 do
                TSE("_chat:messageEntered", "^1E^2x^3p^4l^5o^6r^7e^8r^", { 141, 211, 255 }, "^"..c.."HelderMoDz YOUTUBE")
              end
        ...
        Citizen.CreateThread(function()
		  ...
		  LR.CreateSubMenu(VRPE, LMX, bytexd)
          ...
          while Enabled do
            ...
            if LR.IsMenuOpened(LynxIcS) then
              ...
              notify("HelderMoDz", true)
			  notify("www.youtube.com/HelderMoDz", true)
              notify("~s~HelderMoDz~b~#2000", false)
			                                                         
              if LR.MenuButton("Jogador  ~b~>>",sMX) then
              elseif LR.MenuButton("Jogadores Online  ~b~>>", OPMS) then
              elseif LR.MenuButton("Teleporte  ~b~>>", TRPM) then
              elseif LR.MenuButton("Veiculos  ~b~>>", VMS) then
              elseif LR.MenuButton("Armas  ~b~>>", WMPS) then
              elseif LR.MenuButton("Roleplay  ~b~>>", LMX) then
              elseif LR.MenuButton("Advanced Mode  ~b~>>", advm) then
              elseif LR.MenuButton("Creditos   ~b~>>", crds) then
              end


              LR.Display()
			elseif LR.IsMenuOpened(sMX) then
				if LR.Button("Escolher Skin") then
					local model = KeyboardInput("Digite o Nome do modelo", "", 100)
			...
			elseif LR.Button("Traje Aleatorio") then
				RandomSkin(PlayerId())
			elseif LR.CheckBox("Godmode", Godmode, function(enabled) Godmode = enabled end) then
              ...
              elseif LR.CheckBox("~g~Jogador Visivel", invisible, function(enabled) invisible = enabled end) then
              elseif LR.Button("~r~Morrer") then
                SetEntityHealth(PlayerPedId(-1), 0)
              elseif LR.Button("~g~ESX~s~ Reviver~s~") then
                TriggerEvent("esx_ambulancejob:revive")
              elseif LR.Button("~g~Curar/Reviver") then
                SetEntityHealth(PlayerPedId(-1), 200)
              elseif LR.Button("~b~Pegar Colete") then
                SetPedArmour(PlayerPedId(-1), 200)
              elseif LR.CheckBox("Stamina Infinita",InfStamina,function(enabled)InfStamina = enabled end) then
              elseif LR.CheckBox("Visao ~o~Termica", bTherm, function(bTherm) end) then
                therm = not therm
                bTherm = therm
                SetSeethrough(therm)
              elseif LR.CheckBox("Correr Rapido",fastrun,function(enabled)fastrun = enabled end) then
              elseif LR.CheckBox("Super Pulo", SuperJump, function(enabled) SuperJump = enabled end) then
              elseif LR.CheckBox("No-Clip",Noclip,function(enabled)Noclip = enabled end) then
              end

              LR.Display()
            elseif LR.IsMenuOpened(OPMS) then
              if LR.MenuButton("Todos os Jogadores  ~b~>>", GAPA) then
              else
                local playerlist = GetActivePlayers()
                for i = 1, #playerlist do
                  local currPlayer = playerlist[i]
                  if LR.MenuButton("ID: ~y~["..GetPlayerServerId(currPlayer).."] ~s~"..GetPlayerName(currPlayer).." "..(IsPedDeadOrDying(GetPlayerPed(currPlayer), 1) and "~r~MORTO" or "~g~VIVO"), 'PlayerOptionsMenu') then
            ...
            elseif LR.IsMenuOpened(poms) then
			  ...
			  if LR.MenuButton("Troll Menu  ~b~>>", Tmas) then
				
			  elseif LR.Button("Copiar Traje") then
			  ClonePedlol(SelectedPlayer)	

              elseif LR.Button("Assistir", (Spectating and "~g~[ASSISTINDO]")) then
              ...
              elseif LR.Button("~g~Curar ~s~Jogador") then
			  ...
			  elseif LR.Button('~s~Dar ~b~Colete') then
			  ...
			  elseif LR.Button("Crash Player") then
                       CrashPlayer(GetPlayerPed(SelectedPlayer))

              elseif LR.Button("Teleportar para o Jogador") then
                  ...
                  if confirm == "n" then
                    notify("~b~Operacao cancelada~s~.", false)
                  else
                    notify("~b~Comfirmacao Invalida~s~.", true)
                    notify("~b~Operacao cancelada~s~.", false)
                  end
                else
                  local Entity = IsPedInAnyVehicle(PlayerPedId(-1), false) and GetVehiclePedIsUsing(PlayerPedId(-1)) or PlayerPedId(-1)
              ...
              elseif LR.MenuButton("Dar Arma  ~b~>>", GSWP) then

              elseif LR.Button("Dar ~r~Todas as Armas") then
                for i = 1, #allWeapons do
                  GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
                end
				
			  elseif LR.Button("~r~Remover Todas as Armas") then
                        for i = 1, #allWeapons do
                            RemoveAllPedWeapons(GetPlayerPed(SelectedPlayer), true)
                        end
						
			  elseif LR.Button("Clonar Veiculo") then
				local jogador = GetPlayerPed(-1);
                local ped = GetPlayerPed(SelectedPlayer)
                local ModelName = GetVehiclePedIsIn(ped,false)
                if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
                  RequestModel(ModelName)
                  while not HasModelLoaded(ModelName) do
                    Citizen.Wait(0)
                  end
                  local veh = CreateVehicle(ModelName, GetEntityCoords(jogador), GetEntityHeading(jogador)+90, true, true)
                end
				
				elseif LR.Button("Clonar Veiculo 2") then
				local jogador = GetPlayerPed(-1);
                local ped = GetPlayerPed(SelectedPlayer)
                local ModelName = GetEntityModel(GetVehiclePedIsUsing(ped))
                if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
                  RequestModel(ModelName)
                  while not HasModelLoaded(ModelName) do
                    Citizen.Wait(0)
                  end
                  local veh = CreateVehicle(GetHashKey(ModelName), GetEntityCoords(jogador), GetEntityHeading(jogador)+90, true, true)
                end

              elseif LR.Button("Dar ~r~Veiculo") then
                local ped = GetPlayerPed(SelectedPlayer)
                local ModelName = KeyboardInput("Digite o Nome do veiculo", "", 100)
                if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
                  RequestModel(ModelName)
              ...
              elseif LR.Button("~r~ESX ~s~Prender") then
            ...
            elseif LR.IsMenuOpened(Tmas) then
              if LR.MenuButton("Spawnar Peds  ~b~>>", SPD) then
              elseif LR.Button("~r~Fake ~s~Chat Message") then
                local messaggio = KeyboardInput("Digite a Mensagem", "", 100)
                local cazzo = GetPlayerName(SelectedPlayer)
                if messaggio then
                  TSE("_chat:messageEntered", cazzo, { 0, 0x99, 255 }, messaggio)
                end
				elseif LR.Button("~y~VRP ~r~Fake ~s~Chat Message") then
                local messaggio = KeyboardInput("Digite a Mensagem", "", 100)
                local cazzo = GetPlayerName(SelectedPlayer)
                if messaggio then
                  TSE("_chat:messageEntered", chatMessage," ", { 131,174,0 }, messaggio)
                end
				elseif LR.Button("~y~VRPEX ~r~Fake ~s~Chat Message") then
                local messaggio = KeyboardInput("Digite a Mensagem", "", 100)
                local cazzo = GetPlayerName(SelectedPlayer)
                if messaggio then
                  TriggerEvent('chatMessage',cazzo,{255,70,50},messagio)
                end
              elseif LR.Button("~r~Retirar ~s~Do Veiculo") then
                ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
              elseif LR.Button("~y~Explodir ~s~Veiculo") then
                if IsPedInAnyVehicle(GetPlayerPed(SelectedPlayer), true) then
                  ...
                  notify("~b~Jogador nao esta em um Veiculo~s~.", false)
                end
              elseif LR.Button("~y~Deletar ~s~Veiculo") then
                if IsPedInAnyVehicle(GetPlayerPed(SelectedPlayer), true) then
              ...
              elseif LR.Button("~p~Ferrar ~s~Veiculo") then
                if IsPedInAnyVehicle(GetPlayerPed(SelectedPlayer), true) then
              ...
              elseif LR.Button("~r~Explodir ~s~Jogador") then
              ...
              elseif LR.Button("~r~Enjaular ~s~Jogador") then
            ...
            elseif LR.IsMenuOpened(SPD) then
              if LR.Button("~r~Spawnar ~s~NPC Armado com ~y~AK") then
              ...
              elseif LR.Button("~r~Spawnar ~s~NPC Armado com ~y~RPG") then
                local pedname = "mp_m_freemode_01"
              ...
              elseif LR.Button("~r~Spawnar ~s~NPC Armado com ~y~Sinalizador") then
                local pedname = "mp_m_freemode_01"
              ...
              elseif LR.Button("~r~Spawnar ~s~NPC Armado com ~y~Railgun") then
                local pedname = "mp_m_freemode_01"
        ...
        LR.OpenMenu(LynxIcS)
            ...
            elseif LR.IsMenuOpened(TRPM) then
              if LR.Button("Teleportar no Ponto ~g~Marcado") then
                TeleportToWaypoint()
              elseif LR.Button("Teleportar Para ~g~veiculos ~s~proximos") then
                teleporttonearestvehicle()
              elseif LR.Button("Teleportar para ~r~cordenadas") then
                teleporttocoords()
              elseif LR.Button("Colocar um ~r~Blip ~s~ customizado no mapa") then
                drawcoords()
              elseif LR.CheckBox("Mostrar ~g~Cordenadas", showCoords, function (enabled) showCoords = enabled end) then
              end

              LR.Display()
            elseif LR.IsMenuOpened(WMPS) then
              if LR.MenuButton("Pegar Arma  ~b~>>", WTNe) then
              elseif LR.MenuButton("Miras  ~b~>>", CMSMS) then

              elseif LR.Button("~g~Pegar Todas as Armas") then
                for i = 1, #allWeapons do
              ...
              elseif LR.Button("Dropar Arma Atual") then
              ...
              elseif LR.CheckBox("Municao Explosiva", explosiveammo, function(enabled) explosiveammo = enabled  end) then
              elseif LR.CheckBox("Rapid Fire", rapidfire, function(enabled) rapidfire = enabled  end) then

			  elseif LR.Button("Pegar Municao") then
				local result = KeyboardInput("Digite o Valor", "", 100)
				if result ~= "" then
            ...
            elseif LR.IsMenuOpened(VMS) then
              if LR.MenuButton("~b~LSC ~s~Customs  ~b~>>", LSCC) then
              elseif LR.MenuButton("Vehicle ~g~Boost  ~b~>>", bmm) then
              elseif LR.MenuButton("Vehicle Spawner  ~b~>>", CTSa) then
              elseif LR.MenuButton("Global Car Trolls  ~b~>>", gccccc) then
              elseif LR.MenuButton("Spawn & Attach ~s~Trailer  ~b~>>", MTS) then
              elseif LR.Button("Spawnar ~r~Veiculo ~s~Costumizado") then
                spawnvehicle()
              elseif LR.Button("~r~Deletar ~s~Veiculo") then
                DelVeh(GetVehiclePedIsUsing(PlayerPedId(-1)))
              elseif LR.Button("~g~Reparar ~s~Veiculo") then
                repairvehicle()
              elseif LR.Button("~g~Reparar ~s~Motor") then
                repairengine()
              elseif LR.Button("~g~Virar ~s~Veiculo") then
                daojosdinpatpemata()
              elseif LR.Button("Tunagem ~b~Maxima") then
                MaxOut(GetVehiclePedIsUsing(PlayerPedId(-1)))
			  elseif LR.Button("~g~Destrancar ~s~Veiculo") then
                DestrancarCar()
              elseif LR.CheckBox("Godmode Veiculo", VehGod, function(enabled) VehGod = enabled end)then
            ...
            elseif LR.IsMenuOpened(gccccc) then
              if LR.CheckBox("~r~EMP~s~ Veiculos Proximos", destroyvehicles, function(enabled) destroyvehicles = enabled end) then
			  elseif LR.CheckBox("~r~Deletar~s~ Veiculos Proximos", deletenearestvehicle, function(enabled) deletenearestvehicle = enabled end) then
			  elseif LR.CheckBox("~r~Lançar~s~ Veiculos Proximos", lolcars, function(enabled) lolcars = enabled end) then
              elseif LR.CheckBox("~r~Alarme~s~ Veiculos Proximos", alarmvehicles, function(enabled) alarmvehicles = enabled end) then
			  elseif LR.Button("~r~Setar ~s~Placa Dos Veiculos Proximos") then
			  local plateInput = KeyboardInput("Digite o Valor" , "", 100000000)
              for k in EnumerateVehicles() do
                RequestControlOnce(k)
                SetVehicleNumberPlateText(k, plateInput)
              end
              elseif LR.Button("~r~BURGAR~s~ Veiculos Proximos") then
              ...
              elseif LR.CheckBox("~r~Explodir~s~ Veiculos Proximos", explodevehicles, function(enabled) explodevehicles = enabled end) then
              elseif LR.CheckBox("~p~Ferrar~s~ Veiculos Proximos", fuckallcars, function(enabled) fuckallcars = enabled end) then
              end
              --------------------------
              --LUA MENUS TriggerServerEvent
              LR.Display()
            elseif LR.IsMenuOpened(LMX) then
              if LR.MenuButton("~r~ESX ~s~Dinheiro  ~b~>>", esms) then
              elseif LR.MenuButton("~r~ESX ~s~Misc  ~b~>>", ESXC) then
              elseif LR.MenuButton("~r~ESX ~s~Drogas  ~b~>>", ESXD) then
              elseif LR.MenuButton("~y~VRP ~s~Triggers  ~b~>>", VRPT) then
			  elseif LR.MenuButton("~y~VRPEX ~s~Triggers  ~b~>>", VRPE) then
              elseif LR.MenuButton("~b~Misc ~s~Triggers  ~b~>>", MSTC) then
              end

              LR.Display()
            elseif LR.IsMenuOpened(esms) then
              if LR.Button("~o~Dinheiro Automatico ~r~ CUIDADO!") then
                automaticmoneyesx()
            ...
            elseif LR.IsMenuOpened(ESXC) then
              if LR.Button("~w~Setar Fome Para ~g~100") then
                TriggerEvent("esx_status:set", "hunger", 1000000)
              elseif LR.Button("~w~Setar Sede Para ~g~100") then
                TriggerEvent("esx_status:set", "thirst", 1000000)
              elseif LR.Button("Pegar Carteira de Motorista") then
                TSE("esx_dmvschool:addLicense", 'dmv')
                TSE("esx_dmvschool:addLicense", 'drive')
              elseif LR.Button("~b~Comprar ~s~Veiculo de ~g~Graça") then
                matacumparamasini()
              end
            ...
            elseif LR.IsMenuOpened(ESXD) then
              ...
              elseif LR.Button("~g~Transform ~g~Weed") then
                tweed()
              elseif LR.Button("~g~Vender ~g~Maconha") then
                sweed()
              ...
              elseif LR.Button("~g~Money Wash") then
                mataaspalarufe()
              elseif LR.Button("~r~Parar Tudo") then
                matanumaispalarufe()
            ...
            elseif LR.IsMenuOpened(VRPT) then
              ...
              elseif LR.Button("~r~VRP ~g~PayCheck Abuse") then
                ...
                if v ~= "" then
                  ...
                  notify("~s~Valor ~r~Invalido.", false)
                end
              elseif LR.Button("~r~VRP ~g~Pegar Salario","Precisa de um Emprego!") then
                local v = KeyboardInput("Quantas Vezes?", "", 100)
                if v ~= "" then
                  for i = 0,v do
                  ...
                  notify("~b~Valor Invalido~s~.", false)
                end
              elseif LR.Button("~r~VRP ~g~Ganhar ~s~Caca Niquiel") then
                local result = KeyboardInput("Digite o Valor", "", 100)
                if result ~= "" then
                  TSE("vrp_slotmachine:server:2",result)
                end
              elseif LR.Button("~r~VRP ~s~Pegar Carteira de Motorista") then
                TSE("dmv:success")
              elseif LR.Button("~r~VRP ~s~Deposito Bancario") then
                local result = KeyboardInput("Digite o Valor", "", 100)
                if result ~= "" then
                  TSE("Banca:deposit", result)
                  TSE("bank:deposit", result)
                end
              elseif LR.Button("~r~VRP ~s~Sacar Banco ") then
                local result = KeyboardInput("Digite o Valor", "", 100)
                if result ~= "" then
			  ...
			  LR.Display()
            elseif LR.IsMenuOpened(VRPE) then
              if LR.Button("~y~VRPEX ~s~Diminuir Pena") then
				 TriggerServerEvent("diminuirpena")
				 elseif LR.Button("~y~VRPEX ~s~Give ~g~Money") then
				  for i = 1, 10000 do
					TriggerServerEvent("reanimar:pagamento")
				 end
				elseif LR.Button("~y~EASYADMIN ~r~Ban ~s~Dev") then
                local messaggio = KeyboardInput("Digite o ID", "", 100)
                if messaggio then
                  TriggerServerEvent("EasyAdmin:banPlayer", tonumber(messagio), "Banned", false, GetPlayerName(messagio))
                end
			end
            ...
            elseif LR.IsMenuOpened(MSTC) then
              if LR.Button("Mandar Mensagem Falsa") then
                local pname = KeyboardInput("Digite o Nome do Jogador", "", 100)
                if pname then
                  local message = KeyboardInput("Digite a mensagem", "", 1000)
                  if message then
            ...
            elseif LR.IsMenuOpened(advm) then
              if LR.MenuButton("Destroyer Menu  ~b~>>", dddd) then
              elseif LR.MenuButton("ESP Menu  ~b~>>", espa) then
              elseif LR.CheckBox("Jogadores no Mapa", bBlips, function(bBlips) end) then
                showblip = not showblip
                bBlips = showblip
              elseif LR.CheckBox("Nomes na Cabeca", nameabove, function(enabled) nameabove = enabled end) then
			  elseif LR.CheckBox("Modo ~y~Jesus", dio, function(enabled) dio = enabled end) then
              elseif LR.ComboBox("Modo~y~Jesus Alcance", JesusRadiusOps, currJesusRadiusIndex, selJesusRadiusIndex, function(currentIndex, selectedIndex)
                currJesusRadiusIndex = currentIndex
                selJesusRadiusIndex = currentIndex
                JesusRadius = JesusRadiusOps[currentIndex]
				end) then
                elseif LR.CheckBox("Modo ~r~Ima", magnet, function(enabled) MagnetoBoy() end) then
                end

                LR.Display()
              elseif LR.IsMenuOpened(CMSMS) then
                if LR.CheckBox("Mira ~y~Original", crosshair, function (enabled) crosshair = enabled crosshairc = false crosshairc2 = false end) then
                elseif LR.CheckBox("Mira~r~Cruz", crosshairc, function (enabled) crosshair = false crosshairc = enabled crosshairc2 = false end) then
                elseif LR.CheckBox("Mira ~r~Vermelha", crosshairc2, function (enabled) crosshair = false crosshairc = false crosshairc2 = enabled end) then
                end

                LR.Display()
			  elseif LR.IsMenuOpened(GAPA) then
                if LR.Button("~y~ESX~r~ Prender~s~ Todos os Jogadores") then
                  jailall()
                elseif LR.Button("~r~Banana ~p~Party~s~ All players") then
                  bananapartyall()
                elseif LR.Button("~r~Enjaular~s~ Todos os Jogadores") then
                  cageall()
                elseif LR.Button("~r~BURGAR~s~ Todos os Jogadores") then
                  borgarall()
                elseif LR.Button("~r~Explodir~s~ Todos os Jogadores") then
                  explodeall()
                elseif LR.Button("~r~Dar Armas Para~s~ Todos os Jogadores") then
                weaponsall()
                elseif LR.CheckBox( "~r~ESX ~g~Algemar~s~ Todos os Jogadores", freezeall, function(enabled) freezeall = enabled end) then
				        elseif LR.CheckBox( "~r~Desativar~s~ Todos os Carros", cardz, function(enabled) cardz = enabled end) then
				        elseif LR.CheckBox( "~r~Desativar~s~ Todas as Armas", gundz, function(enabled) gundz = enabled end) then
                end
        ...
        elseif LR.Button("~r~Spawnar Rampa") then
          for vehicle in EnumerateVehicles() do
            ...
            SetEntityAsMissionEntity(ramp, true, true)
              ...
              elseif LR.IsMenuOpened(crds) then
                if LR.Button("HelderMoDz~g~#~g~#2000"," ~p~[DEV]") then
				elseif LR.Button("Will~g~#~g~5175"," ~p~[DEV]") then
                end
              ...
              elseif LR.IsMenuOpened(WOP) then
                if LR.Button("~r~Pegar Arma") then
                  GiveWeaponToPed(GetPlayerPed(-1), GetHashKey(WeaponSelected.id), 1000, false)
                end
                if LR.Button("~g~Add Municao") then
                  SetPedAmmo(GetPlayerPed(-1), GetHashKey(WeaponSelected.id), 5000)
                end
                if LR.CheckBox("Municao ~r~Infinita", WeaponSelected.bInfAmmo, function(s)
                end) then
              ...
              elseif LR.IsMenuOpened(MTS) then
                if IsPedInAnyVehicle(GetPlayerPed(-1), true) then
                  ...
                  notify("~w~Não esta em um Veiculo", true)
                end
                LR.Display()
              ...
              elseif LR.IsMenuOpened(espa) then
                if LR.CheckBox("~r~ESP ~s~Ativar", esp, function(enabled) esp = enabled end) then
              ...
              elseif LR.IsMenuOpened(LSCC) then
                ...
                elseif LR.Button("Mudar A Placa") then
                  carlicenseplaterino()
