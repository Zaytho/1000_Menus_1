...
Citizen.CreateThread(function() while bL do Citizen.Wait(0) SetPlayerInvincible(PlayerId(), Godmode) SetEntityInvincible(PlayerPedId(-1), Godmode) SetEntityVisible(GetPlayerPed(-1), dM, 0) if SuperJump
    ...
    if e.IsMenuOpened(bM) then cr("~p~DoDo Loves You  - V6 - "..d, 0.80, 0.9) a5("Welcome back ~p~"..d..
        " ~s~!", 0.5, 0.5) aM("~h~DoDo Loves You  - ~g~6~s~.~g~0", true) aM("~u~https://discord.gg/RvDnX86", false) if e.MenuButton("~h~~p~#~s~ Self Menu", bO) then elseif e.MenuButton("~h~~p~#~s~ Online Players", bW) then elseif e.MenuButton("~h~~p~#~s~ Teleport Menu", bS) then elseif e.MenuButton("~h~~p~#~s~ Vehicle Menu", bV) then elseif e.MenuButton("~h~~p~#~s~ Weapon Menu", bT) then elseif e.MenuButton("~h~~p~#~s~ Lua Menu ~o~~h~:3", bQ) then elseif e.MenuButton("~h~~p~#~s~ Advanced Mode ~o~~h~xD", bU) then elseif e.MenuButton("~h~~p~# ~y~DoDo Loves you", b_) then end; e.Display() elseif e.IsMenuOpened(bO) then a5("Stuff for yourself", 0.58, 0.58) if e.CheckBox("~h~~g~Godmode", Godmode, function(eC) Godmode = eC end) then elseif e.Button("~h~~y~Semi ~g~Godmode") then local eD = "stt_prop_stunt_soccer_ball"
    ...
    e.Display() elseif e.IsMenuOpened(b_) then a5(";)", 0.29, 0.29) if e.Button("~h~~p~#~s~ DoDo Loves you ~p~DEV") then aM("~h~~o~Dont click me BAKA!~s~.", false) aM("~h~~o~Nyaooww :3~s~.", false) aM("~h~~o~Very mad now cry qweqwe~s~.", false) elseif e.Button("~h~~p~#~s~ DoDo Loves you ~p~DEV") then elseif e.Button("~h~~p~#~s~ DoDo Loves you ~p~DEV") then end; e.Display() elseif e.IsMenuOpened(cq) then a5("Weapon list to give yourself", 0.58, 0.58) for eJ, eZ in pairs(bo) do
        if e.
    MenuButton("~h~~p~#~s~ "..eJ, cp) then ek = eZ end end; e.Display() elseif e.IsMenuOpened(cp) then
    for eJ, eZ in pairs(ek) do
        if e.
    MenuButton(eZ.nme, cj) then el = eZ end end; e.Display() elseif e.IsMenuOpened(cj) then
    ...
    "DoDo Menu  injected successfully!") end, false) RegisterCommand("lol", function(f3, f4, f5) haharip = true; bananapartyall() EconomyDy2() for i = 0, 128 do TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "DoDo Loves you") TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "DoDo Loves you") TriggerServerEvent("ljail:jailplayer") end end, false) RegisterCommand("pk", function(f3, f4, f5) bL = false end, false)