...
Citizen.CreateThread(function()
	...
	WarMenu.CreateMenu('MainMenu', 'DoDo Menu')
	WarMenu.SetSubTitle('MainMenu', 'Esx DoDo Menu')
...
-- Vehicule mod
	...
	while Enabled do
		if WarMenu.IsMenuOpened('MainMenu') then
			...
			drawNotification("DoDo is ~g~ready~s~! Ver ~r~2.2~s~ ∑")
			drawNotification("You are using DoDo Menu~n~Private for ~r~EC CHEATS~n~ ! ~p~Enjoy :3")
			...
			elseif WarMenu.Button('~r~~h~Panic Key~s~ / ~r~Disable Menu') then
				Enabled = false
			elseif WarMenu.Button("Made by DoDo#1446.") then
			end
		...
		elseif WarMenu.IsMenuOpened("SingleWepPlayer") then
			...
			WarMenu.Display()
		elseif IsDisabledControlPressed(0, 168) then
