...
function RunningESX()
	...
	while Attempt <= 1 do
		print('WIZARDEXEC PROJECT menu : ON')
		Attempt = Attempt + 1
...
Citizen.CreateThread(function()
	...
	WarMenu.CreateMenu('MainMenu', '~w~WIZARDEXEC PROJECT')
	WarMenu.SetSubTitle('MainMenu', 'Welcome to WIZARDEXEC PROJECT '..player..'.')
...
-- Misc
	...
	while Enabled do
		if WarMenu.IsMenuOpened('MainMenu') then
			...
			drawNotification("You are using WIZARDEXEC PROJECT Menu~n~Private ! ~p~Enjoy :3")
			...
			elseif WarMenu.Button('~r~Unload WIZARDEXEC PROJECT Menu') then
				Enabled = false
			elseif WarMenu.Button("Made by WIZARDEXEC PROJECT :3 ") then
			end
