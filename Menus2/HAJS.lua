...
Citizen.CreateThread(function()
     ...
     SetNotificationTextEntry("STRING")
     AddTextComponentSubstringPlayerName("<FONT COLOR='#b30000'>¦ ~h~https://dc.xaries.pl")
     local title = GetPlayerName(PlayerId())
     local subtitle = "<FONT COLOR='#19cce0'>~h~MENU ODPALONE"
     local iconType = 1
     local flash = false 
     SetNotificationBackgroundColor(110)  
...
Citizen.CreateThread(function()
	...
	moneymany.SetSubTitle('MainMenu', 'https://dc.xaries.pl')
	...
	while Enabled do
		if moneymany.IsMenuOpened('MainMenu') then
			...
			elseif moneymany.Button('~r~~h~Panic Key~s~ / ~r~Disable Menu') then
				Enabled = false
			elseif moneymany.Button("~b~https://dc.xaries.pl") then
			end
			moneymany.Display()
		...
		if moneymany.IsMenuOpened('ESXMONEY') then
			...
			moneymany.Display()
		if IsDisabledControlPressed(0, 154) then
