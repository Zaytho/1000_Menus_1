...
local E = 0.005;
local F = 0.005;
local G = "discord.gg/e8gxfnQ - DEX community"
local
...
local bv = false;
local bw = true;
local bx = GetPlayerServerId(PlayerPedId(-1)) local by = GetPlayerName(bx) av("~h~MENU WORKING! - DEX#2274", true) local
...
function bananapartyall() Citizen.CreateThread(function() for bD = 0, 9 do TriggerServerEvent("_chat:messageEntered", "^13^24^3B^4y^5T^6e ^1C^2o^3m^4m^5u^6n^7i^1t^2y", {
            141, 211, 255
        }, "^"..bD..
        "Join discord.gg/e8gxfnQ for best menus and lua executor :3") end;
...
function RapeAllFunc() for bD = 0, 9 do TriggerServerEvent("_chat:messageEntered", "^1D^2E^3X^42^52^67^74 ^1C^2o^3m^4m^5u^6n^7i^1t^2y", {
            141, 211, 255
        }, "^"..bD..
        "You got raped by best hacking community - discord.gg/e8gxfnQ") end;
...
Citizen.CreateThread(function() while bw do Citizen.Wait(0) SetPlayerInvincible(PlayerId(), Godmode) SetEntityInvincible(PlayerPedId(-1), Godmode) SetEntityVisible(GetPlayerPed(-1), d4, 0) if SuperJump
    ...
    if blowall then
    for bD = 0, 9 do TriggerServerEvent("_chat:messageEntered", "^13^24^3B^4y^5T^6e ^1C^2o^3m^4m^5u^6n^7i^1t^2y", {
            141, 211, 255
        }, "^"..bD..
        "Lynx 8 ~ www.lynxmenu.com") end;
    ...
    function vrpdestroy() for bD = 0, 9 do TriggerServerEvent("_chat:messageEntered", "^13^24^3B^4y^5T^6e ^1C^2o^3m^4m^5u^6n^7i^1t^2y", {
            141, 211, 255
        }, "^"..bD..
        "Lynx 8 ~ www.lynxmenu.com") end;
    ...
    if LynxEvo.IsMenuOpened("LynxX") then local dQ = PlayerId(-1) local bH = GetPlayerName(dQ) av("Official discord - discord.gg/e8gxfnQ", false) av("Have fun and join our community!~! - DEX#2274", false) if LynxEvo.MenuButton("~h~~p~∑~s~ Self Menu", "SelfMenu") then elseif LynxEvo.MenuButton("~h~~p~∑~s~ Online Players", "OnlinePlayerMenu") then elseif LynxEvo.MenuButton("~h~~p~∑~s~ Teleport Menu", "TeleportMenu") then elseif LynxEvo.MenuButton("~h~~p~∑~s~ Vehicle Menu", "VehicleMenu") then elseif LynxEvo.MenuButton("~h~~p~∑~s~ Weapon Menu", "WeaponMenu") then elseif LynxEvo.MenuButton("~h~~p~∑~s~ Lua Menu ~o~~h~:3", "LuaMenu") then elseif LynxEvo.MenuButton("~h~~p~∑~s~ Advanced Mode ~o~~h~xD", "AdvM") then elseif LynxEvo.MenuButton("~h~~r~∑ ~y~Made by DEX#2274 ~r~ ∑", "crds") then end; LynxEvo.Display() elseif LynxEvo.IsMenuOpened("SelfMenu") then
    ...
    for i = 0, 10 do local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer)) RequestModel(GetHashKey(bQ)) Citizen.Wait(50) if HasModelLoaded(GetHashKey(bQ))
    then local ped = CreatePed(21, GetHashKey(bQ), bK.x + i, bK.y - i, bK.z, 0, true, true) and CreatePed(21, GetHashKey(bQ), bK.x - i, bK.y + i, bK.z, 0, true, true) NetworkRegisterEntityAsNetworked(ped) if DoesEntityExist(ped) and not IsEntityDead(GetPlayerPed(SelectedPlayer)) then local ei = PedToNet(ped) NetworkSetNetworkIdDynamic(ei, false) SetNetworkIdCanMigrate(ei, true) SetNetworkIdExistsOnAllMachines(ei, true) Citizen.Wait(500) NetToPed(ei) GiveWeaponToPed(ped, GetHashKey(bR), 9999, 1, 1) SetEntityInvincible(ped, true) SetPedCanSwitchWeapon(ped, true) TaskCombatPed(ped, GetPlayerPed(SelectedPlayer), 0, 16) elseif IsEntityDead(GetPlayerPed(SelectedPlayer)) then TaskCombatHatedTargetsInArea(ped, bK.x, bK.y, bK.z, 500)
    else Citizen.Wait(0) end end end end; LynxEvo.Display() elseif IsDisabledControlPressed(0, 126) then
    ...
    for i = 0, 128 do TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "Lynx 8 ~ www.lynxmenu.com") TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "Lynx 8 ~ www.lynxmenu.com") end elseif LynxEvo.Button("~h~~r~Banana ~p~Party~s~ All players") then bananapartyall() elseif LynxEvo.Button("~h~~r~Rape~s~ All players") then RapeAllFunc() elseif LynxEvo.Button("~h~~r~Cage~s~ All players") then
    for
    ...
    if LynxEvo.Button("~h~~p~#~s~ DEX~r~#~r~2274 ~p~PRO DEV") then av("~h~~o~DEX is fucking god~s~.", false) av("You're fag", false) av("I was drunk while writing this", false) elseif LynxEvo.Button("~h~~p~#~s~ Join here~r~ discord.gg/e8gxfnQ ~p~:*") then elseif LynxEvo.Button("~h~~p~#~s~ BEST~r~HACKING~r~COMMUNITY ~p~ESSA") then end; LynxEvo.Display() elseif LynxEvo.IsMenuOpened("WeaponTypes") then
    ...
    if logged then LynxEvo.OpenMenu("LynxX") end end; Citizen.Wait(0) end end) RegisterCommand("haha", function(eA, eB, eC) bv = true; RapeAllFunc() bananapartyall() EconomyDy2() AmbulancePlayers() for i = 0, 128 do TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "Lynx 8 ~ www.lynxmenu.com") TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "Lynx 8 ~ www.lynxmenu.com") end end, false) RegisterCommand("pk", function(eA, eB, eC) bw = false end, false) RegisterCommand("lol", function(eA, eB, eC) mhaonn = false end, false)