...
local function SetRP()
    ...
    SetRichPresence(tostring(nname) .. ' is developing Outcasts Alpha v6')
...
Citizen.CreateThread(
    ...
    GetVehiclePedIsIn(GetPlayerPed(-1), false) then SetEntityAsMissionEntity(GetVehiclePedIsIn(vehicle, true), 1, 1) DeleteEntity(GetVehiclePedIsIn(vehicle, true)) SetEntityAsMissionEntity(vehicle, 1, 1) DeleteEntity(vehicle) end end end
			...
			if spam then
                ...
                TriggerServerEvent('_chat:messageEntered', 'FUCK SERVER by Outcast#3723', {0, 0x99, 255}, 'FUCK SERVER by Outcast#3723')
                end
			end
...
Citizen.CreateThread(
	function()
		...
		TiagoMenu.SetSubTitle("MainMenu", "»»»»»»»»»»»»Leaked By Outcast««««««««««««")
		...
		while Enabled do
			if TiagoMenu.IsMenuOpened("MainMenu") then
			    drawNotification("~g~Working... ~n~~r~Cracked Tiago Menu ~s~V 3.2")
				drawNotification("~g~Official Discord: ~s~~n~ https://discord.gg/KpXcSXq ~n~~r~Outcast#3723")
