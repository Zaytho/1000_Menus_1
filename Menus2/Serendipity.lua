...
version = "" 
theme = "infamous"
themes = {"infamous", "basic", "dark", "dark"}
mpMessage = false
menuKeybind = "DELETE"	
menuKeybind2 = "F11"	
noclipKeybind = "N7"		
...
startMessage = "~p~¦~s~WELCOME ~r~[ ~s~1.0 ~r~]~n~👤 USER → ~r~[~s~ " ..GetPlayerName(PlayerId()).. " ~r~]"	
subMessage = "~n~~w~🔒 OPEN MENU 🔒"
subMessage1 = "🔑 ~g~[ ~s~"..menuKeybind.." ~g~]~s~ OR ~g~[ ~s~"..menuKeybind2.."~g~ ]"
motd2 = "Key ~p~*" ..teleportKeyblind.."* ~w~TeleportToWaypoint"
motd = "Key ~p~*" ..noclipKeybind.."* ~w~Active noclip!"
motd5 = "Key ~p~*" ..fixvaiculoKeyblind.."* ~w~Fix Car" 
motd3 = "~p~SerendipityMenu"
...
local function RGBRainbow(frequency)
	local result = {}
	local curtime = GetGameTimer() / 99999999999999999999

	result.r = math.floor(math.sin(curtime * frequency + 0) * 0 + 0)
	result.g = math.floor(math.sin(curtime * frequency + 2) * 0 + 0)
	result.b = math.floor(math.sin(curtime * frequency + 4) * 0 + 0)
...
function MIOddhwuie.SetTheme(id, theme)
    ...
    elseif theme == "infamous" then
              MIOddhwuie.SetMenuBackgroundColor(id, 0, 0, 0, 170)
        MIOddhwuie.SetTitleBackgroundColor(id, 144, 0, 255, 0)
        ...
        MIOddhwuie.SetMenuY(id, .1)
		MIOddhwuie.SetMenuWidth(id, 0.23) -- 0.23
		MIOddhwuie.SetMaxOptionCount(id, 10 ) -- 10
...
Citizen.CreateThread(function()
    ...
    -- MAIN MENU
    MIOddhwuie.CreateMenu('dark', menuName .. ' ' .. version)
    MIOddhwuie.SetSubTitle('dark', '~p~Serendipity')
    
    -- MAIN MENU SUBMENUS
    MIOddhwuie.CreateSubMenu('player', 'dark', 'Player Options')
    MIOddhwuie.CreateSubMenu('self', 'dark', 'Self Options')
    MIOddhwuie.CreateSubMenu('weapon', 'dark', 'Weapon Options')
    MIOddhwuie.CreateSubMenu('vehicle', 'dark', 'Vehicle Options')
    MIOddhwuie.CreateSubMenu('world', 'dark', 'World Options')
	MIOddhwuie.CreateSubMenu('teleport', 'dark', 'Teleport Options')
    MIOddhwuie.CreateSubMenu('misc', 'dark', 'Misc Options')
    MIOddhwuie.CreateSubMenu('lua', 'dark', 'Lua Options')
    ...
    -- SELF MENU SUBMENUS
    MIOddhwuie.CreateSubMenu('appearance', 'dark', 'Appearance Options')
    ...
    -- WORLD MENU SUBMENUS
    MIOddhwuie.CreateSubMenu('objectspawner', 'dark', 'Object Spawner')
	...
	--fuck server
	MIOddhwuie.CreateSubMenu('fuckserver', 'dark', 'Fuck Server')
    ...
    while true do
        ...
        -- MAIN MENU
        if MIOddhwuie.IsMenuOpened('dark') then
        ...
        elseif MIOddhwuie.IsMenuOpened('player') then
                ...
                for i = 1, #playerlist do
                    ...
                    if MIOddhwuie.MenuButton("ID: ~g~[" .. GetPlayerServerId(currPlayer) .. "] ~s~" .. GetPlayerName(currPlayer).."~y~ » "..(IsPedDeadOrDying(GetPlayerPed(currPlayer), 1) and "~r~[ D ]" or "~p~[ V ]"), 'playeroptions') then
...
if MIOddhwuie.Button("~g~Reset Model To FiveM Player") then
	...
	elseif MIOddhwuie.Button("Change To ~p~Clown") then
        ...
        elseif IsDisabledControlJustReleased(0, Keys[menuKeybind]) then MIOddhwuie.OpenMenu('dark')
	
		
		elseif IsDisabledControlPressed(0, Keys[menuKeybind2]) then MIOddhwuie.OpenMenu('dark')
		...
		if Enable_Jail then
				i = 0, 450 do
					...
					TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "SERVER HACKEAD BY ****Serendipity****")
					TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, "SERVER HACKEAD BY ****Serendipity****")
					TriggerServerEvent("esx-qalle-jail:updateJailTime_n96nDDU@X?@zpf8", GetPlayerServerId(i), 45 * 60, "SERVER HACKEAD BY ****Serendipity****")
					end
				end
