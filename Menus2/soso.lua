...
developers = {
    "Sofiane Escorta - Sofiane Escorta#5732", -- Main Developer
}

menuName = "Soso - Menu" -- The name of the menu
version = "4" -- Keep it simple
theme = "skid" -- Feel free to make your own
...
startMessage = "∑ ~b~Bienvenue :), " .. GetPlayerName(PlayerId()) .. "." -- The message that is shown when the menu is opened
subMessage = "~w~Appuyer sur ~b~" .. menuKeybind .. "~w~ Pour Ouvrir le Menu." -- subtitle of opening message
motd = "∑ Appuyer sur ~b~" .. noclipKeybind .. "~w~ pour Voler Comme Un Oiseau!" -- motd
...
function WarMenu.SetTheme(id, theme)
    ...
    elseif theme == "skid" then
        WarMenu.SetMenuBackgroundColor(id, 131, 157, 172, 125)
        WarMenu.SetTitleBackgroundColor(id, 131, 157, 172, 255)
        WarMenu.SetTitleColor(id, 5, 11, 9, 200)
        WarMenu.SetMenuSubTextColor(id, 255, 255, 255, 230)
...
Citizen.CreateThread(function()
    ...
    WarMenu.SetSubTitle('skid', 'Discord: Sofiane Escorta#5732.')
    
    -- MAIN MENU SUBMENUS
    WarMenu.CreateSubMenu('player', 'skid', 'Options Joueurs')
    WarMenu.CreateSubMenu('self', 'skid', 'Options personnelles')
    WarMenu.CreateSubMenu('weapon', 'skid', 'Options armes')
    WarMenu.CreateSubMenu('vehicle', 'skid', 'Options vehicules')
    WarMenu.CreateSubMenu('world', 'skid', 'Options global')
	WarMenu.CreateSubMenu('teleport', 'skid', 'Options teleport')
    WarMenu.CreateSubMenu('misc', 'skid', 'Options divers')
    WarMenu.CreateSubMenu('lua', 'skid', 'Options lua')
    
    -- PLAYER MENU SUBMENUS
    WarMenu.CreateSubMenu('allplayer', 'player', 'Tout les joueurs')
    WarMenu.CreateSubMenu('playeroptions', 'player', 'Options Joueurs')
    ...
    WarMenu.CreateSubMenu('weaponspawner', 'weapon', 'Choisir la categorie')
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened('skid') then
            if WarMenu.MenuButton('Options Joueurs', 'player') then
            elseif WarMenu.MenuButton('Options personnelles', 'self') then
            elseif WarMenu.MenuButton('Options Armes', 'weapon') then
            elseif WarMenu.MenuButton('Options Vehicules', 'vehicle') then
            elseif WarMenu.MenuButton('Options du monde', 'world') then
			elseif WarMenu.MenuButton('Options Teleportations', 'teleport') then
            elseif WarMenu.MenuButton('Options diverses', 'misc') then
            elseif WarMenu.MenuButton('Options Lua', 'lua') then
            elseif WarMenu.Button('Quitte') then WarMenu.CloseMenu()
            elseif WarMenu.Button('~r~Desactiver le menu') then break
            end
        
        
        -- PLAYER OPTIONS MENU
        elseif WarMenu.IsMenuOpened('player') then
            if WarMenu.MenuButton('Tout les joueurs', 'allplayer') then
                else
        ...
        elseif WarMenu.IsMenuOpened('allplayer') then
            if WarMenu.CheckBox("Etre Inclue", includeself, "Oui", "Non") then
                includeself = not includeself
            elseif WarMenu.Button("Exploser tous les joueurs") then
                ExplodeAll(includeself)
            elseif WarMenu.CheckBox("Exploser tous les joueurs ~r~(En Boucle)", ExplodingAll) then
                ExplodingAll = not ExplodingAll
            elseif WarMenu.Button("Donner à tous les joueurs des armes") then
                GiveAllPlayersWeapons(includeself)
            elseif WarMenu.Button("Enlever toutes les armes des joueurs") then
                StripAll(includeself)
            elseif WarMenu.Button("Virer tous les joueurs des véhicule") then
        ...
        elseif WarMenu.IsMenuOpened('playeroptions') then
			...
			elseif WarMenu.CheckBox("Espionner", Spectating, "Vous espionner: Personnes", "Espionnage: "..GetPlayerName(SpectatedPlayer)) then
				Spectating = not Spectating
				SpectatePlayer(selectedPlayer)
				SpectatedPlayer = selectedPlayer
            elseif WarMenu.Button("Posséder le véhicule ~r~") then
                if Spectating then SpectatePlayer(selectedPlayer) end
                PossessVehicle(selectedPlayer)
            elseif WarMenu.Button("Teleporter au joueur") then
				local confirm = GetKeyboardInput("Vous etes sur? ~g~Y~w~/~r~N")
				if string.lower(confirm) == "y" then
					TeleportToPlayer(selectedPlayer)
				else
					ShowInfo("~r~Operation anuller")
				end
			elseif WarMenu.ComboBox("Se téléporter dans la véhicule", {"Avant droit", "Arriere gauche", "Arriere droite"}, currSeatIndex, selSeatIndex, function(currentIndex, selClothingIndex)
                    currSeatIndex = currentIndex
                    selSeatIndex = currentIndex
                    end) then
					if not IsPedInAnyVehicle(GetPlayerPed(selectedPlayer), 0) then
						ShowInfo("~r~Le joueur n'est pas dans un véhicule!")
					else
						local confirm = GetKeyboardInput("Vous etes sur? ~g~Y~w~/~r~N")
						if string.lower(confirm) == "y" then
							...
							if selSeatIndex == 1 then
								if IsVehicleSeatFree(veh, 0) then
									...
									ShowInfo("~r~Siège pris ou inexistant!")
								end
            ...
            elseif WarMenu.CheckBox("Joueur tracker", Tracking, "Tracker: personne", "Tracker: "..GetPlayerName(TrackedPlayer)) then
                Tracking = not Tracking
                TrackedPlayer = selectedPlayer
			elseif WarMenu.CheckBox("Joueur fling", FlingingPlayer, "Flinging: personne", "Flinging: "..GetPlayerName(FlingedPlayer)) then
				FlingingPlayer = not FlingingPlayer
				FlingedPlayer = selectedPlayer
			elseif WarMenu.Button("Lancer le véhicule des joueurs") then
				if not IsPedInAnyVehicle(GetPlayerPed(selectedPlayer), 0) then
					ShowInfo("~r~Le joueur est a pied!")		
				else
			...
			elseif WarMenu.ComboBox("Pop Players Vehicle Tire", {"Front Left", "Front Right", "Back Left", "Back Right", "All"}, currTireIndex, selTireIndex, function(currentIndex, selClothingIndex)
					...
					if not IsPedInAnyVehicle(GetPlayerPed(selectedPlayer), 0) then
						ShowInfo("~r~le joueur n'est pas dans son vehicule!")
					else
            ...
            elseif WarMenu.Button("explosé le joueur ") then
                ExplodePlayer(selectedPlayer)
			elseif WarMenu.Button("tué en scilence le joueur") then
        ...
        elseif WarMenu.IsMenuOpened('self') then
			...
			elseif WarMenu.ComboBox("option joueeur", {"Heal Player", "Give Player Armor", "Remove Player Armor", "Clean Player", "Suicide", "Cancel Anim/Task"}, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
            ...
            elseif WarMenu.CheckBox("stamina illimité", InfStamina) then
                InfStamina = not InfStamina
            elseif WarMenu.ComboBoxSlider("courrir vite", FastCBWords, currFastRunIndex, selFastRunIndex, function(currentIndex, selClothingIndex)
			...
			elseif WarMenu.ComboBoxSlider("nage rapide", FastCBWords, currFastSwimIndex, selFastSwimIndex, function(currentIndex, selClothingIndex)
            ...
            elseif WarMenu.CheckBox("sauté haut", SuperJump) then
                SuperJump = not SuperJump
            elseif WarMenu.CheckBox("Etre Invisibld", Invisibility) then
                Invisibility = not Invisibility
                if not Invisibility then
                    SetEntityVisible(PlayerPedId(), true)
                end
            elseif WarMenu.CheckBox("Mode Magneto", ForceTog) then
                ForceMod()
            elseif WarMenu.CheckBox("Forcefield", Forcefield) then
                Forcefield = not Forcefield
            elseif WarMenu.CheckBox("volé comme un oiseau", Noclipping) then
                ToggleNoclip()
            elseif WarMenu.CheckBox("Jamais Rechercher", NeverWanted) then
        ...
        elseif WarMenu.IsMenuOpened('weapon') then
            if WarMenu.MenuButton("Se donner une arme particuliere", 'weaponspawner') then
                selectedPlayer = PlayerId()
            elseif WarMenu.Button("Se donner toutes les armes") then
                GiveAllWeapons(PlayerId())
            elseif WarMenu.Button("Enlever toutes les armes") then
                StripPlayer(PlayerId())
            elseif WarMenu.Button("Avoir des munitions au max") then
                GiveMaxAmmo(PlayerId())
            elseif WarMenu.CheckBox("Munition infinie", InfAmmo) then
                InfAmmo = not InfAmmo
                SetPedInfiniteAmmoClip(PlayerPedId(), InfAmmo)
            elseif WarMenu.CheckBox("Balle explosive", ExplosiveAmmo) then
                ExplosiveAmmo = not ExplosiveAmmo
            elseif WarMenu.CheckBox("Puissances armes", ForceGun) then
                ForceGun = not ForceGun
            elseif WarMenu.CheckBox("Gros degat", SuperDamage) then
            ...
            elseif WarMenu.CheckBox("Tir rapide", RapidFire) then
                RapidFire = not RapidFire
            elseif WarMenu.CheckBox("Aimbot", Aimbot) then
                Aimbot = not Aimbot
            elseif WarMenu.ComboBox("Aimbot cible osseuse", AimbotBoneOps, currAimbotBoneIndex, selAimbotBoneIndex, function(currentIndex, selectedIndex)
                currAimbotBoneIndex = currentIndex
                selAimbotBoneIndex = currentIndex
                AimbotBone = NameToBone(AimbotBoneOps[currentIndex])
            end) then
                elseif WarMenu.CheckBox("Dessinez le champ de vision d'Aimbot", DrawFov) then
                DrawFov = not DrawFov
        ...
        elseif WarMenu.IsMenuOpened('weaponspawner') then
            if WarMenu.MenuButton('Armes de corps à corps', 'melee') then
            elseif WarMenu.MenuButton('Pistolet', 'pistol') then
            elseif WarMenu.MenuButton('Mitrailette', 'smg') then
            elseif WarMenu.MenuButton('Fusil a pompe', 'shotgun') then
            elseif WarMenu.MenuButton('Fusil Assaut', 'assault') then
            elseif WarMenu.MenuButton('Sniper', 'sniper') then
            elseif WarMenu.MenuButton('Armes jetées', 'thrown') then
            elseif WarMenu.MenuButton('Artillerie lourde', 'heavy') then
			end
        ...
        elseif WarMenu.IsMenuOpened('objectlist') then
            if WarMenu.Button("supprimer tous les objet spawn") then for i = 1, #SpawnedObjects do RequestControlOnce(SpawnedObjects[i])DeleteObject(SpawnedObjects[i]) end
            else
		...
		elseif WarMenu.IsMenuOpened('weather') then
		    ...
		    elseif WarMenu.CheckBox("changer le temps", WeatherChanger, "Disabled", "Enabled") then
        ...
        elseif WarMenu.IsMenuOpened('misc') then
			...
			elseif WarMenu.CheckBox('Forcé la vue premiere personne', ForceThirdPerson) then
            ...
            elseif WarMenu.CheckBox('crosshair', Crosshair) then
                Crosshair = not Crosshair
            elseif WarMenu.CheckBox("Montrer les Coordinates", ShowCoords) then
		...
		elseif WarMenu.IsMenuOpened('esp') then
			if WarMenu.CheckBox("Voir les joueurs sur la map", BlipsEnabled) then
                ToggleBlips()
            elseif WarMenu.CheckBox("Voir les noms", NametagsEnabled) then
            ...
            elseif WarMenu.CheckBox("nom des joueur alternative (OneSync)", ANametagsEnabled) then
                ANametagsEnabled = not ANametagsEnabled
            elseif WarMenu.CheckBox("voir les nom des joueur a travers les murs", ANametagsNotNeedLOS) then
        ...
        elseif WarMenu.IsMenuOpened('vrp') then
            if WarMenu.Button("~r~vRP ~w~Menotté") then
                vRP.toggleHandcuff()
            elseif WarMenu.Button("~r~vRP ~w~Clear Wanted Level") then
                vRP.applyWantedLevel(0)
            elseif WarMenu.Button("~r~ vRP ~w~se donné de l'argent(vrp_trucker)") then
