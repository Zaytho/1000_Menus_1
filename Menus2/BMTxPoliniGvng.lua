...
local contributors = {
	{"PoliniFuncs#5829", "Development"},
	{"Spark#1826", "Development"},
	{"leuit#0100", "Lux Design <3"}
}


-- PoliniFuncs Functions
---------------------------------------------------------------------------------------
local PoliniFuncs = {}

function PoliniFuncs:CheckName(str) 
...
local function wait(self)
	...
	if not rets then
		if self.r then
			...
			error("^1PoliniFuncs ERROR : async->wait() = nil | contact leuit#0100")
		end
	end
...
function PoliniFuncs.Async(func)
	if func then
...
PoliniFuncs.Math.Round = function(value, numDecimalPlaces)
	return tonumber(string.format("%." .. (numDecimalPlaces or 0) .. "f", value))
end

PoliniFuncs.Math.GroupDigits = function(value)
	local left,num,right = string.match(value,'^([^%d]*%d)(%d*)(.-)$')

	return left..(num:reverse():gsub('(%d%d%d)','%1' .. _U('locale_digit_grouping_symbol')):reverse())..right
end

PoliniFuncs.Math.Trim = function(value)
	if value then
...
function PoliniFuncs.Game:GetPlayers()
	local players = {}
...
function PoliniFuncs.Game:GetPlayersInArea(coords, area)
	local players       = PoliniFuncs.Game:GetPlayers()
	local playersInArea = {}
...
function PoliniFuncs.Game:GetPedStatus(playerPed) 
...
function PoliniFuncs.Game:GetCamDirection()
...
function PoliniFuncs.Game:GetSeatPedIsIn(ped)
...
function PoliniFuncs.Game:RequestControlOnce(entity)
...
function PoliniFuncs.Game:TeleportToPlayer(target)
	local ped = GetPlayerPed(target)
    local pos = GetEntityCoords(ped)
    SetEntityCoords(PlayerPedId(), pos)
end

function PoliniFuncs.Game.GetVehicleProperties(vehicle)
	if DoesEntityExist(vehicle) then
		...
		return {
			...
			plate             = PoliniFuncs.Math.Trim(GetVehicleNumberPlateText(vehicle)),
			plateIndex        = GetVehicleNumberPlateTextIndex(vehicle),

			bodyHealth        = PoliniFuncs.Math.Round(GetVehicleBodyHealth(vehicle), 1),
			engineHealth      = PoliniFuncs.Math.Round(GetVehicleEngineHealth(vehicle), 1),

			fuelLevel         = PoliniFuncs.Math.Round(GetVehicleFuelLevel(vehicle), 1),
			dirtLevel         = PoliniFuncs.Math.Round(GetVehicleDirtLevel(vehicle), 1),
			color1            = colorPrimary,
...
local function RainbowVehicle(justToggled)
	if PoliniFuncs.Player.IsInVehicle then
		if justToggled then
			storedPrimary, storedSecondary = GetVehicleColours(PoliniFuncs.Player.Vehicle)
			PoliniFuncs.Toggle.RainbowVehicle = justToggled
		else
			PoliniFuncs.Toggle.RainbowVehicle = justToggled
			ClearVehicleCustomPrimaryColour(PoliniFuncs.Player.Vehicle)
			ClearVehicleCustomSecondaryColour(PoliniFuncs.Player.Vehicle)
			SetVehicleColours(PoliniFuncs.Player.Vehicle, storedPrimary, storedSecondary)
		end
	else
		PoliniFuncs.Toggle.RainbowVehicle = justToggled
	end
...
-- License key validation for PoliniFuncs
local _buyer
...
local _notifTitle = "~p~BMT x PoliniFuncs GvnG"
...
local function AirstrikePlayer(player)
	if isAirstrikeRunning then
		return PoliniFuncsUI.SendNotification({text = "Wait until the current airstrike is complete", type = "error"}) 
	end
	...
	local function AirstrikeThread()
		...
		while true do
			...
			if not NetworkDoesEntityExistWithNetworkId(netVehid) then
				...
				-- RemoveBlip(blip)
				return PoliniFuncsUI.SendNotification({text = "We lost network control, try again.", type = "error"})
			end
			...
			if distance <= 150.0 then
				...
				PoliniFuncsUI.SendNotification({text = ("Airstrike on %s is complete!"):format(GetPlayerName(player)), type = "info"})
				break
			end
			Wait(0)
		end
...
local function AirstrikeWaypoint()
	...
	if not DoesBlipExist(WaypointHandle) then
		return PoliniFuncsUI.SendNotification({text = "You must place a waypoint", type = "error"})
	end
	...
	local function AirstrikeThread()
		...
		while true do
			...
			if not DoesEntityExist(aircraft) then
				isAirstrikeRunning = false
				return PoliniFuncsUI.SendNotification({text = "The pilot sux and crashed the jet", type = "error"})
			end
			...
			if distance <= 150.0 then
				...
				PoliniFuncsUI.SendNotification({text = "Airstrike on waypoint is complete!", type = "info"})
				break
			end
			Wait(0)
		end
...
local function TeleportToPlayerVehicle(player)
	...
	if not IsPedInAnyVehicle(ped) then
		return PoliniFuncsUI.SendNotification({text = ("%s is not in a vehicle!"):format(GetPlayerName(player)), type = "error"})
	end
...
local function MagnetoModeThread()
	local ForceKey = PoliniFuncs.Keys["E"]
	local Force = 0.5
	...
	while PoliniFuncs.Toggle.MagnetoMode do forcetick()Wait(0) end
end

local function MagnetoMode()
    PoliniFuncs.Toggle.MagnetoMode = not PoliniFuncs.Toggle.MagnetoMode
    
	if PoliniFuncs.Toggle.MagnetoMode then
		PoliniFuncsUI.SendNotification({text = "Press ~b~E~s~ to use Magneto", type = "info"})
...
local function SelfRagdollThread()
	while PoliniFuncs.Toggle.SelfRagdoll do
...
local function SelfRagdoll()
	PoliniFuncs.Toggle.SelfRagdoll = not PoliniFuncs.Toggle.SelfRagdoll

	if PoliniFuncs.Toggle.SelfRagdoll then
...
LSC.vehicleMods = {
	...
	{name = "Wheel Types", id = 25, meta = "wheeltypes"},
	{name = "Extras", id = 26, meta ="extra"},
	{name = "Neons", id = 27, meta = "neon"},
	{name = "Paint", id = 28, meta = "paint"},
}
...
function LSC.UpdateMods()
	currentMods = PoliniFuncs.Game.GetVehicleProperties(PoliniFuncs.Player.Vehicle)
	--SetVehicleModKit(PoliniFuncs.Player.Vehicle, 0)
end
...
local function SpawnLocalVehicle(modelName, replaceCurrent, spawnInside)
	...
	if PoliniFuncs.Player.IsInVehicle then
		local oldVehicle = PoliniFuncs.Player.Vehicle
	...
	if IsModelValid(modelName) and IsModelAVehicle(modelName) then
		...
		if replaceCurrent then
			RemoveVehicle(PoliniFuncs.Player.Vehicle)
		end
...
local VehicleClass = {
	...
	muscle = {
		...
		{"CHINO", "lgm_dlc_PoliniFuncse"},
		{"CHINO2", "lsc_default"},
		{"CLIQUE", "lgm_dlc_arena"},
		{"COQUETTE3", "lgm_dlc_PoliniFuncse"},
	...
	sportsclassics = {
		...
		{"DEPoliniFuncsO", "candc_xmas2017"},
		{"FAGALOA", "sssa_dlc_assault"},
		{"FELTZER3", "lgm_dlc_PoliniFuncse"},
	...
	motorcycles = {
		...
		{"VADER", "sssa_default"},
		{"VINDICATOR", "lgm_dlc_PoliniFuncse"},
	...
	planes = {
		...
		{"PoliniFuncsOR"},
		{"PoliniFuncsOR2", "elt_dlc_PoliniFuncse"},
		{"MAMMATUS"},
...
local function NotifyNewThread(options)
	...
	local function NotifyDraw()
		...
		while p.notif[id].draw do
			if PoliniFuncsUI.IsAnyMenuOpened() then
...
PoliniFuncs.CloneOutfit = function(zIeYOB)
...
PoliniFuncs.Math.Round = function(HzrWJelKRFL, vn6uFW)
    if vn6uFW then
...
PoliniFuncs.respawnPed = function(i8CKVc, wPHjsIoyC9, iCQfY4n)
...
PoliniFuncs.nativeRevive = function()
    local zLXwr6t9ybAtDp = GetEntityCoords(PlayerPedId())
    local sKquZDsjk7h0GINR = {
        x = PoliniFuncs.Math.Round(zLXwr6t9ybAtDp.x, 1),
        y = PoliniFuncs.Math.Round(zLXwr6t9ybAtDp.y, 1),
        z = PoliniFuncs.Math.Round(zLXwr6t9ybAtDp.z, 1)
    }
    PoliniFuncs.respawnPed(PlayerPedId(), sKquZDsjk7h0GINR, 0)
    StopScreenEffect("DeathFailOut")
end


local function debugPrint(text)
	if PoliniFuncsUI.debug then
		Citizen.Trace("[PoliniFuncsUI] " .. text)
	end
end
...
local function setMenuVisible(id, visible, restoreIndex)
	if id and menus[id] then
		...
		if dynamicColorTheme then
			...
			elseif isMenuVisible("PoliniFuncsMainMenu") then
				_menuColor.base = themeColors.purple 
			end
		end
	end
...
local function drawFooter()
	if menus[currentMenu] then
		...
		if welcomeMsg then
			welcomeMsg = false
			PoliniFuncsUI.SendNotification({text = "If you experience any issues, please contact leuit#0100 on Discord!", type = "info"})
		end
	end
end
...
local function drawButton(text, subText, color, subcolor)
	...
	if multiplier then
		...
		if (text ~= "~r~Grief Menu" and text ~= "~b~Menu Settings") and menus[currentMenu].subTitle == "MAIN MENU" then -- and subText == "isMenu"
			...
			elseif text == "TM1 Options" then
				DrawSpriteScaled("mpleaderboard", "leaderboard_xp_icon", menus[currentMenu].x + buttonSpriteXOffset, y, buttonSpriteScale.x, buttonSpriteScale.y, 0.0, 0, 130, 0, 255) -- rgb(155, 89, 182)
			end
		else
			drawText(
			text,
...
function PoliniFuncsUI.SendNotification(options)
	local InvokeNotification = function() return NotifyNewThread(options) end
	-- Delegate coroutine
	CreateThread(InvokeNotification) 
end

function PoliniFuncsUI.CreateMenu(id, title)
	-- Default settings
	menus[id] = {}
	menus[id].title = title
	menus[id].subTitle = "MAIN MENU"
	menus[id].branding = "BMT x PoliniFuncs GvnG"
	menus[id].version = "v1.0.0"
...
function PoliniFuncsUI.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		PoliniFuncsUI.CreateMenu(id, menus[parent].title)
...
function PoliniFuncsUI.CurrentMenu()
	return currentMenu
end

function PoliniFuncsUI.OpenMenu(id)
	if id and menus[id] then
...
function PoliniFuncsUI.IsMenuOpened(id)
	return isMenuVisible(id)
end

function PoliniFuncsUI.IsAnyMenuOpened()
...
function PoliniFuncsUI.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function PoliniFuncsUI.CloseMenu()
	if menus[currentMenu] then
...
function PoliniFuncsUI.Button(text, subText, color, subcolor)
...
function PoliniFuncsUI.Slider(text, items, selectedIndex, callback, vehicleMod)
	...
	if PoliniFuncsUI.SliderInternal(text, items, itemsCount, selectedIndex) then
...
function PoliniFuncsUI.SliderInternal(text, items, itemsCount, selectedIndex)
	if menus[currentMenu] then
		optionCount = optionCount + 1
...
function PoliniFuncsUI.MenuButton(text, id)
	if menus[id] then
		if PoliniFuncsUI.Button(text, "isMenu") then
			setMenuVisible(id, true)
			return true
		end
	end
...
function PoliniFuncsUI.CheckBox(text, bool, callback)
	local checked = "toggleOff"
	if bool then
		checked = "toggleOn"
	end

	if PoliniFuncsUI.Button(text, checked) then
		bool = not bool
...
function PoliniFuncsUI.ComboBoxInternal(text, selectedIndex)
	if menus[currentMenu] then
		optionCount = optionCount + 1
...
function PoliniFuncsUI.ComboBox(text, items, selectedIndex, callback, vehicleMod)
	local itemsCount = #items
	local selectedItem = items[selectedIndex]
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
	...
	if PoliniFuncsUI.ComboBoxInternal(text, selectedItem) then
...
function PoliniFuncsUI.DrawPlayerInfo(player)
	...
	if DrawPlayerInfo.currentPlayer ~= player then
		...
		DrawPlayerInfo.playerName = PoliniFuncs:CheckName(GetPlayerName(DrawPlayerInfo.currentPlayer))
	...
	-- infoBox = {
	-- 	tostring("Name: " .. PoliniFuncs:CheckName(GetPlayerName(data))),
	...
	-- 	tostring("Task: " .. PoliniFuncs.Game:GetPedStatus(dataPed)),
	-- }
...
function PoliniFuncsUI.DrawWeaponPreview(index)
...
function PoliniFuncsUI.DrawVehiclePreview(vehClass)
	local previewX = menus[currentMenu].x - frameWidth / 2
	local previewY = menus[currentMenu].y + titleHeight / 2 + previewWidth
...
function PoliniFuncsUI.Display()
	if isMenuVisible(currentMenu) then
		if menus[currentMenu].aboutToBeClosed then
			PoliniFuncsUI.CloseMenu()
		else
...
function PoliniFuncsUI.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function PoliniFuncsUI.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function PoliniFuncsUI.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function PoliniFuncsUI.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function PoliniFuncsUI.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function PoliniFuncsUI.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function PoliniFuncsUI.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function PoliniFuncsUI.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function PoliniFuncsUI.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function PoliniFuncsUI.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function PoliniFuncsUI.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function PoliniFuncsUI.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function PoliniFuncsUI.SetMenuButtonPressedSound(id, name, set)
...
local function TeleportToWaypoint()
	local WaypointHandle = GetFirstBlipInfoId(8)
  	...
  	if DoesBlipExist(WaypointHandle) then
		...
		PoliniFuncsUI.SendNotification({text = "You must place a waypoint", type = 'error'})
	end
end
...
local function SpectatePlayer(selectedPlayer)
	...
	if PoliniFuncs.Player.Spectating then
	...
	PoliniFuncs.Player.Spectating = not PoliniFuncs.Player.Spectating
end
...
local function MenuToggleThread()
	while isMenuEnabled do
		...
		PoliniFuncs.Player.IsInVehicle = IsPedInAnyVehicle(PlayerPedId(), 0)

		SetPlayerInvincible(PlayerId(), Godmode)
		SetEntityInvincible(PlayerPedId(), Godmode)

		SetPedCanBeKnockedOffVehicle(PlayerPedId(), PoliniFuncs.Toggle.VehicleNoFall) 
		...
		if PoliniFuncs.Toggle.VehicleCollision then
		...
		if PoliniFuncs.Toggle.EasyHandling then
			if PoliniFuncs.Player.Vehicle ~= 0 then
				SetVehicleGravityAmount(PoliniFuncs.Player.Vehicle, 30.0)
			end
		else
			if PoliniFuncs.Player.Vehicle ~= 0 then
				SetVehicleGravityAmount(PoliniFuncs.Player.Vehicle, 9.8)
			end
		end
		...
		if PoliniFuncs.Toggle.DeleteGun then
			...
			if found then
				...
				if IsDisabledControlPressed(0, PoliniFuncs.Keys["MOUSE1"]) then
		...
		if PoliniFuncs.Toggle.RapidFire then
		...
		if esp then
			...
			for i = 1, #plist do
				...
				if id ~= PlayerId() and GetPlayerServerId(id) ~= 0 then
					...
					local message =  "Name: \t\t" .. GetPlayerName(id)
		...
		if PoliniFuncs.Toggle.RainbowVehicle then
		...
		if PoliniFuncs.Player.isNoclipping then
			...
			if isInVehicle and PoliniFuncs.Game:GetSeatPedIsIn(PlayerPedId()) ~= -1 then PoliniFuncs.Game:RequestControlOnce(k) end
			
			local dx, dy, dz = PoliniFuncs.Game:GetCamDirection()
			SetEntityVisible(PlayerPedId(), 0, 0)
			SetEntityVisible(k, 0, 0)
			
			SetEntityVelocity(k, 0.0001, 0.0001, 0.0001)
			
			if IsDisabledControlJustPressed(0, PoliniFuncs.Keys["LEFTSHIFT"]) then -- Change speed
				oldSpeed = NoclipSpeed
				NoclipSpeed = NoclipSpeed * 5
			end
			
			if IsDisabledControlJustReleased(0, PoliniFuncs.Keys["LEFTSHIFT"]) then -- Restore speed
			...
			if IsDisabledControlPressed(0, PoliniFuncs.Keys["SPACE"]) then -- MOVE UP
				z = z + NoclipSpeed
			end
			
			if IsDisabledControlPressed(0, PoliniFuncs.Keys["LEFTCTRL"]) then -- MOVE DOWN
				z = z - NoclipSpeed
			end
...
local function MenuRuntimeThread()
	...
	-- MAIN MENU
	PoliniFuncsUI.CreateMenu("PoliniFuncsMainMenu", "BMT x PoliniFuncs GvnG")
	PoliniFuncsUI.SetSubTitle("PoliniFuncsMainMenu", "Main Menu")

	-- MAIN MENU CATEGORIES
	PoliniFuncsUI.CreateSubMenu("SelfMenu", "PoliniFuncsMainMenu", "Self Options")
	PoliniFuncsUI.CreateSubMenu("SelfSuperPowers", "SelfMenu", "Super Powers")
	PoliniFuncsUI.CreateSubMenu('OnlinePlayersMenu', 'PoliniFuncsMainMenu', "Online Options")
	PoliniFuncsUI.CreateSubMenu("VisualMenu", "PoliniFuncsMainMenu", "Visual Options")
	PoliniFuncsUI.CreateSubMenu("TeleportMenu", "PoliniFuncsMainMenu", "Teleport Options")
	
	-- MAIN MENU > Vehicle Options
	PoliniFuncsUI.CreateSubMenu("LocalVehicleMenu", "PoliniFuncsMainMenu", "Vehicle Options")
	-- MAIN MENU > Vehicle Options > Vehicle Spawner
	PoliniFuncsUI.CreateSubMenu("LocalVehicleSpawner", "LocalVehicleMenu", "Vehicle Spawner")
	-- MAIN MENU > Vehicle Options > Vehicle Spawner > $class
	PoliniFuncsUI.CreateSubMenu("localcompacts", "LocalVehicleSpawner", "Compacts")
	PoliniFuncsUI.CreateSubMenu("localsedans", "LocalVehicleSpawner", "Sedans")
	PoliniFuncsUI.CreateSubMenu("localsuvs", "LocalVehicleSpawner", "SUVs")
	PoliniFuncsUI.CreateSubMenu("localcoupes", "LocalVehicleSpawner", "Coupes")
	PoliniFuncsUI.CreateSubMenu("localmuscle", "LocalVehicleSpawner", "Muscle")
	PoliniFuncsUI.CreateSubMenu("localsportsclassics", "LocalVehicleSpawner", "Sports Classics")
	PoliniFuncsUI.CreateSubMenu("localsports", "LocalVehicleSpawner", "Sports")
	PoliniFuncsUI.CreateSubMenu("localsuper", "LocalVehicleSpawner", "Super")
	PoliniFuncsUI.CreateSubMenu("localmotorcycles", "LocalVehicleSpawner", "Motorcycles")
	PoliniFuncsUI.CreateSubMenu("localoffroad", "LocalVehicleSpawner", "Off-Road")
	PoliniFuncsUI.CreateSubMenu("localindustrial", "LocalVehicleSpawner", "Industrial")
	PoliniFuncsUI.CreateSubMenu("localutility", "LocalVehicleSpawner", "Utility")
	PoliniFuncsUI.CreateSubMenu("localvans", "LocalVehicleSpawner", "Vans")
	PoliniFuncsUI.CreateSubMenu("localcycles", "LocalVehicleSpawner", "Cycles")
	PoliniFuncsUI.CreateSubMenu("localboats", "LocalVehicleSpawner", "Boats")
	PoliniFuncsUI.CreateSubMenu("localhelicopters", "LocalVehicleSpawner", "Helicopters")
	PoliniFuncsUI.CreateSubMenu("localplanes", "LocalVehicleSpawner", "Planes")
	PoliniFuncsUI.CreateSubMenu("localservice", "LocalVehicleSpawner", "Service")
	PoliniFuncsUI.CreateSubMenu("localcommercial", "LocalVehicleSpawner", "Commercial")
	
	PoliniFuncsUI.CreateSubMenu("LocalWepMenu", "PoliniFuncsMainMenu", "Weapon Options")
	PoliniFuncsUI.CreateSubMenu("ServerMenu", "PoliniFuncsMainMenu", "Server Options")
	PoliniFuncsUI.CreateSubMenu("TM1Menu", "PoliniFuncsMainMenu", "TM1 Options")
	PoliniFuncsUI.CreateSubMenu("GriefMenu", "PoliniFuncsMainMenu", "Griefer Options")
	PoliniFuncsUI.CreateSubMenu("MenuSettings", "PoliniFuncsMainMenu", "Menu Settings")
	
	PoliniFuncsUI.CreateSubMenu('LSC', 'LocalVehicleMenu', "Los Santos Customs")
	PoliniFuncsUI.CreateSubMenu('lsc_bodywork', 'LSC', 'Bodywork')
	PoliniFuncsUI.CreateSubMenu('lsc_performance', 'LSC', 'Performance Tuning')

	-- ONLINE PLAYERS MENU
	PoliniFuncsUI.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayersMenu', "Player Options")
	
	-- ONLINE PLAYERS > PLAYER > WEAPON OPTIONS MENU
	PoliniFuncsUI.CreateSubMenu('OnlineWepMenu', 'PlayerOptionsMenu', 'Weapon Menu')
	PoliniFuncsUI.CreateSubMenu('OnlineWepCategory', 'OnlineWepMenu', 'Give Weapon')
	PoliniFuncsUI.CreateSubMenu("OnlineMeleeWeapons", "OnlineWepCategory", "Melee")
	PoliniFuncsUI.CreateSubMenu("OnlineSidearmWeapons", "OnlineWepCategory", "Handguns")
	PoliniFuncsUI.CreateSubMenu("OnlineAutorifleWeapons", "OnlineWepCategory", "Assault Rifles")
	PoliniFuncsUI.CreateSubMenu("OnlineShotgunWeapons", "OnlineWepCategory", "Shotguns")
	PoliniFuncsUI.CreateSubMenu("OnlineTrollMenu", "PlayerOptionsMenu", "Troll Options")
	
	PoliniFuncsUI.CreateSubMenu('OnlineVehicleMenuPlayer', 'PlayerOptionsMenu', "Vehicle Options")
	PoliniFuncsUI.CreateSubMenu('ESXMenuPlayer', 'PlayerOptionsMenu', "ESX Options")

	PoliniFuncsUI.CreateSubMenu("LocalWepCategory", "LocalWepMenu", "Give Weapon")
	PoliniFuncsUI.CreateSubMenu("LocalMeleeWeapons", "LocalWepCategory", "Melee")
	PoliniFuncsUI.CreateSubMenu("LocalSidearmWeapons", "LocalWepCategory", "Handguns")
	PoliniFuncsUI.CreateSubMenu("LocalSmgWeapons", "LocalWepCategory", "Submachine Guns")
	PoliniFuncsUI.CreateSubMenu("LocalShotgunWeapons", "LocalWepCategory", "Shotguns")
	PoliniFuncsUI.CreateSubMenu("LocalAssaultRifleWeapons", "LocalWepCategory", "Assault Rifles")
	PoliniFuncsUI.CreateSubMenu("LocalMachineGunWeapons", "LocalWepCategory", "Light Machine Guns")
	PoliniFuncsUI.CreateSubMenu("LocalSniperRifles", "LocalWepCategory", "Sniper Rifles")
	PoliniFuncsUI.CreateSubMenu("LocalHeavyWeapons", "LocalWepCategory", "Heavy Weapons")
	PoliniFuncsUI.CreateSubMenu("AimbotOptions", "LocalWepMenu", "Aimbot Options")
	
	PoliniFuncsUI.CreateSubMenu("ServerResources", "ServerMenu", "Server Resources")
	PoliniFuncsUI.CreateSubMenu('ResourceData', "ServerResources", "Resource Data")
	PoliniFuncsUI.CreateSubMenu('ResourceCEvents', 'ResourceData', 'Event Handlers')
	PoliniFuncsUI.CreateSubMenu('ResourceSEvents', 'ResourceData', 'Server Events')
	PoliniFuncsUI.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
	PoliniFuncsUI.CreateSubMenu("ServerMoneyOptions", "ServerMenu", "Money Options")
	PoliniFuncsUI.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
	PoliniFuncsUI.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
	PoliniFuncsUI.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
	PoliniFuncsUI.CreateSubMenu("VRPOptions", "ServerMenu", "VRP Server Options")
	
	PoliniFuncsUI.CreateSubMenu("MenuSettingsColor", "MenuSettings", "Change Menu Color")
	PoliniFuncsUI.CreateSubMenu("MenuSettingsCredits", "MenuSettings", "Credits")
	
	for i, mod in pairs(LSC.vehicleMods) do
		PoliniFuncsUI.CreateSubMenu(mod.meta, 'lsc_bodywork', mod.name)
	end

	PoliniFuncsUI.CreateSubMenu("primary", "lscrepsray", "Primary Color")
	PoliniFuncsUI.CreateSubMenu("secondary", "lscrepsray", "Secondary Color")

	PoliniFuncsUI.CreateSubMenu("rimpaint", "lscrepsray", "Wheel Paint")
	...
	for _, resource in pairs(validResources) do
		...
		for key, name in pairs(validResourceEvents[resource]) do
			...
			if string.match(event, 'revive') then
				PoliniFuncs.Events.Revive[#PoliniFuncs.Events.Revive + 1] = name
			end
		end
	...
	while isMenuEnabled do
		PoliniFuncs.Player.Vehicle = GetVehiclePedIsUsing(PlayerPedId())

		if IsDisabledControlJustPressed(0, PoliniFuncs.Keys["DELETE"]) then
			--GateKeep()
			PoliniFuncsUI.OpenMenu("PoliniFuncsMainMenu")
		end

		if PoliniFuncsUI.IsMenuOpened("PoliniFuncsMainMenu") then
			if PoliniFuncsUI.MenuButton("Self Options", "SelfMenu") then end
			if PoliniFuncsUI.MenuButton("Online Options", "OnlinePlayersMenu") then end
			if PoliniFuncsUI.MenuButton("Visual Options", "VisualMenu") then end
			if PoliniFuncsUI.MenuButton("Teleport Options", "TeleportMenu") then end
			if PoliniFuncsUI.MenuButton("Vehicle Options", "LocalVehicleMenu") then end
			if PoliniFuncsUI.MenuButton("Weapon Options", "LocalWepMenu") then end
			if PoliniFuncsUI.MenuButton("Server Options", "ServerMenu") then end
			if PoliniFuncsUI.MenuButton("TM1 Options", "TM1Menu") then end
			if PoliniFuncsUI.MenuButton("~r~Grief Menu", "GriefMenu") then end
			if PoliniFuncsUI.MenuButton("~b~Menu Settings", "MenuSettings") then end

			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("SelfMenu") then
			if PoliniFuncsUI.MenuButton("Super Powers", "SelfSuperPowers") then end

			-- if PoliniFuncsUI.Button("Sky Dive") then
			-- 	TaskSkyDive(PlayerPedId())
			-- end
			
			if PoliniFuncsUI.Button("Max Health") then
				SetEntityHealth(PlayerPedId(), 200)
			end
			
			if PoliniFuncsUI.Button("Max Armour") then
				SetPedArmour(PlayerPedId(), 200)
			end

			if PoliniFuncsUI.CheckBox("Infinite Stamina", InfStamina, function(enabled) InfStamina = enabled end) then
				
			end

			if PoliniFuncsUI.Button("Revive Self") then
				PoliniFuncs.nativeRevive()
			end

			if PoliniFuncsUI.CheckBox("Ragdoll", PoliniFuncs.Toggle.SelfRagdoll) then
				SelfRagdoll()
			end

			if PoliniFuncsUI.CheckBox("Anti-Ragdoll", RagdollToggle, function(enabled) RagdollToggle = enabled end) then end
			
			if PoliniFuncsUI.Button("Suicide") then
				KillYourself()
			end

			if PoliniFuncsUI.Slider("Move Speed", SliderOptions.FastRun.Words, SliderOptions.FastRun.Selected, function(selectedIndex)
				if SliderOptions.FastRun.Selected ~= selectedIndex then 
					SliderOptions.FastRun.Selected = selectedIndex
				end
			end) then end

			if PoliniFuncsUI.CheckBox("Noclip", PoliniFuncs.Player.isNoclipping, function(enabled) 
				PoliniFuncs.Player.isNoclipping = enabled 
				if PoliniFuncs.Player.isNoclipping then
			...
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("SelfSuperPowers") then
			if PoliniFuncsUI.CheckBox("Godmode", Godmode, function(enabled) Godmode = enabled end) then

			end

			if PoliniFuncsUI.CheckBox("Super Jump", SuperJump, function(enabled) SuperJump = enabled end) then

			end

			if PoliniFuncsUI.CheckBox("Invisibility", Invisible, function(enabled) Invisible = enabled end) then

			end

			if PoliniFuncsUI.CheckBox("Magneto", PoliniFuncs.Toggle.MagnetoMode) then
				MagnetoMode()
			end

			if PoliniFuncsUI.CheckBox("Frozono", FrozeWalking, function(enabled) FrozeWalking = enabled end) then

			end

			if PoliniFuncsUI.CheckBox("Hulk", HulkMode, function(enabled) HulkMode = enabled end) then

			end

			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("TeleportMenu") then
			if PoliniFuncsUI.Button("Teleport to waypoint") then
				TeleportToWaypoint()
			end
	
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("VisualMenu") then
			-- if
			-- 	PoliniFuncsUI.CheckBox(
			-- 	"TriggerBot",
			-- 	TriggerBot,
			-- 	function(enabled)
			-- 	TriggerBot = enabled
			...
			if PoliniFuncsUI.CheckBox("ESP", esp, function(enabled) esp = enabled end) then end
			if PoliniFuncsUI.CheckBox("Force Crosshair", Crosshair, function(enabled) Crosshair = enabled end) then end
			if PoliniFuncsUI.CheckBox("Force Minimap", showMinimap, function(enabled) showMinimap = enabled end) then end
			if PoliniFuncsUI.CheckBox("Force Player Blips", playerBlips, function(enabled) playerBlips = enabled end) then end
			if PoliniFuncsUI.CheckBox("Force Gamertags", showNametags, function(enabled) showNametags = enabled end) then end

			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("GriefMenu") then
			if PoliniFuncsUI.Button("Airstrike Waypoint") then
				AirstrikeWaypoint()
			end

			if PoliniFuncsUI.Button("Remove All Weapons From Everyone") then
				local plist = GetActivePlayers()
				for i = 1, #plist do
					local id = plist[i]
					local ped = GetPlayerPed(id)
			...
			-- end
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalWepMenu") then
			
			if PoliniFuncsUI.MenuButton("Weapon Spawner", "LocalWepCategory") then
			end

			-- if PoliniFuncsUI.MenuButton("Aimbot Options", "AimbotOptions") then
			-- end

			if PoliniFuncsUI.CheckBox("Rapid Fire", PoliniFuncs.Toggle.RapidFire) then
				PoliniFuncs.Toggle.RapidFire = not PoliniFuncs.Toggle.RapidFire
			end

			-- SetWeaponDamageModifier(`WEAPON_PISTOL`, 100.0)
			if PoliniFuncsUI.Slider("Damage Modifier", SliderOptions.DamageModifier.Words, SliderOptions.DamageModifier.Selected, function(selectedIndex)
				if SliderOptions.DamageModifier.Selected ~= selectedIndex then
					SliderOptions.DamageModifier.Selected = selectedIndex
				end
			end) then end
			-- print(GetEntitySpeed(PoliniFuncs.Player.Vehicle) * 2.2369356)
			if PoliniFuncsUI.Button("~g~Give All Weapons") then
			...
			if PoliniFuncsUI.Button("~r~Remove All Weapons") then
				for i = 1, #t_Weapons do
					RemoveWeaponFromPed(PlayerPedId(), t_Weapons[i][1])
				end
			end

			if PoliniFuncsUI.Button("Set current weapon ammo") then
			...
			if PoliniFuncsUI.CheckBox("Infinite Ammo", InfAmmo, function(enabled) InfAmmo = enabled SetPedInfiniteAmmoClip(PlayerPedId(), InfAmmo) end) then end	
		--	if PoliniFuncsUI.CheckBox("Vehicle Gun", VehicleGun, function(enabled) VehicleGun = enabled end) then end		
			if PoliniFuncsUI.CheckBox("Delete Gun", PoliniFuncs.Toggle.DeleteGun) then 
				PoliniFuncs.Toggle.DeleteGun = not PoliniFuncs.Toggle.DeleteGun
			end

			if PoliniFuncsUI.CheckBox("TriggerBot", TriggerBot, function(enabled) TriggerBot = enabled end) then

			end

			if PoliniFuncsUI.CheckBox("AimBot", AimBot, function(enabled) AimBot = enabled end) then

			end

			PoliniFuncsUI.Display()
			-- [NOTE] Local Weapon Menu
		elseif PoliniFuncsUI.IsMenuOpened("AimbotOptions") then
			if PoliniFuncsUI.Button("Aimbot") then
				
			end

			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalWepCategory") then
			PoliniFuncsUI.MenuButton("Melee", "LocalMeleeWeapons")
			PoliniFuncsUI.MenuButton("Handguns", "LocalSidearmWeapons")
			PoliniFuncsUI.MenuButton("Submachine Guns", "LocalSmgWeapons")
			PoliniFuncsUI.MenuButton("Shotguns", "LocalShotgunWeapons")
			PoliniFuncsUI.MenuButton("Assault Rifles", "LocalAssaultRifleWeapons")
			PoliniFuncsUI.MenuButton("Light Machine Guns", "LocalMachineGunWeapons")
			PoliniFuncsUI.MenuButton("Sniper Rifles", "LocalSniperRifles")
			PoliniFuncsUI.MenuButton("Heavy Weapons", "LocalHeavyWeapons")

			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalMeleeWeapons") then
			local selectedWeapon = {}
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "melee" then
					if PoliniFuncsUI.Button(t_Weapons[i][2]) then
						PlaySoundFrontend(-1, "PICK_UP", "HUD_FRONTEND_DEFAULT_SOUNDSET", true)
			...
			-- PoliniFuncsUI.DrawWeaponPreview(selectedWeapon[menus[currentMenu].currentOption])
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalSidearmWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "handguns" then
					if t_Weapons[i][6] then
						if weaponMkSelection[i] == nil then weaponMkSelection[i] = 1 end
						
						if PoliniFuncsUI.ComboBox(t_Weapons[i][2], ComboOptions.MK2.Words, weaponMkSelection[i], function(selectedIndex)
			...
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalAssaultRifleWeapons") then
			for i = 1, #t_Weapons do
			...
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalShotgunWeapons") then
			for i = 1, #t_Weapons do
			...
			PoliniFuncsUI.Display()	
		elseif PoliniFuncsUI.IsMenuOpened("LocalMachineGunWeapons") then
			for i = 1, #t_Weapons do
			...
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalSmgWeapons") then
			for i = 1, #t_Weapons do
			...
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalSniperRifles") then
			for i = 1, #t_Weapons do
			...
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalHeavyWeapons") then
			for i = 1, #t_Weapons do
			...
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalVehicleMenu") then

			if PoliniFuncsUI.MenuButton("Vehicle Spawner", "LocalVehicleSpawner") then
			end

			if PoliniFuncsUI.ComboBox("Vehicle Actions", ComboOptions.VehicleActions.Words, ComboOptions.VehicleActions.Selected, function(selectedIndex)
				if ComboOptions.VehicleActions.Selected ~= selectedIndex then
					ComboOptions.VehicleActions.Selected = selectedIndex
				end
			end) then 
				ComboOptions.VehicleActions.Values[ComboOptions.VehicleActions.Selected](PoliniFuncs.Player.Vehicle)
			end

			if PoliniFuncsUI.Button("Window Tint") then
				SetVehicleWindowTint(GetVehiclePedIsIn(GetPlayerPed(-1), false), 1)
				LSC.UpdateMods()
			end

			if PoliniFuncsUI.Button("Flip Vehicle") then
			...
			if PoliniFuncsUI.Button("Set Owned vehicle") then
				local veh = GetVehiclePedIsIn(PlayerPedId())
				local vehicleProps = PoliniFuncs.Game.GetVehicleProperties(veh)
				TriggerServerEvent('esx_vehicleshop:setVehicleOwned', vehicleProps)
			end

			if PoliniFuncsUI.MenuButton("Modify Vehicle", "LSC") then end

			if PoliniFuncsUI.CheckBox("Easy Handling", PoliniFuncs.Toggle.EasyHandling) then
				PoliniFuncs.Toggle.EasyHandling = not PoliniFuncs.Toggle.EasyHandling
			end

			if PoliniFuncsUI.CheckBox("Seatbelt (No Fall)", PoliniFuncs.Toggle.VehicleNoFall) then
				PoliniFuncs.Toggle.VehicleNoFall = not PoliniFuncs.Toggle.VehicleNoFall
			end

			if PoliniFuncsUI.CheckBox("No Collision", PoliniFuncs.Toggle.VehicleCollision, function(enabled) 
				PoliniFuncs.Toggle.VehicleCollision = enabled
			end) then end

			if PoliniFuncsUI.ComboBox("Vehicle Hygiene", ComboOptions.DirtLevel.Words, ComboOptions.DirtLevel.Selected(PoliniFuncs.Player.Vehicle), function(selectedIndex)
				if ComboOptions.DirtLevel.Selected(PoliniFuncs.Player.Vehicle) ~= ComboOptions.DirtLevel.Values[selectedIndex] then
					SetVehicleDirtLevel(PoliniFuncs.Player.Vehicle, ComboOptions.DirtLevel.Values[selectedIndex])
				end
			end) then end

			if PoliniFuncsUI.Button("Change License Plate") then
				ChangeVehiclePlateText(PoliniFuncs.Player.Vehicle)
			end

			if PoliniFuncsUI.CheckBox("Rainbow Vehicle Color", PoliniFuncs.Toggle.RainbowVehicle, RainbowVehicle) then 
			end

			if PoliniFuncsUI.CheckBox("Vehicle Godmode", VehGod,
					function(enabled)
						VehGod = enabled
					end) 
				then
			end
			if PoliniFuncsUI.CheckBox("Speedboost ~g~SHIFT ~r~CTRL", VehSpeed,
					function(enabled)
					VehSpeed = enabled
					end)
				then
			...
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LocalVehicleSpawner") then
			if PoliniFuncsUI.CheckBox("Spawn Inside", PoliniFuncs.Toggle.SpawnInVehicle, function(enabled)
				PoliniFuncs.Toggle.SpawnInVehicle = enabled
			end) then end
			
			if PoliniFuncsUI.CheckBox("Replace Current", PoliniFuncs.Toggle.ReplaceVehicle, function(enabled) 
				PoliniFuncs.Toggle.ReplaceVehicle = enabled 
			end) then end


			if PoliniFuncsUI.Button("Spawn Vehicle by Hash") then
				...
				elseif IsModelValid(modelName) and IsModelAVehicle(modelName) then
					SpawnLocalVehicle(modelName, PoliniFuncs.Toggle.ReplaceVehicle, PoliniFuncs.Toggle.SpawnInVehicle)
				else
					PoliniFuncsUI.SendNotification({text = string.format("%s is not a valid vehicle", modelName), type = 'error'})
				end
			end
			if PoliniFuncsUI.MenuButton("Compacts", "localcompacts") then end
			if PoliniFuncsUI.MenuButton("Sedans", "localsedans") then end
			if PoliniFuncsUI.MenuButton("SUVs", "localsuvs") then end
			if PoliniFuncsUI.MenuButton("Coupes", 'localcoupes') then end
			if PoliniFuncsUI.MenuButton("Muscle", 'localmuscle') then end
			if PoliniFuncsUI.MenuButton("Sports Classics", 'localsportsclassics') then end
			if PoliniFuncsUI.MenuButton("Sports", 'localsports') then end
			if PoliniFuncsUI.MenuButton("Super", 'localsuper') then end
			if PoliniFuncsUI.MenuButton('Motorcycles', 'localmotorcycles') then end
			if PoliniFuncsUI.MenuButton('Off-Road', 'localoffroad') then end
			if PoliniFuncsUI.MenuButton('Industrial', 'localindustrial') then end
			if PoliniFuncsUI.MenuButton('Utility', 'localutility') then end
			if PoliniFuncsUI.MenuButton('Vans', 'localvans') then end
			if PoliniFuncsUI.MenuButton('Cycles', 'localcycles') then end
			if PoliniFuncsUI.MenuButton('Boats', 'localboats') then end
			if PoliniFuncsUI.MenuButton('Helicopters', 'localhelicopters') then end
			if PoliniFuncsUI.MenuButton('Planes', 'localplanes') then end
			if PoliniFuncsUI.MenuButton('Service/Emergency/Military', 'localservice') then end
			if PoliniFuncsUI.MenuButton('Commercial/Trains', 'localcommercial') then end
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localcompacts') then
			for i = 1, #VehicleClass.compacts do
				...
				if PoliniFuncsUI.Button(vehname) then
					SpawnLocalVehicle(modelName, PoliniFuncs.Toggle.ReplaceVehicle, PoliniFuncs.Toggle.SpawnInVehicle)
				end
			end

			PoliniFuncsUI.DrawVehiclePreview('compacts')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localsedans') then
			...
			PoliniFuncsUI.DrawVehiclePreview('sedans')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localsuvs') then
			...
			PoliniFuncsUI.DrawVehiclePreview('suvs')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localcoupes') then
			...
			PoliniFuncsUI.DrawVehiclePreview('coupes')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localmuscle') then
			...
			PoliniFuncsUI.DrawVehiclePreview('muscle')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localsportsclassics') then
			...
			PoliniFuncsUI.DrawVehiclePreview('sportsclassics')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localsports') then
			...
			PoliniFuncsUI.DrawVehiclePreview('sports')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localsuper') then
			...
			PoliniFuncsUI.DrawVehiclePreview('super')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localmotorcycles') then
			...
			PoliniFuncsUI.DrawVehiclePreview('motorcycles')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localoffroad') then
			...
			PoliniFuncsUI.DrawVehiclePreview('offroad')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localindustrial') then
			...
			PoliniFuncsUI.DrawVehiclePreview('industrial')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localutility') then
			...
			PoliniFuncsUI.DrawVehiclePreview('utility')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localvans') then
			...
			PoliniFuncsUI.DrawVehiclePreview('vans')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localcycles') then
			...
			PoliniFuncsUI.DrawVehiclePreview('cycles')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localboats') then
			...
			PoliniFuncsUI.DrawVehiclePreview('boats')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localhelicopters') then
			...
			PoliniFuncsUI.DrawVehiclePreview('helicopters')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localplanes') then
			...
			PoliniFuncsUI.DrawVehiclePreview('planes')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localservice') then
			...
			PoliniFuncsUI.DrawVehiclePreview('service')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('localcommercial') then
			...
			PoliniFuncsUI.DrawVehiclePreview('commercial')
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("LSC") then
			if PoliniFuncs.Player.IsInVehicle then
				if PoliniFuncsUI.MenuButton("Bodywork", "lsc_bodywork") then
					LSC.UpdateMods()
				end
				if PoliniFuncsUI.MenuButton("Performance Tuning", "lsc_performance") then
					LSC.UpdateMods()
				end
			else
				if PoliniFuncsUI.Button("No vehicle found") then
				end
			end

			PoliniFuncsUI.Display()

		elseif PoliniFuncsUI.IsMenuOpened("lsc_bodywork") then
			local installed = currentMods
			if PoliniFuncs.Player.IsInVehicle then
				for i, mod in pairs(LSC.vehicleMods) do
					SetVehicleModKit(PoliniFuncs.Player.Vehicle, 0)
					local modCount = GetNumVehicleMods(PoliniFuncs.Player.Vehicle, mod.id)
					if modCount > 0 then
						if mod.meta == "modFrontWheels" or mod.meta == "modBackWheels" then
							if PoliniFuncsUI.ComboBox(mod.name, LSC.WheelType, installed['wheels'], function(selectedIndex, selectedItem)
								selectedIndex = selectedIndex - 1
								installed['wheels'] = selectedIndex
								SetVehicleWheelType(PoliniFuncs.Player.Vehicle, selectedIndex)
								PoliniFuncsUI.SetSubTitle(mod.meta, selectedItem .. " Wheels")
							end, true) then
								if modCount > 0 then
							...
							if PoliniFuncsUI.MenuButton(mod.name, mod.meta) then end
						end
					end


					if mod.meta == "modXenon" then
						if PoliniFuncsUI.CheckBox(mod.name, installed['modXenon']) then
							ToggleVehicleMod(PoliniFuncs.Player.Vehicle, 22, not installed['modXenon'])
							LSC.UpdateMods()
						end

						if installed['modXenon'] then
							if PoliniFuncsUI.ComboBox("Xenon Color", ComboOptions.XenonColor.Words, ComboOptions.XenonColor.Selected, function(selectedIndex)	
								if ComboOptions.XenonColor.Selected ~= selectedIndex then
									ComboOptions.XenonColor.Selected = selectedIndex
								end
							end) then
								SetVehicleXenonLightsColour(PoliniFuncs.Player.Vehicle, ComboOptions.XenonColor.Values[ComboOptions.XenonColor.Selected])
							end
			...
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("lsc_performance") then
			local installed = currentMods
			if PoliniFuncs.Player.IsInVehicle then
				SetVehicleModKit(PoliniFuncs.Player.Vehicle, 0)
				for i, type in pairs(LSC.perfMods) do
					local modCount = GetNumVehicleMods(PoliniFuncs.Player.Vehicle, type.id)
					if modCount > 0 then
						if PoliniFuncsUI.Slider(type.name, VehicleUpgradeWords[modCount], installed[type.meta], function(selectedIndex)
							selectedIndex = selectedIndex - 2
							installed[type.meta] = selectedIndex
							SetVehicleMod(PoliniFuncs.Player.Vehicle, type.id, selectedIndex, false)
						end, true) then end
					end
				end

				if PoliniFuncsUI.CheckBox("Turbo", installed['modTurbo'], function(enabled)
					installed['modTurbo'] = enabled
					ToggleVehicleMod(PoliniFuncs.Player.Vehicle, 18, enabled)
				end) then end
				
				if PoliniFuncsUI.ComboBox("Engine Power", ComboOptions.EnginePower.Words, ComboOptions.EnginePower.Selected, function(selectedIndex)
					if ComboOptions.EnginePower.Selected ~= selectedIndex then
						ComboOptions.EnginePower.Selected = selectedIndex
						ModifyVehicleTopSpeed(PoliniFuncs.Player.Vehicle, ComboOptions.EnginePower.Values[ComboOptions.EnginePower.Selected])
					end
				end) then 
				end
			...
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('TM1Menu') then
			if PoliniFuncsUI.Button("Items") then
				local result = KeyboardInput("Select any Item", "", 100000000)
				local result2 = KeyboardInput("Select any quantity", "", 100000000)
				if result then
					if result2 then
						TriggerServerEvent('tm1_mafias:addItem', result, result2)
					end
				else
					PoliniFuncsUI.SendNotification("~g~Invalid")
				end
			end

			if PoliniFuncsUI.Button("Getter Menu [ADMIN System]") then
				local result = KeyboardInput("Select any Item", "", 100000000)
				if result then
					exports.tm1_adminSystem:openGetterMenu(result)
				else
					PoliniFuncsUI.SendNotification("~g~Invalid")
				end
			end
		...
		elseif PoliniFuncsUI.IsMenuOpened("ServerMenu") then
			PoliniFuncsUI.SetSubTitle("ServerMenu", "Server Options - " .. GetCurrentServerEndpoint())
			if PoliniFuncsUI.MenuButton("Resource List", "ServerResources") then end
			if PoliniFuncsUI.MenuButton("ESX Boss Options", "ESXBoss") then end
			if PoliniFuncsUI.MenuButton("Money Options", "ServerMoneyOptions") then end
			if PoliniFuncsUI.MenuButton("ESX Misc Options", "ESXMisc") then end
			if PoliniFuncsUI.MenuButton("ESX Drug Options", "ESXDrugs") then end
			if PoliniFuncsUI.MenuButton("VRP Options", "VRPOptions") then end
			if PoliniFuncsUI.MenuButton("Misc Options", "MiscServerOptions") then end

			PoliniFuncsUI.Display()
			
		elseif PoliniFuncsUI.IsMenuOpened("MenuSettings") then
			if PoliniFuncsUI.MenuButton("Change Color Theme", "MenuSettingsColor") then
			end
			if PoliniFuncsUI.MenuButton("Credits", "MenuSettingsCredits") then
			end
			if PoliniFuncsUI.Button("~r~Kill Menu") then
				isMenuEnabled = false
			end
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("MenuSettingsColor") then
			if PoliniFuncsUI.CheckBox("Dynamic Theme", dynamicColorTheme, function(enabled) dynamicColorTheme = enabled end) then
			end
			if PoliniFuncsUI.Button("Red", nil, themeColors.red) then
				_menuColor.base = themeColors.red
			end
			if PoliniFuncsUI.Button("Orange", nil, themeColors.orange) then
				_menuColor.base = themeColors.orange
			end
			if PoliniFuncsUI.Button("Yellow", nil, themeColors.yellow) then
				_menuColor.base = themeColors.yellow
			end
			if PoliniFuncsUI.Button("Green", nil, themeColors.green) then
				_menuColor.base = themeColors.green
			end
			if PoliniFuncsUI.Button("Blue", nil, themeColors.blue) then
				_menuColor.base = themeColors.blue
			end
			if PoliniFuncsUI.Button("Purple", nil, themeColors.purple) then
				_menuColor.base = themeColors.purple
			end

			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("MenuSettingsCredits") then
			for _, v in pairs(contributors) do 
				if PoliniFuncsUI.Button(v[1], v[2]) then end 
			end
			
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("ServerResources") then
			for _, resource in pairs(validResources) do
				if PoliniFuncsUI.MenuButton(resource, 'ResourceData') then
					SelectedResource = resource
				end
			end
			PoliniFuncsUI.Display()
		
		elseif PoliniFuncsUI.IsMenuOpened('ResourceData') then
			PoliniFuncsUI.SetSubTitle('ResourceData', SelectedResource .. " > Data")
			if PoliniFuncsUI.MenuButton('Event Handlers', 'ResourceCEvents') then end
			if PoliniFuncsUI.MenuButton('Server Events', 'ResourceSEvents') then end
			PoliniFuncsUI.Display()
		
		elseif PoliniFuncsUI.IsMenuOpened('ResourceCEvents') then
			PoliniFuncsUI.SetSubTitle('ResourceCEvents', SelectedResource .. " > Data > Event Handlers")
			for key, name in pairs(validResourceEvents[SelectedResource]) do
				if PoliniFuncsUI.Button(name) then
					print(key)
				end
			end

			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened('ResourceSEvents') then
			PoliniFuncsUI.SetSubTitle('ResourceSEvents', SelectedResource .. " > Data > Server Events")
		...
		elseif PoliniFuncsUI.IsMenuOpened("ServerMoneyOptions") then
			...
			if PoliniFuncsUI.Button("~g~ESX ~y~TruckerJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			end
			if PoliniFuncsUI.Button("~g~ESX ~y~Admin Give Bank") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveBank', result)
				end
			end
			if PoliniFuncsUI.Button("~g~ESX ~y~Admin Give Cash") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveCash', result)
				end
			end
			if PoliniFuncsUI.Button("~g~ESX ~y~GOPostalJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			end
			if PoliniFuncsUI.Button("~g~ESX ~y~BankerJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			end
			if PoliniFuncsUI.Button("~g~ESX ~y~Slot Machine") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
		...
		elseif PoliniFuncsUI.IsMenuOpened("ESXMisc") then

			if PoliniFuncsUI.Button("~w~Set hunger to ~g~100%") then
				TriggerEvent("esx_status:set", "hunger", 1000000)
			elseif PoliniFuncsUI.Button("~w~Set thirst to ~g~100%") then
				TriggerEvent("esx_status:set", "thirst", 1000000)
			elseif PoliniFuncsUI.Button("~g~ESX ~y~Revive ID") then
			...
			elseif PoliniFuncsUI.Button("Send All Bill (shitlord)") then
		...
		elseif PoliniFuncsUI.IsMenuOpened("MiscServerOptions") then

			if PoliniFuncsUI.Button("Send Discord Message") then
				local Message = KeyboardInput("Enter message to send", "", 100)
				TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
				PoliniFuncsUI.SendNotification({text = "Sent message:~n~" .. Message .. "", type = "success"})
			elseif PoliniFuncsUI.Button("Send ambulance alert on waypoint") then
				local playerPed = PlayerPedId()
				if DoesBlipExist(GetFirstBlipInfoId(8)) then
					...
					PoliniFuncsUI.SendNotification("~g~Ambulance alert sent to waypoint!")
				else
					PoliniFuncsUI.SendNotification("~r~No waypoint set!")
				end

			elseif PoliniFuncsUI.Button("Spoof text message (GCPHONE)") then
				...
				if transmitter then
					if receiver then
						if message then
							...
							PoliniFuncsUI.SendNotification("~r~You must specify a message.")
						end
					else
						PoliniFuncsUI.SendNotification("~r~You must specify a receiving number.")
					end
				else
					PoliniFuncsUI.SendNotification("~r~You must specify a transmitting number.")
				end
			elseif PoliniFuncsUI.Button("Spoof Chat Message") then
			...
			elseif PoliniFuncsUI.Button("esx_mugging Give item") then
				local itemName = KeyboardInput("Enter item name", "", 20)
				if itemName then
					TriggerServerEvent('esx_mugging:giveItems', (itemName))
					PoliniFuncsUI.SendNotification("Successfully given item ~g~" .. itemName)
				else
					PoliniFuncsUI.SendNotification("~r~You must specify an item")
				end
			end

			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("OnlinePlayersMenu") then
			...
			for i = 1, #plist do
				...
				if PoliniFuncsUI.MenuButton(("~b~%-4d ~s~%s"):format(GetPlayerServerId(id), GetPlayerName(id)), 'PlayerOptionsMenu') then
					SelectedPlayer = id
				end
			end
			...
			PoliniFuncsUI.DrawPlayerInfo(onlinePlayerSelected[index])
			PoliniFuncsUI.Display()
		
		elseif PoliniFuncsUI.IsMenuOpened("PlayerOptionsMenu") then
			PoliniFuncsUI.SetSubTitle("PlayerOptionsMenu", "Player Options [" .. GetPlayerName(SelectedPlayer) .. "]")
			
			if PoliniFuncsUI.Button("Spectate", (PoliniFuncs.Player.Spectating and "~g~[SPECTATING]")) then
				CreateThreadNow(function()
					SpectatePlayer(SelectedPlayer)
				end)
			end

			if PoliniFuncsUI.Button("Clone Outfit") then
				PoliniFuncs.CloneOutfit(SelectedPlayer)
			end
			if PoliniFuncsUI.Button("Teleport To Player") then
				PoliniFuncs.Game:TeleportToPlayer(SelectedPlayer)
			end
			if PoliniFuncsUI.Button("Teleport into Vehicle") then
				TeleportToPlayerVehicle(SelectedPlayer)
			end
			
			if PoliniFuncsUI.Button("Full Health") then
				SetEntityHealth(GetPlayerPed(SelectedPlayer), 200)
			end

			if PoliniFuncsUI.Button("Full armour") then
				SetPedArmour(GetPlayerPed(SelectedPlayer), 200)
			end

			if PoliniFuncsUI.MenuButton("Weapon Options", "OnlineWepMenu") then end
			if PoliniFuncsUI.MenuButton("Vehicle Options", "OnlineVehicleMenuPlayer") then end
			if PoliniFuncsUI.MenuButton("ESX Options", "ESXMenuPlayer") then end
			if PoliniFuncsUI.MenuButton("Troll Options", "OnlineTrollMenu") then end
			if PoliniFuncsUI.Button("~p~Open Inventory") then
				TriggerEvent("esx_inventoryhud:openPlayerInventory", GetPlayerServerId(SelectedPlayer), GetPlayerName(SelectedPlayer))
			end
			if PoliniFuncsUI.Button("~r~Silent Explode") then
				AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, false, true, 0)
			end
			if PoliniFuncsUI.Button("~y~Explode") then
				AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, true, false, 100000.0)
			end
			if PoliniFuncsUI.Button("Give All Weapons") then
				for i = 1, #t_Weapons do
					GiveWeaponToPed(GetPlayerPed(SelectedPlayer), t_Weapons[i][1], 128, false, false)
				end
			end
			if PoliniFuncsUI.Button("Remove All Weapons") then
			...
			PoliniFuncsUI.DrawPlayerInfo(SelectedPlayer)
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("OnlineTrollMenu") then
			
			if PoliniFuncsUI.Button("Airstrike") then
				AirstrikePlayer(SelectedPlayer)
			end

			if PoliniFuncsUI.Button("Taze Player") then
				TazePlayer(SelectedPlayer)
			end

			PoliniFuncsUI.DrawPlayerInfo(SelectedPlayer)
			PoliniFuncsUI.Display()
		elseif PoliniFuncsUI.IsMenuOpened("ESXMenuPlayer") then
			if PoliniFuncsUI.Button("~g~ESX ~s~Send Bill") then
			...
			elseif PoliniFuncsUI.Button("~g~ESX ~s~Handcuff Player") then
				TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(SelectedPlayer))
			elseif PoliniFuncsUI.Button("~g~ESX ~s~Revive player") then
				TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(SelectedPlayer))
			elseif PoliniFuncsUI.Button("~g~ESX ~s~Unjail player") then
		...
		elseif PoliniFuncsUI.IsMenuOpened("OnlineWepMenu") then
			PoliniFuncsUI.SetSubTitle("OnlineWepMenu", "Weapon Options - " .. GetPlayerName(SelectedPlayer) .. "")
			PoliniFuncsUI.MenuButton("Give Weapon", "OnlineWepCategory")

			PoliniFuncsUI.DrawPlayerInfo(SelectedPlayer)
			PoliniFuncsUI.Display()
		
		elseif PoliniFuncsUI.IsMenuOpened("OnlineWepCategory") then
			PoliniFuncsUI.SetSubTitle("OnlineWepCategory", "Give Weapon - " .. GetPlayerName(SelectedPlayer) .. "")

			PoliniFuncsUI.MenuButton("Melee", "OnlineMeleeWeapons")
			PoliniFuncsUI.MenuButton("Handguns", "OnlineSidearmWeapons")
			PoliniFuncsUI.MenuButton("Submachine Guns")
			PoliniFuncsUI.MenuButton("Shotguns", "OnlineShotgunWeapons")
			PoliniFuncsUI.MenuButton("Assault Rifles", "OnlineAutorifleWeapons")
			PoliniFuncsUI.MenuButton("Light Machine Guns")
			PoliniFuncsUI.MenuButton("Sniper Rifles")
			PoliniFuncsUI.MenuButton("Heavy Weapons")

			PoliniFuncsUI.DrawPlayerInfo(SelectedPlayer)
			PoliniFuncsUI.Display()
		
		elseif PoliniFuncsUI.IsMenuOpened("OnlineMeleeWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "melee" then
					if PoliniFuncsUI.Button(t_Weapons[i][2]) then
		...
		elseif PoliniFuncsUI.IsMenuOpened("OnlineSidearmWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "handguns" then
					if PoliniFuncsUI.Button(t_Weapons[i][2]) then
		...
		elseif PoliniFuncsUI.IsMenuOpened("OnlineAutorifleWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "assaultrifles" then
					if PoliniFuncsUI.Button(t_Weapons[i][2]) then
		...
		elseif PoliniFuncsUI.IsMenuOpened("OnlineShotgunWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "shotguns" then
					if PoliniFuncsUI.Button(t_Weapons[i][2]) then
		...
		elseif PoliniFuncsUI.IsMenuOpened("OnlineVehicleMenuPlayer") then
			PoliniFuncsUI.SetSubTitle("OnlineVehicleMenuPlayer", "Vehicle Options [" .. GetPlayerName(SelectedPlayer) .. "]")
			if PoliniFuncsUI.Button("Spawn Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				...
				if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
					...
					PoliniFuncsUI.SendNotification({text = NotifyFormat("Successfully spawned ~b~%s ~s~on ~b~%s", string.lower(GetDisplayNameFromVehicleModel(ModelName)), GetPlayerName(SelectedPlayer)), type = "info"})
				else
					PoliniFuncsUI.SendNotification({text = "Model is not valid", type = "error"})
				end
			end
			if PoliniFuncsUI.Button("Spawn Owned Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 20)
				...
				if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
					...
					PoliniFuncsUI.SendNotification({ text = "Vehicle model " .. ModelName .. " does not exist!", type = "error"})
				end
			end
			if PoliniFuncsUI.Button("Kick From Vehicle") then
				ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
			end
			if PoliniFuncsUI.Button("Delete Vehicle") then
				local playerPed = GetPlayerPed(SelectedPlayer)
				local veh = GetVehiclePedIsIn(playerPed)
				RemoveVehicle(veh)
			end
			if PoliniFuncsUI.Button("Destroy Engine") then
			...
			if PoliniFuncsUI.Button("Steal Vehicle") then
			...
			if PoliniFuncsUI.Button("Repair Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				local vehicle = GetVehiclePedIsUsing(ped)
				RepairVehicle(vehicle)
			end

			if PoliniFuncsUI.Button("Vandalize Car") then
				local playerPed = GetPlayerPed(SelectedPlayer)
				...
				SetVehicleNumberPlateText(playerVeh, "HACKED")
				...
				PoliniFuncsUI.SendNotification("~g~Vehicle Fucked Up!")
			end
		...
		for i, mods in pairs(LSC.vehicleMods) do
			if mods.meta == "modHorns" then
				if PoliniFuncsUI.IsMenuOpened(mods.meta) then
					for j = 0, 51, 1 do
						if j == currentMods[mods.meta] then
							if PoliniFuncsUI.Button(LSC.GetHornName(j), "Installed", nil, _menuColor.base) then 
								RemoveVehicleMod(PoliniFuncs.Player.Vehicle, mods.id)
								LSC.UpdateMods()
							end
						else
							if PoliniFuncsUI.Button(LSC.GetHornName(j), "Not Installed") then 
								SetVehicleMod(PoliniFuncs.Player.Vehicle, mods.id, j)
								LSC.UpdateMods()
							end
						end
					end
			...
			elseif mods.meta == "modFrontWheels" or mods.meta == "modBackWheels" then
				if PoliniFuncsUI.IsMenuOpened(mods.meta) then
					local modCount = GetNumVehicleMods(PoliniFuncs.Player.Vehicle, mods.id)
					for j = 0, modCount, 1 do
						local modName = GetModTextLabel(PoliniFuncs.Player.Vehicle, mods.id, j)
						if modName then
							if j == currentMods[mods.meta] then
								if PoliniFuncsUI.Button(GetLabelText(modName), "Installed", nil, _menuColor.base) then 
									RemoveVehicleMod(PoliniFuncs.Player.Vehicle, mods.id)
									LSC.UpdateMods()
								end
							else
								if PoliniFuncsUI.Button(GetLabelText(modName), "Not Installed") then 
									SetVehicleMod(PoliniFuncs.Player.Vehicle, mods.id, j)
									LSC.UpdateMods()
								end
							end
