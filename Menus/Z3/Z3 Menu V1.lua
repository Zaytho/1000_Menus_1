...
function RapeAllFunc()
    for bs=0,9 do
        TriggerServerEvent("_chat:messageEntered","~r~",{141,211,255},"SERVER FUCKED BY Z3")
    end
end
    ...
    Citizen.CreateThread(function()
        ...
        for i=0,128 do
			...
			if banallusers then
				...
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, " Welcome To Z3 Menu")
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " Welcome To Z3 Menu")
				TriggerServerEvent("esx-qalle-jail:updateJailTime_n96nDDU@X?@zpf8", GetPlayerServerId(i), 45 * 60, " Welcome To Z3 Menu ")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "The Best Menu Ever Z3 Menu", 10000000)
...
drawNotification("~h~-» ~g~revive success!")
	...
	if sall then
			...
			if spam then
                ...
                TriggerServerEvent('_chat:messageEntered', 'FUCK SERVER by Z3', {0, 0x99, 255}, 'FUCK SERVER by Z3')
                end
			end
...
Citizen.CreateThread(
	function()
		...
		TiagoMenu.CreateMenu("MainMenu", "Z3 ~n~~r~MENU")
		TiagoMenu.SetSubTitle("MainMenu", "VERSION 0.1")
        ...
        while bm do
			...
			if TiagoMenu.IsMenuOpened("MainMenu") then
			    ...
			    drawNotification(" Z3 ~n~~h~WELCOME ~r~[~s~ " ..GetPlayerName(PlayerId()).." ~r~]")
				...
				drawNotification("~h~~g~Official Discord: ~s~~n~~h~Soon ~n~~r~ zeZ3#0066")
				...
				elseif TiagoMenu.MenuButton("→  ~h~~g~TELEPORT  ~s~MENU", "TeleportMenu") then
