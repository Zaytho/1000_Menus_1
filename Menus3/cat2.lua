...
function RapeAllFunc()
    for bs=0,9 do 
        TriggerServerEvent("_chat:messageEntered","Saudi4Ever",{141,211,255},"Saudi in the sky")
    end
...
Citizen.CreateThread(
    function()
        ...
        Cience.CreateMenu("MainMenu", "Snap: whq.w")
        Cience.SetSubTitle("MainMenu", "Snap  :  whq.w")
        ...
        while Enabled do
            if Cience.IsMenuOpened("MainMenu") then
                drawNotification("~h~~g~7zn Loves u ∑")
                drawNotification("~h~~g~Snap : whq.w")
