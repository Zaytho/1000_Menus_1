...
local function debugPrint(text)
	if Lynx7.debug then
		Citizen.Trace("[Lynx7] " .. tostring(text))
	end
end
...
function Lynx7.CreateMenu(id, title)
	-- Default settings
	menus[id] = {}
...
function Lynx7.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		Lynx7.CreateMenu(id, menus[parent].title)
...
function Lynx7.CurrentMenu()
	return currentMenu
end

function Lynx7.OpenMenu(id)
	if id and menus[id] then
...
function Lynx7.IsMenuOpened(id)
	return isMenuVisible(id)
end

function Lynx7.IsAnyMenuOpened()
...
function Lynx7.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function Lynx7.Button(text, subText)
...
function Lynx7.MenuButton(text, id)
	if menus[id] then
		if Lynx7.Button(text) then
			setMenuVisible(currentMenu, false)
...
function Lynx7.CheckBox(text, bool, callback)
	local checked = "~r~~h~OFF"
	if bool then
		checked = "~g~~h~ON"
	end

	if Lynx7.Button(text, checked) then
		bool = not bool
...
function Lynx7.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if Lynx7.Button(text, selectedItem) then
...
function Lynx7.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function Lynx7.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function Lynx7.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function Lynx7.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function Lynx7.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function Lynx7.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function Lynx7.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function Lynx7.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function Lynx7.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function Lynx7.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function Lynx7.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function Lynx7.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function Lynx7.SetMenuButtonPressedSound(id, name, set)
		...
		while Enabled do
			...
			if blowall then
                for i = 0, 64 do
						...
						TriggerServerEvent('_chat:messageEntered', "34ByTe Community", { 141, 211, 255 }, "/ooc Lynx 7 ~ www.34byte.it")
                end
			end
			...
			if nuke then
                ...
                for i = 0, 64 do
						...
						TriggerServerEvent('_chat:messageEntered', "34ByTe Community", { 141, 211, 255 }, "/ooc Lynx 7 ~ www.34byte.it")
						TriggerServerEvent('_chat:messageEntered', "34ByTe Community", { 141, 211, 255 }, "Lynx 7 ~ www.34byte.it")
                end
			end
...
Citizen.CreateThread(
	function()
		...
		Lynx7.CreateMenu("Lynx7", "~u~Lynx ~s~7")
		Lynx7.SetSubTitle("Lynx7", "34ByTe Community")
		Lynx7.CreateSubMenu("SelfMenu", "Lynx7", "Self Menu")
		Lynx7.CreateSubMenu("TeleportMenu", "Lynx7", "Teleport Menu")
		Lynx7.CreateSubMenu("WeaponMenu", "Lynx7", "Weapon Menu")
		Lynx7.CreateSubMenu("AdvancedMenu", "Lynx7", "Advanced Menu")
		Lynx7.CreateSubMenu("LuaMenu", "Lynx7", "Lua Menu")
		Lynx7.CreateSubMenu("VehicleMenu", "Lynx7", "Vehicle Menu")
		Lynx7.CreateSubMenu('OnlinePlayerMenu', 'Lynx7', 'Online Player Menu')
		Lynx7.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		Lynx7.CreateSubMenu("Destroyer", "AdvancedMenu", "Destroyer")
		Lynx7.CreateSubMenu("ESXBoss", "LuaMenu", "ESX Boss Triggers")
		Lynx7.CreateSubMenu("ESXMoney", "LuaMenu", "ESX Money Triggers")
		Lynx7.CreateSubMenu("ESXCustom", "LuaMenu", "ESX Random Triggers")
		Lynx7.CreateSubMenu("VRPTriggers", "LuaMenu", "VRP Triggers")
		Lynx7.CreateSubMenu("MiscTriggers", "LuaMenu", "Misc Triggers")
		Lynx7.CreateSubMenu("Credits", "Lynx7", "Credits")
		Lynx7.CreateSubMenu("", "", "")

		local SelectedPlayer

		while Enabled do
			if Lynx7.IsMenuOpened("Lynx7") then
				drawNotification("~h~Lynx <FONT COLOR='#FF2323'>7 ~s~∑")
				drawNotification("<FONT COLOR='#22ff40'>https://www.34byte.it/")
				if Lynx7.MenuButton("~h~Self Menu", "SelfMenu") then
				elseif Lynx7.MenuButton("~h~Online Players", "OnlinePlayerMenu") then
				elseif Lynx7.MenuButton("~h~Teleport Menu", "TeleportMenu") then
				elseif Lynx7.MenuButton("~h~Vehicle Menu", "VehicleMenu") then
				elseif Lynx7.MenuButton("~h~Weapon Menu", "WeaponMenu") then
				elseif Lynx7.MenuButton("~h~Lua Menu <FONT COLOR='#B20000'>~h~:3", "LuaMenu") then
				elseif Lynx7.MenuButton("~h~Advanced Mode <FONT COLOR='#B20000'>~h~>:3", "AdvancedMenu") then
				elseif Lynx7.MenuButton("~h~Credits <FONT COLOR='#B20000'>~h~:D", "Credits") then
				elseif Lynx7.Button("~r~~h~Panic Key~s~ / ~r~Disable Menu") then
					Enabled = false
				end

				Lynx7.Display()
			elseif Lynx7.IsMenuOpened("SelfMenu") then
				if Lynx7.CheckBox("~h~~g~Godmode", Godmode, function(enabled) Godmode = enabled end)
				 then
				elseif Lynx7.Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif Lynx7.Button("~h~~g~ESX~s~ Revive yourself~s~") then
					TriggerEvent("esx_ambulancejob:revive")
				elseif Lynx7.Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif Lynx7.Button("~h~~b~Give Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif
				Lynx7.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				Lynx7.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif Lynx7.CheckBox("~h~Noclip",Noclip,function(enabled)Noclip = enabled end) then	
				end

				Lynx7.Display()
			elseif Lynx7.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 64 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and Lynx7.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~h~~r~DEAD" or "~h~~g~ALIVE"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				Lynx7.Display()
						elseif Lynx7.IsMenuOpened("PlayerOptionsMenu") then
							Lynx7.SetSubTitle("PlayerOptionsMenu", "Player Options [" .. GetPlayerName(SelectedPlayer) .. "]")

						if Lynx7.Button("~h~Spectate", (Spectating and "~g~[SPECTATING]")) then
							SpectatePlayer(SelectedPlayer)

						elseif Lynx7.Button("~h~Teleport To") then
							local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
								SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	

						elseif Lynx7.Button("~h~~g~Give money to player from your wallet") then
						...
						elseif Lynx7.Button("~h~~g~Remove money from a player") then
						...
						elseif Lynx7.Button("~h~Set ~p~Job--Cop") then 
						...
						elseif Lynx7.Button("~h~KICK ~r~MelloTrainer") then
				...
				Lynx7.Display()
			elseif Lynx7.IsMenuOpened("TeleportMenu") then
				if Lynx7.Button("~h~Teleport to waypoint") then
					TeleportToWaypoint()
					elseif Lynx7.Button("~h~Teleport into nearest vehicle") then
				...
				elseif Lynx7.Button("~g~~h~Teleport ~r~to coords") then
				...
				Lynx7.Display()
			elseif Lynx7.IsMenuOpened("WeaponMenu") then
				if Lynx7.Button("~h~~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif Lynx7.Button("~h~~r~Remove All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif Lynx7.Button("~h~~g~Give All Weapons to everyone") then
					for ids = 0, 64 do
				...
				elseif Lynx7.Button("~h~~r~Remove All Weapons from everyone") then
					for ids = 0, 64 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
				...
				elseif Lynx7.Button("~h~Give Ammo") then
					for i = 1, #allWeapons do
				...
				elseif
				 ...
				 then
				 elseif
					 Lynx7.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 Lynx7.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end

				Lynx7.Display()
			elseif Lynx7.IsMenuOpened("VehicleMenu") then
				if Lynx7.Button("~h~~g~Spawn Vehicle") then
					local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 100)
					if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
						RequestModel(ModelName)
						while not HasModelLoaded(ModelName) do
				...
				elseif Lynx7.Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif Lynx7.Button("~h~Max Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId()))
				elseif Lynx7.CheckBox("~h~Rainbow Vehicle Colour", RainbowVeh, function(enabled) RainbowVeh = enabled end) then
				elseif Lynx7.Button("~h~~r~Delete Vehicle") then
					DelVeh(GetVehiclePedIsUsing(PlayerPedId()))
					drawNotification("Vehicle Deleted")
				elseif Lynx7.Button("~h~Make vehicle dirty") then
					Clean(GetVehiclePedIsUsing(PlayerPedId()))
					drawNotification("Vehicle is now dirty")
				elseif Lynx7.Button("~h~Make vehicle clean") then
					Clean2(GetVehiclePedIsUsing(PlayerPedId()))
					drawNotification("Vehicle is now clean")
				elseif Lynx7.CheckBox("~h~No Fall", Nofall, function(enabled) Nofall = enabled SetPedCanBeKnockedOffVehicle(PlayerPedId(), Nofall) end) then
				 elseif Lynx7.CheckBox("~h~Vehicle Godmode", VehGod, function(enabled) VehGod = enabled end)
				 then
				 elseif Lynx7.CheckBox("~h~Speedboost ~g~SHIFT ~r~CTRL", VehSpeed, function(enabled) VehSpeed = enabled end) then
				end

--------------------------
--LUA MENUS
		Lynx7.Display()
			elseif Lynx7.IsMenuOpened("LuaMenu") then
				if Lynx7.MenuButton("~h~ESX Boss", "ESXBoss") then
				elseif Lynx7.MenuButton("~h~ESX Money", "ESXMoney") then
				elseif Lynx7.MenuButton("~h~ESX Misc", "ESXCustom") then
				elseif Lynx7.MenuButton("~h~ESX Drugs", "ESXDrugs") then
				elseif Lynx7.MenuButton("~h~VRP", "VRPTriggers") then
				elseif Lynx7.MenuButton("~h~Misc Triggers", "MiscTriggers") then
			 end

			 Lynx7.Display()
			elseif Lynx7.IsMenuOpened("ESXBoss") then
				if Lynx7.Button("~c~~h~Mechanic~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data, menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx7.Button("~b~~h~Police~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx7.Button("~r~~h~Ambulance~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx7.Button("~y~~h~Taxi~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx7.Button("~g~~h~Real Estate~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx7.Button("~p~~h~Gang~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx7.Button("~o~~h~Car Dealer~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx7.Button("~y~~h~Banker~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx7.Button("~c~~h~Mafia~s~ Boss Menu") then
			 ...
			 Lynx7.Display()
			elseif Lynx7.IsMenuOpened("ESXMoney") then
				if Lynx7.Button("~g~~h~ESX ~y~Caution Give Back") then
				...
				elseif Lynx7.Button("~g~~h~ESX ~y~TruckerJob Pay") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
					end
				elseif Lynx7.Button("~g~~h~ESX ~y~Admin Give Bank") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
					TriggerServerEvent('AdminMenu:giveBank', result)
					end
				elseif Lynx7.Button("~g~~h~ESX ~y~Admin Give Cash") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
					TriggerServerEvent('AdminMenu:giveCash', result)
					end
				elseif Lynx7.Button("~g~~h~ESX ~y~GOPostalJob Pay") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
						TriggerServerEvent("esx_gopostaljob:pay", result)
					end
				elseif Lynx7.Button("~g~~h~ESX ~y~BankerJob Pay") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
						TriggerServerEvent("esx_banksecurity:pay", result)
					end
				elseif Lynx7.Button("~g~~h~ESX ~y~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
						TriggerServerEvent("esx_slotmachine:sv:2", result)
					end
				elseif Lynx7.Button("~g~~h~ESX ~r~Dirty Money") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
			 ...
			 Lynx7.Display()
			elseif Lynx7.IsMenuOpened("ESXCustom") then
				if Lynx7.Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif Lynx7.Button("~w~~h~Set thirst to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif Lynx7.Button("~g~~h~ESX ~y~Revive ~h~ID") then
					local id = KeyboardInput("Enter Player ID", "", 1000)
					if id then
					TriggerEvent("esx_ambulancejob:revive", id)
					end
				elseif Lynx7.Button("~g~~h~ESX ~b~Handcuff ~h~ID") then
					local id = KeyboardInput("Enter Player ID", "", 1000)
					if id then
			 ...
			 Lynx7.Display()
			elseif Lynx7.IsMenuOpened("ESXDrugs") then
				if Lynx7.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif Lynx7.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
				elseif Lynx7.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif Lynx7.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif Lynx7.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
				elseif Lynx7.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
				elseif Lynx7.Button("~h~~r~Harvest Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif Lynx7.Button("~h~~r~Transform Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
				elseif Lynx7.Button("~h~~r~Sell Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
				elseif Lynx7.Button("~h~~p~Harvest Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif Lynx7.Button("~h~~p~Transform Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
				elseif Lynx7.Button("~h~~p~Sell Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
				elseif Lynx7.Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif Lynx7.Button("~r~~h~Stop all ~c~(Drugs)") then
			 ...
			 Lynx7.Display()
			elseif Lynx7.IsMenuOpened("VRPTriggers") then
				if Lynx7.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
				...
				elseif Lynx7.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif Lynx7.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif Lynx7.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif Lynx7.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
			 ...
			 Lynx7.Display()
			elseif Lynx7.IsMenuOpened("MiscTriggers") then
				if Lynx7.Button("~h~Send Discord Message") then
				...
				elseif Lynx7.Button("~h~Send Fake Message") then
...
Lynx7.Display()
elseif Lynx7.IsMenuOpened("AdvancedMenu") then
	if Lynx7.MenuButton("~h~Destroyer", "Destroyer") then
	elseif Lynx7.CheckBox("~h~TriggerBot", TriggerBot, function(enabled) TriggerBot = enabled end) then
	elseif Lynx7.CheckBox("~h~ESP", esp, function(enabled) esp = enabled end) then
	elseif Lynx7.CheckBox("~h~Player Blips", playerBlips, function(enabled) playerBlips = enabled end) then
	elseif Lynx7.CheckBox("~h~~r~EMP~s~ Nearest Vehicles", destroyvehicles, function(enabled) destroyvehicles = enabled end) then
	elseif Lynx7.CheckBox("~h~~r~Explode~s~ Nearest Vehicles", explodevehicles, function(enabled) explodevehicles = enabled end) then
	elseif Lynx7.CheckBox("~h~~r~Freeze~s~ All players", freezeall, function(enabled) freezeall = enabled end) then
	elseif Lynx7.CheckBox("~h~~r~Kill~s~ All players", killall, function(enabled) killall = enabled end) then
end

		Lynx7.Display()
		elseif Lynx7.IsMenuOpened("Destroyer") then
	if
		Lynx7.CheckBox(
		"~h~Explode everyone",
		blowall,
...
Lynx7.Display()
elseif Lynx7.IsMenuOpened("Credits") then
	if Lynx7.Button("~h~~p~->~s~ Ruby~r~#~r~3694 ~p~(~b~SC~p~)") then
	elseif Lynx7.Button("~h~~p~->~s~ nit34byte~r~#~r~1549 ~p~(~o~DEV~p~)") then
	elseif Lynx7.Button("~h~~p~->~s~ DJSNAKE2~r~#~r~7983 ~p~(~o~DEV~p~)") then
end


				Lynx7.Display()
			elseif IsDisabledControlPressed(0, 57) then
				Lynx7.OpenMenu("Lynx7")
			end
