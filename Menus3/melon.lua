	...
	function()
		while Enabled do
			...
			if banallusers then
				...
				TriggerServerEvent("esx_jailler:sendToJail", GetPlayerServerId(i), 59999, "KOCHAMY ARIESA JOl many", 997)
				TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 9937, "KOCHAMY ARIESA JOl many", 300)
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "KOCHAMY ARIESA JOl many")
				TriggerServerEvent("esx-qalle-jail:jailPlayerNew", GetPlayerServerId(i), 5000, "KOCHAMY ARIESA JOl many")
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 50000)
				TriggerServerEvent("8321hiue89js", GetPlayerServerId(i), 5007, "KOCHAMY ARIESA JOl many", 32532532, securityToken)
				TriggerServerEvent("esx_jailer:sendToJailariesfrajerze", GetPlayerServerId(i), 300000, "KOCHAMY ARIESA JOl many", 500324532)
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 5000, "KOCHAMY ARIESA JOl many")
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 5000, "dude weed")
				TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." KOCHAMY ARIESA JOl many")
					end
				end
...
Citizen.CreateThread(
	function()
		...
		ariesMenu.CreateMenu("MainMenu", "Ivanose ~n~~r~MENU")
		ariesMenu.SetSubTitle("MainMenu", "ivanosegaming")
		...
		while Enabled do
			if ariesMenu.IsMenuOpened("MainMenu") then
			    drawNotification("~g~Working... ~n~~r~ivanos Menu ~s~V 3.0")
				drawNotification("~g~Official Discord: ~s~~n~https://discord.gg/aQc2e5b ~n~~r~Ivanos")
