...
local _notifTitle = "~p~WarzModz Premium"
...
local function drawFooter()
	if menus[currentMenu] then
		...
		if welcomeMsg then
			welcomeMsg = false
			SwagUI.SendNotification({text = "Si vous rencontrez des problèmes, veuillez me contacter sur Discord : qEnN72u ", type = "info"})
		end
	end
end
...
local function drawButton(text, subText, color, subcolor)
	...
	if multiplier then
		...
		if (text ~= "~r~Dieu Menu" and text ~= "~b~Paramètres Menu") and menus[currentMenu].subTitle == "WarzModz Premium" then -- and subText == "isMenu"
			drawText(
			text,
			...
			elseif text == "Visuel Options" then
			...
			elseif text == "Vehicule Options" then
...
function SwagUI.CreateMenu(id, title)
	...
	menus[id].subTitle = "WarzModz Premium"
	menus[id].branding = "YouTube/WarzModz"
	menus[id].version = "v1.5"
...
function SwagUI.DrawPlayerInfo(player)
	...
	-- infoBox = {
	-- 	tostring("Name: " .. Swag:CheckName(GetPlayerName(data))),
	...
	-- 	tostring("Task: " .. Swag.Game:GetPedStatus(dataPed)),
	-- }
...
function SwagUI.DrawWeaponPreview(index)
...
local function RegarderPlayer(selectedPlayer)
...
local ptags = {}
-- Thread that handles all menu toggles (Invinsible, ESP, etc)
local function MenuToggleThread()
	while isMenuEnabled do
		...
		SetPlayerInvincible(PlayerId(), Invinsible)
		SetEntityInvincible(PlayerPedId(), Invinsible)
		...
		if Exploservehicles then
			for vehicle in EnumerateVehicles() do
				if (vehicle ~= GetVehiclePedIsIn(PlayerPedId(), false)) then
					NetworkRequestControlOfEntity(vehicle)
					NetworkExploserVehicle(vehicle, true, true, false)
				end
			end
		end
...
local function MenuRuntimeThread()
	...
	SwagUI.SetSubTitle("SwagMainMenu", "WarzModz Premium")

	-- WarzModz Premium CATEGORIES
	SwagUI.CreateSubMenu("SelfMenu", "SwagMainMenu", "Mes Options")
	SwagUI.CreateSubMenu("SelfSuperPowers", "SelfMenu", "Super Pouvoirs")
	SwagUI.CreateSubMenu('OnlinePlayersMenu', 'SwagMainMenu', "Player List")
	
	SwagUI.CreateSubMenu("VisualMenu", "SwagMainMenu", "Visuel Options")
	SwagUI.CreateSubMenu("ESPMenu", "VisualMenu", "ESP")
	
	SwagUI.CreateSubMenu("TeleportMenu", "SwagMainMenu", "Teleport Options")
	
	-- WarzModz Premium > Vehicule Options
	SwagUI.CreateSubMenu("LocalVehicleMenu", "SwagMainMenu", "Vehicule Options")
	-- WarzModz Premium > Vehicule Options > Apparition De Véhicule
	SwagUI.CreateSubMenu("LocalVehicleSpawner", "LocalVehicleMenu", "Apparition De Véhicule")
	-- WarzModz Premium > Vehicule Options > Apparition De Véhicule > $class
	...
	SwagUI.CreateSubMenu("LocalWepMenu", "SwagMainMenu", "Arme Options")
	SwagUI.CreateSubMenu("ServerMenu", "SwagMainMenu", "Server Options")
	SwagUI.CreateSubMenu("GriefMenu", "SwagMainMenu", "Griefer Options")
	SwagUI.CreateSubMenu("MenuSettings", "SwagMainMenu", "Paramètres Menu")
	...
	-- ONLINE PLAYERS > PLAYER > Arme Options MENU
	SwagUI.CreateSubMenu('OnlineWepMenu', 'PlayerOptionsMenu', 'Weapon Menu')
	SwagUI.CreateSubMenu('OnlineWepCategory', 'OnlineWepMenu', 'Donner Une Arme')
	...
	SwagUI.CreateSubMenu("OnlineTrollMenu", "PlayerOptionsMenu", "Troll Options")
	
	SwagUI.CreateSubMenu('OnlineVehicleMenuPlayer', 'PlayerOptionsMenu', "Vehicule Options")
	SwagUI.CreateSubMenu('ESXMenuPlayer', 'PlayerOptionsMenu', "ESX Options")

	SwagUI.CreateSubMenu("LocalWepCategory", "LocalWepMenu", "Donner Une Arme")
	...
	SwagUI.CreateSubMenu("AimbotOptions", "LocalWepMenu", "Aimbot Options")
	...
	SwagUI.CreateSubMenu("MenuSettingsColor", "MenuSettings", "Change Menu Color")
	...
	while isMenuEnabled do
		...
		if SwagUI.IsMenuOpened("SwagMainMenu") then
			if SwagUI.MenuButton("Mes Options", "SelfMenu") then end
			if SwagUI.MenuButton("Player List", "OnlinePlayersMenu") then end
			if SwagUI.MenuButton("Visuel Options", "VisualMenu") then end
			if SwagUI.MenuButton("Teleport Options", "TeleportMenu") then end
			if SwagUI.MenuButton("Vehicule Options", "LocalVehicleMenu") then end
			if SwagUI.MenuButton("Arme Options", "LocalWepMenu") then end
			if SwagUI.MenuButton("Server Options", "ServerMenu") then end
			if SwagUI.MenuButton("~r~Dieu Menu", "GriefMenu") then end
			if SwagUI.MenuButton("~b~Paramètres Menu", "MenuSettings") then end

			SwagUI.Display()
		elseif SwagUI.IsMenuOpened("SelfMenu") then
			if SwagUI.MenuButton("Super Pouvoirs", "SelfSuperPowers") then end

			-- if SwagUI.Button("Sky Dive") then
			-- 	TaskSkyDive(PlayerPedId())
			-- end
			
			if SwagUI.Button("Santé Maximale") then
				SetEntityHealth(PlayerPedId(), 200)
			end
			
			if SwagUI.Button("Armure Maximale") then
				SetPedArmour(PlayerPedId(), 200)
			end

			if SwagUI.CheckBox("Endurance Infinie", InfStamina, function(enabled) InfStamina = enabled end) then
				
			end

			if SwagUI.Button("Se Ressusciter") then
			...
			if SwagUI.Slider("Vitesse De Déplacement", SliderOptions.FastRun.Words, SliderOptions.FastRun.Selected, function(selectedIndex)
		...
		elseif SwagUI.IsMenuOpened("SelfSuperPowers") then
			if SwagUI.CheckBox("Invinsible", Invinsible, function(enabled) Invinsible = enabled end) then

			end

			if SwagUI.CheckBox("Super Saut", SuperJump, function(enabled) SuperJump = enabled end) then

			end

			if SwagUI.CheckBox("Invisible", Invisible, function(enabled) Invisible = enabled end) then
		...
		elseif SwagUI.IsMenuOpened("VisualMenu") then
			...
			if SwagUI.CheckBox("Force Crosshair", Crosshair, function(enabled) Crosshair = enabled end) then end
			if SwagUI.CheckBox("Force Minimap", showMinimap, function(enabled) showMinimap = enabled end) then end
			if SwagUI.CheckBox("Force Player Blips", playerBlips, function(enabled) playerBlips = enabled end) then end
			if SwagUI.CheckBox("Force Gamertags", showNametags, function(enabled) showNametags = enabled end) then end
		...
		elseif SwagUI.IsMenuOpened("GriefMenu") then
			...
			if SwagUI.Button("Retirer Toutes Les Armes From Everyone") then
				local plist = GetActivePlayers()
				for i = 1, #plist do
					local id = plist[i]
					local ped = GetPlayerPed(id)
		...
		elseif SwagUI.IsMenuOpened("LocalWepMenu") then
			...
			-- if SwagUI.MenuButton("Aimbot Options", "AimbotOptions") then
			-- end
			...
			if SwagUI.Button("~g~Donner Toutes Les Armes") then
			...
			if SwagUI.Button("~r~Retirer Toutes Les Armes") then
				for i = 1, #t_Weapons do
		...
		elseif SwagUI.IsMenuOpened("LocalVehicleMenu") then
			...
			if SwagUI.MenuButton("Apparition De Véhicule", "LocalVehicleSpawner") then
			end

			if SwagUI.ComboBox("Véhicule Actions", ComboOptions.VehicleActions.Words, ComboOptions.VehicleActions.Selected, function(selectedIndex)
			...
			if SwagUI.MenuButton("Modifier Le Véhicule", "LSC") then end

			if SwagUI.CheckBox("Easy Handling", Swag.Toggle.EasyHandling) then
				Swag.Toggle.EasyHandling = not Swag.Toggle.EasyHandling
			end

			if SwagUI.CheckBox("Seatbelt (No Fall)", Swag.Toggle.VehicleNoFall) then
			...
			if SwagUI.CheckBox("Vehicle Invinsible", VehGod,
					function(enabled)
						VehGod = enabled
					end) 
				then
			end
			if SwagUI.CheckBox("Speedboost ~g~SHIFT ~r~CTRL", VehSpeed,
					function(enabled)
					VehSpeed = enabled
					end)
				then
		...
		elseif SwagUI.IsMenuOpened("LocalVehicleSpawner") then
			...
			if SwagUI.Button("Apparition Véhicule by Hash") then
		...
		elseif SwagUI.IsMenuOpened("ServerMenu") then
			SwagUI.SetSubTitle("ServerMenu", "Server Options - " .. GetCurrentServerEndpoint())
			if SwagUI.MenuButton("Resource List", "ServerResources") then end
			if SwagUI.MenuButton("ESX Boss Options", "ESXBoss") then end
			if SwagUI.MenuButton("Money Options", "ServerMoneyOptions") then end
			if SwagUI.MenuButton("ESX Misc Options", "ESXMisc") then end
			if SwagUI.MenuButton("ESX Drug Options", "ESXDrugs") then end
			if SwagUI.MenuButton("VRP Options", "VRPOptions") then end
			if SwagUI.MenuButton("Misc Options", "MiscServerOptions") then end

			SwagUI.Display()
			
		elseif SwagUI.IsMenuOpened("MenuSettings") then
			if SwagUI.MenuButton("Change Color Theme", "MenuSettingsColor") then
			end
			if SwagUI.Button("~r~Stop Menu") then
				isMenuEnabled = false
			end
			SwagUI.Display()
		elseif SwagUI.IsMenuOpened("MenuSettingsColor") then
			if SwagUI.CheckBox("Dynamic Theme", dynamicColorTheme, function(enabled) dynamicColorTheme = enabled end) then
			end
			if SwagUI.Button("Red", nil, themeColors.red) then
				_menuColor.base = themeColors.red
			end
			if SwagUI.Button("Orange", nil, themeColors.orange) then
				_menuColor.base = themeColors.orange
			end
			if SwagUI.Button("Yellow", nil, themeColors.yellow) then
				_menuColor.base = themeColors.yellow
			end
			if SwagUI.Button("Green", nil, themeColors.green) then
				_menuColor.base = themeColors.green
			end
			if SwagUI.Button("Blue", nil, themeColors.blue) then
				_menuColor.base = themeColors.blue
			end
			if SwagUI.Button("Purple", nil, themeColors.purple) then
				_menuColor.base = themeColors.purple
			end
		...
		elseif SwagUI.IsMenuOpened("ServerResources") then
			for _, resource in pairs(validResources) do
				if SwagUI.MenuButton(resource, 'ResourceData') then
		...
		elseif SwagUI.IsMenuOpened("ServerMoneyOptions") then
			...
			if SwagUI.Button("~g~ESX ~y~Caution Give Back") then
			...
			if SwagUI.Button("~g~ESX ~y~TruckerJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			end
			if SwagUI.Button("~g~ESX ~y~Admin Give Bank") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveBank', result)
				end
			end
			if SwagUI.Button("~g~ESX ~y~Admin Give Cash") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveCash', result)
				end
			end
			if SwagUI.Button("~g~ESX ~y~GOPostalJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			end
			if SwagUI.Button("~g~ESX ~y~BankerJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			end
			if SwagUI.Button("~g~ESX ~y~Slot Machine") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
		...
		elseif SwagUI.IsMenuOpened("ESXMisc") then
			...
			elseif SwagUI.Button("~g~ESX ~y~Revive ID") then
			...
			elseif SwagUI.Button("Send All Bill (shitlord)") then
			...
			elseif SwagUI.Button("Handcuff ID") then
		...
		elseif SwagUI.IsMenuOpened("MiscServerOptions") then
			...
			elseif SwagUI.Button("Send ambulance alert on waypoint") then
				...
				if DoesBlipExist(GetFirstBlipInfoId(8)) then
					...
					SwagUI.SendNotification("~g~Ambulance alert sent to waypoint!")
				else
					SwagUI.SendNotification("~r~No waypoint set!")
				end
				...
				if transmitter then
					if receiver then
						if message then
							...
							SwagUI.SendNotification("~r~You must specify a message.")
						end
					else
						SwagUI.SendNotification("~r~You must specify a receiving number.")
					end
				else
					SwagUI.SendNotification("~r~You must specify a transmitting number.")
				end
			...
			elseif SwagUI.Button("esx_mugging Give item") then
				local itemName = KeyboardInput("Enter item name", "", 20)
				if itemName then
					TriggerServerEvent('esx_mugging:giveItems', (itemName))
					SwagUI.SendNotification("Successfully given item ~g~" .. itemName)
				else
					SwagUI.SendNotification("~r~You must specify an item")
				end
			end
		...
		elseif SwagUI.IsMenuOpened("PlayerOptionsMenu") then
			...
			if SwagUI.Button("Regarder", (Swag.Player.Spectating and "~g~[SPECTATING]")) then
				CreateThreadNow(function()
					RegarderPlayer(SelectedPlayer)
				end)
			end

			if SwagUI.Button("Téléportation Sur Le Joueur") then
				Swag.Game:TeleportToPlayer(SelectedPlayer)
			end

			if SwagUI.Button("Téléportation Dans Le Véhicule") then
				TeleportToPlayerVehicle(SelectedPlayer)
			end

			if SwagUI.MenuButton("Arme Options", "OnlineWepMenu") then end
			if SwagUI.MenuButton("Vehicule Options", "OnlineVehicleMenuPlayer") then end
			if SwagUI.MenuButton("ESX Options", "ESXMenuPlayer") then end
			if SwagUI.MenuButton("Troll Options", "OnlineTrollMenu") then end
			if SwagUI.Button("~r~Explosion Invisible") then
				AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, false, true, 0)
			end
			if SwagUI.Button("~y~Exploser") then
				AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, true, false, 100000.0)
			end
			if SwagUI.Button("Donner Toutes Les Armes") then
				for i = 1, #t_Weapons do
					GiveWeaponToPed(GetPlayerPed(SelectedPlayer), t_Weapons[i][1], 128, false, false)
				end
			end
			if SwagUI.Button("Retirer Toutes Les Armes") then
		...
		elseif SwagUI.IsMenuOpened("OnlineTrollMenu") then
			...
			if SwagUI.Button("Airstrike") then
				AirstrikePlayer(SelectedPlayer)
			end

			if SwagUI.Button("Tazer Le Joueur") then
		...
		elseif SwagUI.IsMenuOpened("ESXMenuPlayer") then
			if SwagUI.Button("~g~ESX ~s~Envoyer Une Facture") then
			...
			elseif SwagUI.Button("~g~ESX ~s~Mètre Les Menottes") then
				TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(SelectedPlayer))
			elseif SwagUI.Button("~g~ESX ~s~Réanimer Le Joueur") then
				TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(SelectedPlayer))
			elseif SwagUI.Button("~g~ESX ~s~Mètre En Prison") then
		...
		elseif SwagUI.IsMenuOpened("OnlineWepMenu") then
			SwagUI.SetSubTitle("OnlineWepMenu", "Arme Options - " .. GetPlayerName(SelectedPlayer) .. "")
			SwagUI.MenuButton("Donner Une Arme", "OnlineWepCategory")

			SwagUI.DrawPlayerInfo(SelectedPlayer)
			SwagUI.Display()
		
		elseif SwagUI.IsMenuOpened("OnlineWepCategory") then
			SwagUI.SetSubTitle("OnlineWepCategory", "Donner Une Arme - " .. GetPlayerName(SelectedPlayer) .. "")
		...
		elseif SwagUI.IsMenuOpened("OnlineVehicleMenuPlayer") then
			SwagUI.SetSubTitle("OnlineVehicleMenuPlayer", "Vehicule Options [" .. GetPlayerName(SelectedPlayer) .. "]")
			if SwagUI.Button("Apparition Véhicule") then
				local ped = GetPlayerPed(SelectedPlayer)
			...
			if SwagUI.Button("Donner Clé Du Véhicule") then
				local ped = GetPlayerPed(SelectedPlayer)
				local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 20)
			...
			if SwagUI.Button("Viré Du Véhicule") then
				ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
			end
			if SwagUI.Button("Supprimer Le Véhicule") then
				local playerPed = GetPlayerPed(SelectedPlayer)
				local veh = GetVehiclePedIsIn(playerPed)
				RemoveVehicle(veh)
			end
			if SwagUI.Button("Détruire Le Moteur") then
			...
			if SwagUI.Button("Voler Le Véhicule") then
			...
			if SwagUI.Button("Vandaliser La Voiture") then
				local playerPed = GetPlayerPed(SelectedPlayer)
				...
				SetVehicleNumberPlateText(playerVeh, "Swag Menu")
				...
				SwagUI.SendNotification("~g~Vehicle Fucked Up!")
			end
