		...
		while Enabled do
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best Nisi Menu", 10000000)
				end
			end
...
Citizen.CreateThread(
	function()
		...
		Nisi.CreateMenu("MainMenu", "~u~Nisi")
		Nisi.SetSubTitle("MainMenu", "🇦🇱                 Nisi's Menu                  🇽🇰")
		...
		while Enabled do
			if Nisi.IsMenuOpened("MainMenu") then
				...
				drawNotification("Discord: ~n~~r~Nisi#5322")
				...
				elseif Nisi.MenuButton("~h~<FONT COLOR='#FF0000'>LUA MENU                                               ~s~~h~~r~»", "ServerMenu") then
	...
	then
			...
			elseif Nisi.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and Nisi.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."~h~~y~ » "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~s~[~r~DEAD~s~]" or "~s~[~g~ALIVE~s~]"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
			...
			elseif Nisi.IsMenuOpened("PlayerOptionsMenu") then
				...
				elseif Nisi.Button("~g~Revive ~o~Korioz") then
					TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(SelectedPlayer))
				elseif Nisi.Button("~g~Revive ~b~VRP") then CreatePickup(GetHashKey("PICKUP_HEALTH_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif Nisi.Button("Give ~b~Armour ") then CreatePickup(GetHashKey("PICKUP_ARMOUR_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif Nisi.Button("~h~~r~Kill ~s~Player") then AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 4, 1337.0, false, true, 0.0) 
