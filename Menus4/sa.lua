    ...
    Citizen.CreateThread(
        function()
            while true do
                ...
                elseif IsControlJustPressed(1, 14) then
...
function SHARKMenu.CreateMenu(id, title)
...
function SHARKMenu.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		SHARKMenu.CreateMenu(id, menus[parent].title)
...
function SHARKMenu.CurrentMenu()
	return currentMenu
end

function SHARKMenu.OpenMenu(id)
	if id and menus[id] then
...
function SHARKMenu.IsMenuOpened(id)
	return isMenuVisible(id)
end

function SHARKMenu.IsAnyMenuOpened()
...
function SHARKMenu.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function SHARKMenu.Button(text, subText)
...
function SHARKMenu.MenuButton(text, id)
	if menus[id] then
		if SHARKMenu.Button(text) then
			setMenuVisible(currentMenu, false)
...
function SHARKMenu.CheckBox(text, bool, callback)
	local checked = "~r~~h~Off"
	if bool then
		checked = "~g~~h~On"
	end

	if SHARKMenu.Button(text, checked) then
		bool = not bool
...
function SHARKMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if SHARKMenu.Button(text, selectedItem) then
...
function SHARKMenu.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function SHARKMenu.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function SHARKMenu.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function SHARKMenu.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function SHARKMenu.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["111"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function SHARKMenu.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"titleBackgroundColor",
		{["215"] = r, ["152"] = g, ["254"] = b, ["114"] = a or menus[id].titleBackgroundColor.a}
	)
end

function SHARKMenu.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function SHARKMenu.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function SHARKMenu.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function SHARKMenu.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function SHARKMenu.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function SHARKMenu.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function SHARKMenu.SetMenuButtonPressedSound(id, name, set)
...
Citizen.CreateThread(
    ...
    GetVehiclePedIsIn(GetPlayerPed(-1), false) then SetEntityAsMissionEntity(GetVehiclePedIsIn(vehicle, true), 1, 1) DeleteEntity(GetVehiclePedIsIn(vehicle, true)) SetEntityAsMissionEntity(vehicle, 1, 1) DeleteEntity(vehicle) end end end
			...
			if banallusers then
				...
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "ALL GAY WELCOME GHOST menu LUA")
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " Welcom *GHOST menu LUA 3.1* Sell Mod Menu in https://discord.gg/45zyGC7")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best GHOST menu мeɴυ 5.1 https://discord.gg/45zyGC7", 10000000)
				end
			end
			...
			if spam then
                for i = 0, 128 do
                    ...
                    '/ooc ^1GHOST menu мeɴυ ^4LUA :^2https://discord.gg/45zyGC7',
                    {0, 0x99, 255},
                    '/ooc ^1GHOST menu мeɴυ ^4LUA :^2https://discord.gg/45zyGC7'
                )
                TriggerServerEvent('_chat:messageEntered', '^3Free ^4LUAS', {0, 0x99, 255}, '/ooc ^1GHOST menu^4LUA :^2https://discord.gg/45zyGC7')
                end
			end
...
Citizen.CreateThread(
	function()
		...
		SHARKMenu.CreateMenu("MainMenu", "~g~GHOST menu ~n~~y~MENU")
		SHARKMenu.SetSubTitle("MainMenu", "        ❗️ discord.gg/ZMew76a ❗️")
		SHARKMenu.CreateSubMenu("SelfMenu", "MainMenu", "PlayerMenu")
		SHARKMenu.CreateSubMenu("World", "MainMenu", "World")
		SHARKMenu.CreateSubMenu("Fuck", "MainMenu", "Fuck")
		SHARKMenu.CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
		SHARKMenu.CreateSubMenu("ServerMenu", "MainMenu", "LUA Execution")
		SHARKMenu.CreateSubMenu("TeleportMenu", "MainMenu", "Teleport Menu")
		SHARKMenu.CreateSubMenu('OnlinePlayerMenu', 'MainMenu', 'Online Player Menu')
		SHARKMenu.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		SHARKMenu.CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Single Weapon Menu')
		SHARKMenu.CreateSubMenu("WepMenu", "MainMenu", "Weapon Menu")
		SHARKMenu.CreateSubMenu("SingleWepMenu", "WepMenu", "Single Weapon Menu")
		SHARKMenu.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
		SHARKMenu.CreateSubMenu("ESXMoney", "ServerMenu", "Money Options")
		SHARKMenu.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
		SHARKMenu.CreateSubMenu("MODEL", "ServerMenu", "MODEL PLAYER")
		SHARKMenu.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
		SHARKMenu.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		SHARKMenu.CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		SHARKMenu.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")

		local SelectedPlayer

		while Enabled do
			if SHARKMenu.IsMenuOpened("MainMenu") then
			    drawNotification("~g~Working... ~n~~r~GHOST menu мeɴυ ~s~V 3.2")
				drawNotification("~g~Official Discord: ~s~~n~ https://discord.gg/45zyGC7 ~n~~r~GHOST menu мeɴυ")
				   if SHARKMenu.MenuButton("🗣 ~y~Player ~b~Menu   ~h~~r~»", "SelfMenu") then
				elseif SHARKMenu.MenuButton("⚡️ ~r~~h~Online ~b~Players   ~h~~r~»", "OnlinePlayerMenu") then
				elseif SHARKMenu.MenuButton("🌍 ~y~~h~World ~b~Menu   ~h~~r~»", "World") then
				elseif SHARKMenu.MenuButton("⭐️ ~y~~h~Teleporte ~b~Menu   ~h~~r~»", "TeleportMenu") then
				elseif SHARKMenu.MenuButton("⭐️ ~y~~h~Vehicles ~b~Menu   ~h~~r~»", "VehMenu") then
				elseif SHARKMenu.MenuButton("🔫 ~y~~h~Weapons ~b~Menu  ~h~~r~»", "WepMenu") then
				elseif SHARKMenu.MenuButton("💣 ~y~~h~Fuck ~b~Server  💣   ~h~~r~»", "Fuck") then
				elseif SHARKMenu.MenuButton("~h~<FONT COLOR='#15f600'>∑ LUA MENU ∑   ~s~~h~~r~»", "ServerMenu") then
				elseif SHARKMenu.Button("~r~Disconnect") then
					Enabled = false
				end

				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("SelfMenu") then
				if
					SHARKMenu.CheckBox(
						"~h~~g~God ~h~~r~Mode",
						Godmode,
				 ...
				 then
				elseif SHARKMenu.Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif SHARKMenu.Button("~s~Revive ~h~~g~ESX") then
					TriggerEvent("esx_ambulancejob:revive")
				    TriggerEvent('ambulancier:selfRespawn')
				elseif SHARKMenu.Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif SHARKMenu.Button("~h~~b~Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif SHARKMenu.Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif SHARKMenu.Button("~w~~h~Set thirst to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif SHARKMenu.Button("~h~~b~Open Menu Jail ~g~ESX") then
					TriggerEvent("esx-qalle-jail:openJailMenu")
				elseif SHARKMenu.Button("~h~~b~Unjail") then
					TriggerServerEvent('esx_jailer:unjailTime', -1)
					TriggerServerEvent('JailUpdate', 0)
					TriggerEvent('UnJP')
				elseif
				SHARKMenu.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				SHARKMenu.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif
				SHARKMenu.CheckBox("~h~Fast Run ~r~NEW",fastrun,function(enabled)fastrun = enabled end)
			 	then
				elseif
				SHARKMenu.CheckBox(
					"~h~Invisible",
				...
				then
				elseif
				SHARKMenu.CheckBox("~h~NoClip",Noclip,function(enabled)Noclip = enabled end)
				then	
				end

		SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("TeleportMenu") then
				if SHARKMenu.Button("~h~Teleport ~h~~g~Waypoint") then
				TeleportToWaypoint()
			 elseif SHARKMenu.Button("~h~Teleport ~h~~g~Vehicle ") then
		...
		SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("World") then
			if
				SHARKMenu.CheckBox(
				"~h~ESP",
				esp,
		...
		SHARKMenu.Display()
		elseif SHARKMenu.IsMenuOpened("Fuck") then
		if SHARKMenu.Button("~h~~r~Give All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
				...
				elseif SHARKMenu.Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
	 ...
	 elseif SHARKMenu.Button("~h~~g~Fake chat Message") then 
	...
	then
	elseif SHARKMenu.Button("~h~~b~------Fuck all Vehicles------") then
	elseif SHARKMenu.Button("~h~~r~Rampinator") then
    ...
    elseif SHARKMenu.CheckBox("~h~~r~Delete~s~ Vehicles", deletenearestvehicle, function(eC) deletenearestvehicle = eC end) then 
	elseif SHARKMenu.Button("~h~~r~BORGAR~s~ Vehicles") then local cC = GetHashKey("xs_prop_hamburgher_wl") for vehicle in EnumerateVehicles() do local cD = CreateObject(cC, 0, 0, 0, true, true, true) AttachEntityToEntity(cD, vehicle, 0, 0, -1.0, 0.0, 0.0, 0, true, true, false, true, 1, true) end 
	elseif SHARKMenu.CheckBox("~h~~r~Explode~s~ Vehicles", explodevehicles, function(eC) explodevehicles = eC end) then
	elseif SHARKMenu.CheckBox("~h~~r~Fuck~s~ Vehicles", fuckallcars, function(eC) fuckallcars = eC end) then end;
	
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("WepMenu") then
				if SHARKMenu.MenuButton("~h~Specific Weapon", "SingleWepMenu") then
				elseif SHARKMenu.Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif SHARKMenu.Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif SHARKMenu.Button("~h~~b~Give Ammo") then
					for i = 1, #allWeapons do
			...
			elseif
				...
				elseif
				 ...
				 then
				 elseif
					 SHARKMenu.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 SHARKMenu.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end

				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("SingleWepMenu") then
				for i = 1, #allWeapons do
					if SHARKMenu.Button(allWeapons[i]) then
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				end

				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("VehMenu") then
				if SHARKMenu.Button("~h~~g~Spawn Vehicle") then
				...
				elseif SHARKMenu.Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif SHARKMenu.Button("~h~Max Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId())
)				elseif SHARKMenu.Button("~h~Max mechanics ~r~new") then
				...
				then
			    elseif SHARKMenu.Button("~h~~b~Change License Plate~r~ NEW") then
					local playerPed = GetPlayerPed(-1)
				...
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("ServerMenu") then
				if SHARKMenu.MenuButton("~s~Menu ~h~~r~RECRUIT PLAYERS", "RecrutarPlayers") then
				elseif SHARKMenu.MenuButton("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elseif SHARKMenu.MenuButton("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elseif SHARKMenu.MenuButton("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elseif SHARKMenu.MenuButton("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				elseif SHARKMenu.MenuButton("~s~Menu ~r~~h~Model ~s~NEW", "MODEL") then
				end
				
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("MODEL") then	
			if SHARKMenu.Button("~b~~h~Clown") then
			...
			elseif SHARKMenu.Button("~b~~h~Stripper") then
			local model = "s_f_y_stripper_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif SHARKMenu.Button("~b~~h~Cop") then
			local model = "s_m_y_cop_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif SHARKMenu.Button("~b~~h~CIA") then
			local model = "S_M_M_CIASec_01"
				RequestModel(GetHashKey(model)) 
				Wait(500)
				if HasModelLoaded(GetHashKey(model)) then
					SetPlayerModel(PlayerId(), GetHashKey(model))
					end					
					elseif SHARKMenu.Button("~b~~h~BlackOps") then
			local model = "s_m_y_blackops_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif SHARKMenu.Button("~b~~h~GunMan") then
			local model = "hc_gunman"
				RequestModel(GetHashKey(model)) 
				Wait(500)
				if HasModelLoaded(GetHashKey(model)) then
					SetPlayerModel(PlayerId(), GetHashKey(model))
					end				
					elseif SHARKMenu.Button("~b~~h~avongoon") then
			local model = "mp_m_avongoon"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif SHARKMenu.Button("~b~~h~CJ") then
			local model = "g_m_y_strpunk_02"
				RequestModel(GetHashKey(model)) 
				Wait(500)
				if HasModelLoaded(GetHashKey(model)) then
					SetPlayerModel(PlayerId(), GetHashKey(model))
					end
					elseif SHARKMenu.Button("~b~~h~Niko") then
			local model = "mp_m_niko_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif SHARKMenu.Button("~b~~h~Pongo") then
			local model = "u_m_y_pogo_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif SHARKMenu.Button("~b~~h~Babyd") then
			local model = "u_m_y_babyd"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif SHARKMenu.Button("~b~~h~Fivem") then
			...
			elseif SHARKMenu.Button("~r~~h~Randomize Clothing") then
			...
			elseif SHARKMenu.IsMenuOpened("ESXBoss") then
				...
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("ESXMoney") then
				if SHARKMenu.Button("~h~~r~-»~r~~h~ BEST MONEY «-") then
			...
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~1") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent("esx_jobs:caution", "give_back", result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~2") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~3") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveBank', result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~4") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveCash', result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~5") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~6") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~7") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_slotmachine:sv:2", result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~8") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
					TriggerServerEvent("lscustoms:payGarage", {costs = -result})
				end		
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~9") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				TriggerServerEvent("vrp_slotmachine:server:2", result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~10") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveDirtyMoney', result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~11") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~12") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('delivery:success', result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~13") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent ('taxi:success', result)
				end
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~14") then
					...
					TriggerServerEvent('esx_taxijob:success')
					TriggerServerEvent('esx_taxijob:success')
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~15") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_garbagejob:pay", result)
				end	
			elseif SHARKMenu.Button("~g~~h~MONEY ~s~16") then
				...
				TriggerServerEvent('paycheck:salary')
				TriggerServerEvent('paycheck:salary')
			elseif SHARKMenu.Button("~s~Bank ~r~~h~Deposit") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				TriggerServerEvent("bank:deposit", result)
				end
			elseif SHARKMenu.Button("~s~Bank ~r~~h~Withdraw ") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
			...
			SHARKMenu.Display()
				elseif SHARKMenu.IsMenuOpened("ESXMisc") then
				    if SHARKMenu.Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
				...
				elseif SHARKMenu.Button("~g~~h~ESX ~w~Get all licenses Drive") then
				...
				elseif SHARKMenu.Button("~h~Send Discord Message") then
				...
				elseif SHARKMenu.Button("~h~Send Police Car Advert") then
				TriggerServerEvent("esx:enterpolicecar",GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
				end
				

				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("MiscServerOptions") then
				if SHARKMenu.Button("~h~Send Discord Message") then
					local Message = KeyboardInput("Enter message to send", "", 100)
					TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
					drawNotification("The message:~n~" .. Message .. "~n~Has been ~g~sent!")
				elseif SHARKMenu.Button("~h~Send Police Car Advert") then
				...
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("VRPOptions") then
				if SHARKMenu.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
						TriggerServerEvent("lscustoms:payGarage", {costs = -result})
					end		
				elseif SHARKMenu.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif SHARKMenu.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif SHARKMenu.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif SHARKMenu.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
				...
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("ESXDrugs") then
				if SHARKMenu.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif SHARKMenu.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
				elseif SHARKMenu.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif SHARKMenu.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif SHARKMenu.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
				elseif SHARKMenu.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
				elseif SHARKMenu.Button("~h~~r~Harvest Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif SHARKMenu.Button("~h~~r~Transform Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
				elseif SHARKMenu.Button("~h~~r~Sell Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
				elseif SHARKMenu.Button("~h~~p~Harvest Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif SHARKMenu.Button("~h~~p~Transform Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
				elseif SHARKMenu.Button("~h~~p~Sell Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
				elseif SHARKMenu.Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif SHARKMenu.Button("~r~~h~Stop all ~c~(Drugs)") then
				...
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and SHARKMenu.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
				
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("RecrutarPlayersOptions") then
				SHARKMenu.SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
			    if SHARKMenu.Button("~s~Recruit~c~~h~ Mechanic") then
			...
			elseif SHARKMenu.Button("~s~Recruit~b~~h~ Police") then
			...
			elseif SHARKMenu.Button("~s~Recruit~c~~h~ Mafia") then
			...
			elseif SHARKMenu.Button("~s~Recruit~p~~h~ Gang") then
			...
			elseif SHARKMenu.Button("~s~Recruit~r~~h~ Inem") then
			...
			elseif SHARKMenu.Button("~h~Custom Recruit") then
				...
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and SHARKMenu.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."~h~~y~ » "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~s~[~r~DEAD~s~]" or "~s~[~g~ALIVE~s~]"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("PlayerOptionsMenu") then
				SHARKMenu.SetSubTitle("PlayerOptionsMenu", "~h~Player ~s~~r~[" .. GetPlayerName(SelectedPlayer) .. "~r~]")
				if SHARKMenu.Button("Spectate", (Spectating and "~g~[SPECTATING]")) then
					SpectatePlayer(SelectedPlayer)
				elseif SHARKMenu.Button("spam msg") then
					TriggerServerEvent('_chat:messageEntered', GetPlayerServerId(SelectedPlayer), {0, 0x99, 255}, '/tw /ooc ^1GHOST menu ^4LUA :^2https://discord.gg/45zyGC7')
				elseif SHARKMenu.Button("Teleport To Player") then
					local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
					SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	
				elseif SHARKMenu.Button("~g~Give ~s~Money") then
				...
				elseif SHARKMenu.Button("~g~Revive ~g~ESX") then
					...
					TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(SelectedPlayer))
				elseif SHARKMenu.Button("~g~Revive ~b~VRP") then CreatePickup(GetHashKey("PICKUP_HEALTH_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif SHARKMenu.Button("Give ~b~Armour ") then CreatePickup(GetHashKey("PICKUP_ARMOUR_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif SHARKMenu.Button("~h~~r~Kill ~s~Player") then AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 4, 1337.0, false, true, 0.0) 
				elseif SHARKMenu.Button('Fire') then
				    FirePlayer(SelectedPlayer)
					elseif SHARKMenu.Button("Jail") then		
				...
				elseif SHARKMenu.Button("Unjail") then
				...
				elseif SHARKMenu.Button("~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
					end
					elseif SHARKMenu.Button("~r~Remove All Weapons") then
                    RemoveAllPedWeapons(PlayerPedId(-1), true)
				elseif SHARKMenu.MenuButton("Weapon Menu", "SingleWepPlayer") then
				elseif SHARKMenu.Button("Spawn Vehicle") then
					local ped = GetPlayerPed(SelectedPlayer)
					local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 100)
				...
				elseif SHARKMenu.Button('~h~Clone Car') then
                    ClonePedVeh()
				elseif SHARKMenu.Button("Cuff ~g~ESX") then
					TriggerServerEvent("esx_policejob:handcuff", GetPlayerPed(SelectedPlayer))
					TriggerServerEvent('esx_policejob:handcuff', GetPlayerPed(SelectedPlayer))
				elseif SHARKMenu.Button('~h~Spawn acult ~r~AK') then
                    for i = 0, 32 do
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                        RequestModel(GetHashKey('a_m_m_acult_01'))
                        Citizen.Wait(50)
                        if HasModelLoaded(GetHashKey('a_m_m_acult_01')) then
                            local ped =
                                CreatePed(21, GetHashKey('a_m_m_acult_01'), coords.x, coords.y, coords.z, 0, true, false)
                ...
                elseif SHARKMenu.Button('~h~Spawn Wade ~r~RPG') then
                    for i = 0, 32 do
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer))
				...
				elseif SHARKMenu.Button('~h~Spawn Swat in car ~r~Attack') then
					...
					elseif SHARKMenu.Button("~h~~b~--Vehicle Options--") then
				elseif SHARKMenu.Button("~b~Kick Vehicle") then
					ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
				elseif SHARKMenu.Button("~b~Fuck Engine ~r~NEW") then
				...
				elseif SHARKMenu.Button("~b~Repair Vehicle ~r~NEW") then
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				elseif SHARKMenu.Button("~b~Fuck Car ~r~NEW ") then
					local playerPed = GetPlayerPed(SelectedPlayer)
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
			    ...
			    elseif SHARKMenu.Button("~r~Explode ~b~Vehicle") then	
				...
				SHARKMenu.Display()
			elseif SHARKMenu.IsMenuOpened("SingleWepPlayer") then
				for i = 1, #allWeapons do
					if SHARKMenu.Button(allWeapons[i]) then
